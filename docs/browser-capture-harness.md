...
```bash
npm run capture-harness --workspace=@osuna/desktop
```
...
OSUNA_CAPTURE_HARNESS_GROUP=automation npm run capture-harness --workspace=@osuna/desktop
```

Run the shared browser profile fixture with:

```bash
OSUNA_CAPTURE_HARNESS_GROUP=browser-profile npm run capture-harness --workspace=@osuna/desktop
```
...
The existing `npm run test:e2e:browser-tabs --workspace=@osuna/desktop` journey
