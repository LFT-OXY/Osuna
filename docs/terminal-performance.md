...
- **Terminal size has one daemon-owned claimant.** Focus and direct interaction send a `claim`; later geometry changes from that connection send `update`. A claim transfers ownership even when the dimensions are unchanged, while an update from any other connection is ignored. This lets an owning pane follow splits and keyboard insets without allowing an idle phone or browser to steal the PTY size. Terminal colors follow the same owner: a `terminal_input.view_attributes` message is applied only when it comes from the connection that currently owns the size, so switching devices moves the colors together with the size claim and nothing else arbitrates between themes.
- **Terminal color queries are answered from client-pushed view attributes, and unknown means silent.** The daemon replies to OSC 10/11/12 and `CSI ?996n` with the foreground/background/cursor colors the client sent (`create_terminal_request.viewAttributes` at creation, `terminal_input.view_attributes` afterwards), still written straight to the PTY inside the worker. When no client has sent colors, the query is consumed and not answered. A fabricated default is the old "default black" bug: a light-themed client's TUI rendered a dark input box because the daemon claimed a near-black background. Dark/light for `?996n` is decided by comparing background and foreground relative luminance, not by the app's theme mode.
- **Theme changes reach a running TUI only through `DECSET 2031`.** The daemon's headless parser records whether the foreground program enabled mode 2031. When pushed colors change the dark/light classification (including from unknown to known), a subscribed program gets one `CSI ?997;1n` or `?997;2n`; an unsubscribed program gets nothing, and two palettes on the same side (light to light) send nothing, so a theme tweak never forces a full TUI redraw. The client pushes colors right after every size `claim` and whenever its theme changes, deduplicating only the theme-change path: after a claim the daemon may hold another device's colors, so that push is never skipped.

## Measuring

- **Node-only benchmark (fast iteration, server pipeline):** `npx tsx scripts/benchmark-terminal-latency.ts`. Boots an isolated daemon (fresh `OSUNA_HOME`, random port — never 6777), measures echo latency percentiles, burst jitter, and snapshot counts under ramped mock-agent load. Writes JSON to `/tmp/osuna-terminal-bench/`. Healthy numbers (2026-06): echo p50 ~2.3ms, p95 ~3.3ms, a 2MB burst fully streamed with `snap=0`.
- **Browser perf specs (user-perceived path):** gated behind `OSUNA_TERMINAL_PERF_E2E=1` —
