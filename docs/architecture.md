...
Osuna is a client-server system for monitoring and controlling local AI coding agents. The daemon runs on your machine, manages agent processes, and streams their output in real time over WebSocket. Clients (mobile app, CLI, desktop app) connect to the daemon to observe and interact with agents.

Your code never leaves your machine. Osuna is local-first.
...
The heart of Osuna. A Node.js process that:
...
| `server/agent/agent-storage.ts` | File-backed JSON persistence at `$OSUNA_HOME/agents/`                          |
| `server/agent/tools/`           | Transport-neutral catalog for workspaces, agents, permissions, and automation  |
| `server/agent/mcp-server.ts`    | Thin MCP adapter that registers the Osuna tool catalog with the MCP SDK        |
...
and clients. Server, app, CLI, and `@osuna/client` all depend on this package;
...
`@osuna/client/internal/daemon-client` during migration, while new SDK-shaped
code imports from `@osuna/client`.
...
Commander.js CLI with Docker-style commands. Common agent operations are also exposed at the top level (e.g. `osuna ls`, `osuna run`).

- `osuna agent ls/run/import/attach/logs/stop/delete/send/inspect/wait/archive/reload/update/mode`
- `osuna daemon start/run/stop/restart/status/reload/config/pair/set-password`
- `osuna terminal ls/create/capture/send-keys/kill`
- `osuna script ls/start/stop`
- `osuna schedule create/ls/inspect/update/pause/resume/run-once/logs/delete`
- `osuna heartbeat create/update/delete`
- `osuna project create/ls/rename/delete`
- `osuna workspace create/ls/rename/archive`
- `osuna permit allow/deny/ls`
- `osuna provider ls/models`
- hidden legacy `osuna worktree create/ls/archive` compatibility alias
- `osuna speech …`
...
>
> **Browser keyboard boundary.** Guest pages receive renderer-published shortcuts first. `Cmd/Ctrl+L` and `Cmd/Ctrl+R` are explicit guest-shell reservations; ordinary Osuna shortcuts run only after the page declines them. The sandboxed guest preload runs in every frame so focused iframes use the same boundary, while Node integration remains disabled. Human guest input disables Electron's menu fallback for plain keys. Agent-generated keys use guest `sendInputEvent` with `skipIfUnhandled`, so an unhandled Enter stops at the guest instead of reaching the host composer. Main selects the preload; it exposes no APIs to guest pages.
...
Human key -> guest WebContents
  ...
  `-- page keydown
        ...
        `-- published shortcut -> guest preload -> IPC(browserId) -> Osuna resolver
...
TanStack Router + Cloudflare Workers. Not deployed by this fork and outside the npm workspaces.
...
- Agent state persists to `$OSUNA_HOME/agents/{cwd-with-dashes}/{agent-id}.json`. Timeline rows are runtime memory; provider history is the durable transcript authority and resumed agents rebuild from it. That storage path is derived from `cwd`, not from workspace id.
...
- Handle their own authentication (Osuna does not manage API keys)
...
Providers that can accept native tool definitions should set `supportsNativeOsunaTools` and read `launchContext.osunaTools`. The daemon then passes the shared Osuna tool catalog directly and removes the internal Osuna MCP server from that provider launch config. Providers that only support MCP continue to receive the same tools through the MCP fallback at `/mcp/agents`.
...
`$OSUNA_HOME` defaults to `~/.osuna`. The most important files:
