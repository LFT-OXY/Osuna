...
Provider message IDs are not guaranteed for every displayed item. Osuna-generated system errors are one example. Rendered item indices are not durable either because pagination and projection can merge source rows.
...
the Osuna timeline, so one visible response can span several canonical turns without a user message
