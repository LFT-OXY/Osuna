...
Osuna proxies HTTP traffic to services running inside your workspaces. Localhost service URLs are always enabled; optional public aliases and a separate service-only listener can be layered on through config.

## How it works

When a `osuna.json` script of `"type": "service"` starts, Osuna assigns it a local port and registers a route in the service proxy. Incoming requests whose `Host` header matches the script's generated hostname are forwarded to that port.
...
Local and public routes use one combined leftmost label (`script--branch--project`). This keeps the hostname compatible with normal single-level wildcard DNS and TLS. If the combined label would exceed DNS's 63-character label limit, Osuna truncates it with a deterministic hash suffix to avoid collisions.

## Managing workspace scripts

Configured `osuna.json` scripts can be managed without addressing their backing terminal directly:
...
Add a `serviceProxy` block under `daemon` in `~/.osuna/config.json`:
...
```json
{
  ...
  "daemon": {
    "serviceProxy": {
      "listen": "0.0.0.0:8080",
      "publicBaseUrl": "https://osunaapps.my.domain.com"
    }
  }
}
```
...
For generated URLs to be reachable, you need wildcard DNS pointing to the machine running the Osuna daemon.

**Example:** to expose services at `https://dev--miniweb.osunaapps.my.domain.com` where the daemon host is `10.1.1.1`:

1. Configure a wildcard DNS record:

   ```
   *.osunaapps.my.domain.com  →  10.1.1.1
   ```

2. Set `publicBaseUrl` to `https://osunaapps.my.domain.com` in your config.

3. If you put a reverse proxy (nginx, Caddy, Traefik, etc.) in front of Osuna, point it at either the daemon listener or the optional service-only listener and ensure it forwards the `Host` header unchanged. The proxy uses the `Host` header to route requests to the correct service — rewriting it will break routing.
...
`OSUNA_TRUSTED_PROXIES` accepts the same comma-separated values, for example `loopback,172.16.0.0/12`. Use `true` only when the final trusted proxy overwrites client-supplied `X-Forwarded-*` headers.
...
server {
    listen 443 ssl;
    server_name *.osunaapps.my.domain.com;
...
Osuna sets these when it forwards a request to a workspace service:
...
| ------------------------------------- | ------------------------------------------------------------------------- |
| `OSUNA_SERVICE_PROXY_ENABLED`         | Compatibility shim; `false` suppresses optional public/listen layers only |
| `OSUNA_SERVICE_PROXY_LISTEN`          | Starts the optional service-only listener, e.g. `0.0.0.0:8080`            |
| `OSUNA_SERVICE_PROXY_PUBLIC_BASE_URL` | Adds public service aliases and links                                     |
