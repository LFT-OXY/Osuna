...
catalog probe. `OSUNA_PROVIDER_REFRESH_TIMEOUT_MS` sets it when the config field is absent.
...
Osuna passes those variables through to the Codex app-server process **and** maps them into Codex's thread config under `model_provider` / `model_providers`, because Codex reads provider routing from config rather than from `OPENAI_BASE_URL` alone.
...
Under the hood, for each custom Codex provider Osuna injects this into Codex's config:
...
- `base_url` — taken from `OPENAI_BASE_URL`. If it does not already end in `/v1`, Osuna appends `/v1`. Trailing slashes are stripped.
- `wire_api` — always `"responses"` (OpenAI Responses API protocol).
- `env_key` — set to `"OPENAI_API_KEY"` when that env var is present and non-empty, so Codex reads the key from the same env var Osuna passes through.
...
- Set `models` explicitly. Custom endpoints expose their own model IDs (`anthropic/claude-opus-4-7`, `qwen/qwen3-coder`, `local/llama`, etc.), and Osuna does not discover them automatically for Codex.
...
Each profile appears as a separate provider in the Osuna app. You can select which one to use when launching an agent.
...
The `command` array completely replaces the default command for that provider. The binary must exist on the system — Osuna checks for its availability and will mark the provider as unavailable if not found.
...
Custom OMP profiles should extend `omp`. They inherit the OMP adapter's `rpc-ui` approvals, native Osuna host tools, provider-managed subagents, and import behavior:
...
The [Agent Client Protocol (ACP)](https://agentclientprotocol.com) is an open standard for communication between editors and AI coding agents — think LSP but for AI agents. Any agent that supports ACP can be added to Osuna as a custom provider.

ACP agents communicate over JSON-RPC 2.0 on stdio. Osuna spawns the agent process and talks to it through stdin/stdout.

Osuna also ships an in-app ACP provider catalog for common agents, including CodeWhale, Cursor, DeepAgents, DimCode, Gemini CLI, Hermes, Qwen Code, and Kimi Code. Catalog entries create the same `extends: "acp"` provider config shown below.
...
Osuna tools such as subagent creation come from the shared internal tool catalog. ACP providers receive those tools through the MCP fallback by default because ACP exposes `mcpServers`, not Osuna's native tool catalog. Some ACP adapters cannot create sessions when `mcpServers` is non-empty. Disable injected MCP for those providers with `params.supportsMcpServers: false`:
...
while terminal operations run through Osuna on the host. To customize which
operations Osuna handles, configure client capabilities in provider params:
...
container. When delegating filesystem operations to Osuna (`fs.readTextFile: true`
or `fs.writeTextFile: true`), ensure the agent and Osuna share equivalent
absolute workspace paths.

### Generic ACP diagnostics

Osuna diagnostics for `extends: "acp"` providers report the configured command, resolved launcher binary, version output, ACP `initialize`, ACP `session/new`, model count, modes, and final status.
...
### How ACP providers work in Osuna

When you launch an agent with an ACP provider:

1. Osuna spawns the process using the configured `command`
2. Sends an `initialize` JSON-RPC request over stdin
3. The agent responds with its capabilities, available modes, and models
4. Osuna creates a session and sends prompts through the ACP protocol
5. The agent streams responses, tool calls, and permission requests back over stdout

Every ACP provider exposes an **Auto Accept** toggle. Enable it per session to let Osuna approve
ACP permission requests without surfacing each prompt. If the provider sends no allow option,
Osuna leaves the request for you to answer. Unattended agents enable Auto Accept unless you
