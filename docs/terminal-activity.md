...
Osuna surfaces terminal activity as a tab indicator (the same "running" dot used by agents).
...
The plugin translates both event contracts into the existing Osuna hook events. OpenCode 2 disposes its event subscription when the plugin unloads.
...
removes Osuna's marker-matched hooks on disable. `osuna hooks` keeps working regardless — the gate
...
When enabled, Osuna installs provider hooks globally:

- Claude hooks are written to `~/.claude/settings.json` (or `CLAUDE_CONFIG_DIR/settings.json` when that override is set).
- Codex hooks are written to `~/.codex/hooks.json` (or `CODEX_HOME/hooks.json` when that override is set). Codex supports a native `commandWindows`, so each Osuna hook includes both POSIX and Windows commands. Non-managed Codex hooks are trust-gated by Codex; users may see Codex's hook review prompt before the hook runs.
- OpenCode gets a self-contained plugin at `$XDG_CONFIG_HOME/opencode/plugins/osuna-terminal-activity.js` (or `~/.config/opencode/plugins/osuna-terminal-activity.js` when XDG is unset; `OPENCODE_CONFIG_DIR` still wins when set).

Installation is marker-based/idempotent for config hooks and exact-file/idempotent for the OpenCode plugin. Osuna preserves user hooks, removes only its own marker-matched command hooks, and leaves hooks installed across daemon shutdown. Outside an Osuna terminal they are inert because the command or plugin is gated on `OSUNA_TERMINAL_ID`.
...
```sh
[ -n "$OSUNA_TERMINAL_ID" ] && "${OSUNA_HOOK_CLI:-osuna}" hooks claude <event>
```

Codex also receives the Windows equivalent:

```bat
if defined OSUNA_TERMINAL_ID (if defined OSUNA_HOOK_CLI ("%OSUNA_HOOK_CLI%" hooks codex <event>) else (osuna hooks codex <event>))
```

The daemon resolves the current CLI through `OSUNA_CLI` when its launcher supplies one, or through the npm package shim for standalone installs. Terminal setup exposes that resolved executable to hooks as `OSUNA_HOOK_CLI`; desktop and other daemon launchers do not know about the hook-specific variable. The generated command falls back to bare `osuna` if the hook env is missing and no-ops outside Osuna terminals because the `OSUNA_TERMINAL_ID` gate remains first. Osuna also prepends the resolved CLI directory to each terminal `PATH` as a secondary fallback. All other behavior lives in `osuna hooks`: read the env, map the event, POST activity, and no-op/fail-open when anything is missing or unavailable.
