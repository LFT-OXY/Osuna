...
- **`lineHeight.diff` is the code/diff line-height axis** — it is coupled to the code-font-size control (≈ `codeFontSize * 1.5`). Do NOT use it for prose. Markdown body line-height scales with content size (the `prose` step, see [design.md](design.md) §3); routing prose through `lineHeight.diff` clips text at small code sizes.
