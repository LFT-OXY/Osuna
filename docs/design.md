...
The composer is the page's raised input: a `surfaceCard` panel, glass on web and Electron (§6), with `radius["3xl"]` corners and a 1px `borderComposer` outline, the foreground at 9%. Light casts the `shadowComposer` drop shadow; Dark casts none and draws the `insetHighlight` top rule. The toolbar holds the Agent controls on the left as 28px ghost buttons (`radius.md`, `label` text, `interactionHighlight` when hovered or open) and, on the right, attach, the context meter, voice, then Stop and Send as 32px circles; the surface is `composerSurfaceStyle` in `packages/app/src/styles/floating-surface.ts`. Stop is a `foreground` fill, not red. While the agent runs, Stop stays and Send joins it once there is a draft; what Send does is still the running-send setting. The right group never shrinks: in a narrow split pane the Agent controls condense and clip first. In workspace panes a read-only strip hangs from the composer's bottom edge with Worktree or Local, the branch, and the host badge, inset 24px on each side, `surface2` under the same outline; compact layouts drop it.
