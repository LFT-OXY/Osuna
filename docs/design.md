...
Workspace chrome draws no horizontal rule between itself and the content. The workspace header, the workspace tab row, the Explorer tab row, and pane toolbars (`PaneContentToolbar`, the browser pane's bar) sit on the content's surface with no rule and no shadow, the left sidebar draws none between its items, its callout, and its footer, and the Changes view draws none above its Commits section; spacing and surface carry the grouping. The sticky diff file header keeps its bottom rule, because it scrolls over code. Vertical dividers — sidebar to workspace, split to split — stay. On desktop the workspace header is a breadcrumb: the project in `foregroundMuted`, a `foregroundExtraMuted` `/`, then the workspace `<ScreenTitle>`. Compact keeps the project on its own line under the title, next to the host badge.
