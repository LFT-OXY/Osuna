...
Desktop dev launches its desktop-managed daemon with `OSUNA_NODE_ENV=development`,
...
Linux produces the `osuna-desktop` launcher and desktop entry. macOS produces
`Applications/Osuna.app` plus the `osuna-desktop` launcher. Both use the nixpkgs
...
```bash
OSUNA_HOME=~/.osuna-blue npm run dev          # explicit home
OSUNA_DEV_SEED_HOME=/path/to/home npm run dev # seed from a different source home
OSUNA_DEV_RESET_HOME=1 npm run dev            # clear and reseed the derived worktree home
```
...
Osuna worktrees expose the native iOS dev app through the `ios-simulator` service in `osuna.json`. The service URL serves the simulator preview at `/.sim`, so the preview link is `${OSUNA_URL}/.sim`.
...
- **An iOS Simulator runtime with at least one iPhone device type**. Recent Xcode versions may not bundle a runtime — add one via Xcode → Settings → Components (older Xcode: "Platforms"). The service targets `iPhone 16 Pro` by default (override with `OSUNA_IOS_DEVICE_TYPE`) and falls back to any iPhone; it fails with `No iPhone simulator device type is installed` when none exist.
...
`OSUNA_ELECTRON_REMOTE_DEBUGGING_PORT` when a QA workflow requires a validated,
fixed port.

Desktop dev also scopes Electron `userData` to the current dev root. This prevents
desktop-only environment inherited by terminals opened inside Osuna from coupling
...
```bash
npm run verify:electron-cdp --workspace=@osuna/desktop
```

The verifier reads the same `EXPO_PORT` and
`OSUNA_ELECTRON_REMOTE_DEBUGGING_PORT` environment names as desktop dev. Set an
...
`OSUNA_PORT=57928` alone starts Metro on 57928 but Electron still loads 8081.
...
Captured samples are exposed on `globalThis.__OSUNA_RENDER_PROFILE__`. Call
`globalThis.__OSUNA_RESET_RENDER_PROFILE__?.()` after warm-up and before the
...
reason counts are exposed on `globalThis.__OSUNA_RENDER_PROFILE_REASONS__`.
...
```bash
OSUNA_PROFILE_SERVER_ID=<server-id> \
OSUNA_PROFILE_WORKSPACE_ID=<workspace-path> \
OSUNA_PROFILE_AGENT_ID=<agent-id> \
  npm run profile:workspace-tabs --workspace=@osuna/app
```
...
```bash
OSUNA_PROFILE_APP_URL=http://localhost:19010 # Expo web URL
OSUNA_PROFILE_SWITCH_COUNT=1                # number of agent/terminal switch pairs
OSUNA_PROFILE_SWITCH_WAIT_MS=250            # delay after each click
OSUNA_PROFILE_IDLE_WAIT_MS=3000             # idle baseline before switching
OSUNA_PROFILE_DUMP_COMMITS=1                # include per-commit profiler samples
```
...
```bash
OSUNA_PROFILE_APP_URL=http://localhost:19010 \
  npm run profile:workspace-switching --workspace=@osuna/app
```
...
React commits on the same browser clock. Set `OSUNA_PROFILE_WORKSPACE_DIGITS`,
`OSUNA_PROFILE_WORKSPACE_LRU_SIZE`, or `OSUNA_PROFILE_WARM_QUIET_MS` to change the shape. Set
`OSUNA_PROFILE_DUMP_COMMITS=1` to include the nested component breakdown for every commit.
Set `OSUNA_PROFILE_RETAINED_REPEATS=5` to repeat the retained-LRU burst for a less noisy sample.
Set `OSUNA_PROFILE_CPU_PATH=/tmp/workspace-switch.cpuprofile` to run a separate retained-LRU
capture after the latency scenarios and write a raw CDP CPU profile without contaminating their
timings.
Set `OSUNA_PROFILE_TRACE_PATH=/tmp/workspace-switch.trace.json` to run another separate retained-LRU
capture and write a Chromium Performance trace with User Timing marks and screenshots. Open the
trace in the Chrome DevTools Performance panel. Set `OSUNA_PROFILE_TRACE_INVALIDATIONS=1` only for a
focused invalidation capture; React Native's generated stacks make that mode highly intrusive.
Set `OSUNA_PROFILE_TRACE_FOCUS=1` to include focus targets, durations, and JavaScript call stacks in
...
`OSUNA_PROFILE_CPU_PATH=/tmp/explorer-toggle.cpuprofile` for separate Chromium captures. Override
the app URL, daemon port, workspace, or server with the corresponding `OSUNA_PROFILE_*` variables.
...
```bash
OSUNA_PROFILE_APP_URL=http://localhost:19010 \
  npm run profile:composer-typing --workspace=@osuna/app
```
...
Playwright dispatch lateness. Set `OSUNA_PROFILE_TYPING_KEYS`, `OSUNA_PROFILE_TYPING_CADENCE_MS`,
`OSUNA_PROFILE_TYPING_REPEATS`, or `OSUNA_PROFILE_WORKSPACE_DIGIT` to change the scenario. Optional
`OSUNA_PROFILE_CPU_PATH` and `OSUNA_PROFILE_TRACE_PATH` captures run separately after the latency
measurements so profiling overhead does not contaminate them.

Set `OSUNA_PROFILE_TYPING_SCENARIO=height-growth` to alternate `Shift+Enter` and printable input.
...
Desktop development can replace native macOS traffic lights with Osuna's custom controls:

```bash
OSUNA_DESKTOP_WINDOW_CONTROLS=windows npm run dev:desktop
OSUNA_DESKTOP_WINDOW_CONTROLS=linux npm run dev:desktop
```
...
Check `$OSUNA_HOME/daemon.log` for daemon logs. The default level is `info`; set
`OSUNA_LOG_LEVEL=trace` before launching the daemon when you need full provider,
...
`$OSUNA_HOME/config.json` win first. Without persisted config, the optional
`OSUNA_LOG_ROTATE_SIZE` and `OSUNA_LOG_ROTATE_COUNT` env vars override the
...
the daemon-global Git process limits in `$OSUNA_HOME/config.json`:
...
Reload the daemon with `osuna reload`. Environment-variable overrides still require a restart because
...
Measure the MCP `tools/list` payload that Osuna injects into agents with:

```bash
npm run measure:agent-tools --workspace=@osuna/server
```
...
Windows. They inherit the daemon environment plus Osuna's lifecycle variables;
...
{
  "worktree": {
    "setup": "npm ci\ncp \"$OSUNA_SOURCE_CHECKOUT_PATH/.env\" .env\nnpm run db:migrate",
...
| Variable                    | Value                                                                                                                     |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `OSUNA_SERVICE_<NAME>_URL`  | Proxied URL for a declared peer service. Prefer this for peer discovery; it survives peer restarts.                       |
| `OSUNA_SERVICE_<NAME>_PORT` | Raw ephemeral port for a declared peer service. Use only as a bypass escape hatch; it can go stale if that peer restarts. |
| `OSUNA_URL`                 | Self alias for `OSUNA_SERVICE_<SELF>_URL`.                                                                                |
| `OSUNA_PORT`                | Self alias for `OSUNA_SERVICE_<SELF>_PORT`.                                                                               |
...
```json
{
  "scripts": {
    "web": {
      "type": "service",
      "command": "PORT=$OSUNA_PORT npm run dev:web"
    }
  }
}
```
...
`$OSUNA_HOME/config.json`, or replace it for one project with `worktree.servicePorts` in
`osuna.json`. The block accepts an inclusive `range` such as `"3000-4000"` or a `portScript`
...
values are available as `OSUNA_SCRIPTNAME`, `OSUNA_WORKSPACE_ID`, `OSUNA_BRANCH_NAME`, and
`OSUNA_WORKTREE_PATH`. The script must print one valid TCP port. Osuna trusts the external allocator,
...
The desktop-managed daemon disables the bundled web UI by default (`OSUNA_WEB_UI_ENABLED=false`) because the desktop app already ships the renderer as `app-dist`. Shipping the same assets again inside `@osuna/server` would duplicate the ~10.8 MiB install. Desktop packaging also excludes `node_modules/@osuna/server/dist/server/web-ui/**` from the packaged app.
...
`npm run dev:server` builds the server-side workspace packages once, then keeps `@osuna/protocol` and `@osuna/client` fresh with TypeScript watch builds while the daemon runs. If you change protocol schemas or client code outside that watch workflow, rebuild the producer before trusting runtime behavior.
...
Use `npm run cli` to run the in-repo CLI from source (`npx tsx packages/cli/src/index.ts`). The script wraps the CLI with `scripts/dev-home.sh`, so it automatically uses this checkout's `.dev/osuna-home` and dev daemon endpoint unless you pass an explicit override. The globally installed `osuna` binary on macOS is a symlink into the installed Osuna desktop app, not this checkout — use it to drive the desktop's built-in daemon, but use `npm run cli` when you want to talk to the CLI you are editing.

Canonical automation uses `osuna project create/ls/rename/delete`, `osuna workspace create/ls/rename/archive`, `osuna heartbeat create/update/delete`, and the full `osuna schedule` group. MCP heartbeat automation is intentionally smaller: create and delete only. Detach remains an explicit user lifecycle action rather than an agent tool. `osuna run --new-workspace local|worktree` composes workspace creation with agent creation. The old `osuna worktree` and `osuna run --worktree` forms are hidden compatibility aliases.
...
npm run cli -- agent open <id>       # Focus an existing agent in Osuna Desktop
...
`osuna agent open <agent-id>`. The CLI reads the local daemon's server ID by
...
```
$OSUNA_HOME/agents/{cwd-with-dashes}/{agent-id}.json
```

Find an agent by ID:

```bash
find $OSUNA_HOME/agents -name "{agent-id}.json"
```

Find by content:

```bash
rg -l "some title text" $OSUNA_HOME/agents/
```
...
Point Playwright MCP at the running Expo web target. For root checkout dev, `npm run dev:app` reserves `http://localhost:8081`. For Osuna-managed worktree app services, use the service URL or port shown by Osuna for that worktree.
...
directory to Cloudflare Pages with `npm run deploy:web --workspace=@osuna/app`.
...
Do not add service-worker caching casually. Osuna is a live control surface for
