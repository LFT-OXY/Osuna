...
const serverId = args["server-id"] ?? process.env.OSUNA_SERVER_ID;
const daemonPublicKeyB64 = args["daemon-public-key-b64"] ?? process.env.OSUNA_DAEMON_PUBLIC_KEY_B64;
const relayEndpoint =
  args["relay-endpoint"] ?? process.env.OSUNA_RELAY_ENDPOINT ?? "relay.paseo.sh:443";
const baseUrl = args["base-url"] ?? process.env.OSUNA_APP_URL ?? "https://app.paseo.sh";
const timeoutMs = Number(args["timeout-ms"] ?? process.env.OSUNA_PROVE_TIMEOUT_MS ?? 60_000);
const stabilityMs = Number(args["stability-ms"] ?? process.env.OSUNA_PROVE_STABILITY_MS ?? 30_000);

if (!serverId || typeof serverId !== "string") {
  console.error("Missing server ID. Provide --server-id or OSUNA_SERVER_ID.");
  process.exit(2);
}
if (!daemonPublicKeyB64 || typeof daemonPublicKeyB64 !== "string") {
  console.error(
    "Missing daemon public key. Provide --daemon-public-key-b64 or OSUNA_DAEMON_PUBLIC_KEY_B64.",
  );
  process.exit(2);
}
...
await page.addInitScript(
  (seed) => {
    localStorage.setItem("@osuna:daemon-registry", JSON.stringify([seed.daemon]));
