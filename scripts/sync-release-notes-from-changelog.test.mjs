...
function withTempChangelog(fn, changelogText = "## 0.1.60-beta.1 - 2026-04-20\n\n- Beta notes.\n") {
  ...
  const tempDir = mkdtempSync(path.join(tmpdir(), "osuna-release-notes-test-"));
...
test("uses the untagged URL slug for draft release CLI operations", () => {
  assert.equal(
    getReleaseLookupTag({
      draft: true,
      html_url: "https://github.com/lft-oxy/osuna/releases/tag/untagged-draft",
...
test("does not treat GitHub authentication failures as missing releases", () => {
  ...
  assert.throws(
    () =>
      getGitHubRelease("lft-oxy/osuna", "v0.1.60-beta.1", (command, args) => {
...
test("updates an existing release body through the release id API", () => {
  withTempChangelog(() => {
    ...
    const execFileSync = (command, args, options) => {
      ...
      if (args[0] === "api" && args[1] === "repos/lft-oxy/osuna/releases/tags/v0.1.60-beta.1") {
    ...
    syncReleaseNotes(["--repo", "lft-oxy/osuna", "--tag", "v0.1.60-beta.1"], {
      execFileSync,
    });
    ...
    assert.equal(
      calls.some(
        (call) =>
          ...
          call.args[1] === "-X" &&
          call.args[2] === "PATCH" &&
          call.args[3] === "repos/lft-oxy/osuna/releases/311163621",
      ),
      true,
...
test("updates a draft release body without publishing it", () => {
  withTempChangelog(() => {
    ...
    const execFileSync = (command, args, options) => {
      ...
      if (args[0] === "api" && args[1] === "repos/lft-oxy/osuna/releases?per_page=100") {
        return JSON.stringify([
          {
            id: 311163621,
            draft: true,
            name: "Osuna v0.1.60-beta.1",
...
test("creates missing beta releases as drafts", () => {
  withTempChangelog(() => {
    ...
    syncReleaseNotes(
      ["--repo", "lft-oxy/osuna", "--tag", "v0.1.60-beta.1", "--create-if-missing"],
      { execFileSync },
    );
...
test("converts contributor profile links to mentions in synced release notes", () => {
  const changelogText = [
    ...
    "",
    "- Beta notes. ([#526](https://github.com/lft-oxy/osuna/pull/526) by [@therainisme](https://github.com/therainisme))",
    "",
  ].join("\n");
