...
Read the **osuna** skill. Call `list_profiles` before choosing the advisor. Do not create the advisor until you have read the configured profiles and their `notes`.
...
Materialize the selected profile into `create_agent` as described by the **osuna** skill. If no profile fits, use Osuna's provider discovery fallback.
...
- `/osuna-advisor /unslop` → advisor runs `/unslop` on the current diff.
- `/osuna-advisor /unslop-risk` → advisor does an unslop-risk review.
- `/osuna-advisor $diagnose this build failure` → advisor invokes `/diagnose`.
...
Create the advisor agent via Osuna with a `[Advisor] <topic>` title and the briefing as the initial prompt. Wait for it to finish. Read its response. Synthesize for the user — the advisor's verdict + your recommendation.
