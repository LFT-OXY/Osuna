---
name: osuna-help
description: Answer questions about the Osuna product and app, including setup, configuration, connectivity, providers, workspaces, updates, logs, and troubleshooting. Use when a user inside Osuna asks how Osuna works, how to configure it, or why something is broken; use the osuna skill instead to operate agents and workspaces through MCP or the CLI.
---

# Osuna Help

You are helping a user understand, configure, or troubleshoot Osuna itself. Answer their question directly, verify the answer against the current public documentation, and include the relevant documentation link. Do not send the user away to read the docs in place of helping them.
...
Identify the daemon involved before diagnosing versions, paths, providers, logs, updates, or connectivity. Do not infer the daemon from the client: Osuna Desktop can manage its bundled local daemon and connect to other remote daemons at the same time.
...
Use **Settings → About** to compare the app version with each connected host. For the affected host, open **Settings → your host → Overview → Full status**. On the daemon machine, `osuna daemon status --json` reports facts such as server ID, hostname, version, home, listen address, process owner, log path, and whether the daemon is desktop-managed.

Record which host the user is viewing and which machine or container runs it. A local `osuna daemon status` describes the daemon for that CLI's local `OSUNA_HOME`; it may not be the remote host visible in the app.
...
```bash
osuna --version
osuna daemon status --json
osuna provider diagnostic <provider> --json
```

Use the status-reported home, listen address, and log path for further checks. Probe `http://127.0.0.1:6777/api/health` or read `~/.osuna/daemon.log` only when those values match the affected daemon. Do not restart the daemon, edit config, update software, or expose a network listener without the user's explicit permission. A daemon restart can interrupt the agent doing the diagnosis.

For a missing provider or `command not found`, run `osuna provider diagnostic <provider>` against the affected host, or open **Settings → your host → Providers → provider → Diagnostic**. Compare its resolved binary, daemon `PATH`, and provider version with a brand-new login shell. Shell aliases and functions are not executable paths.
...
- Daemon config: `~/.osuna/config.json`
- Daemon log: `~/.osuna/daemon.log`
- Agent state directory: `~/.osuna/agents/`
- Default managed worktree root: `~/.osuna/worktrees/`
- macOS desktop log: `~/Library/Logs/Osuna/main.log`
- Linux desktop log: `~/.config/Osuna/logs/main.log`
- Windows desktop log: `%APPDATA%\Osuna\logs\main.log`

Substitute the status-reported `OSUNA_HOME` for `~/.osuna`. In the official Docker image, the default is `/home/osuna/.osuna`; its host path depends on the volume mount, and container stdout is available through Docker. Desktop app logs describe the Desktop process; daemon logs describe the selected daemon. Read the narrowest useful slice and redact credentials, pairing offers, tokens, passwords, and user code before sharing logs.

If diagnosing the bundled daemon on a computer with Osuna Desktop installed, but `osuna` is not on `PATH`, the bundled CLI is at:

- macOS: `/Applications/Osuna.app/Contents/Resources/bin/osuna`
- Linux: `<install-dir>/resources/bin/osuna`
- Windows: `C:\Program Files\Osuna\resources\bin\osuna.cmd`
