...
```bash
osuna plugin install /absolute/path/to/plugin-examples/agent-configuration
```

The callback receives the request after earlier plugins have transformed it. The nested spreads
preserve their settings; the explicit values replace only the named fields. Osuna validates and saves
