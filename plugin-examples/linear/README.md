...
```bash
osuna plugin add /absolute/path/to/osuna/plugin-examples/linear
osuna plugin ls linear
```

Use `osuna plugin reload linear` after source edits. Changing the API key requires restarting the
