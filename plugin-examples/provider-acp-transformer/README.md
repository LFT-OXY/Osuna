...
and SDK details behind Osuna's callback-based provider connection.

`server/vendor-edit.ts` shows the intended extension seam: a focused transformer converts one
vendor tool payload into Osuna's ordinary file-edit shape. It does not translate the whole event
stream.
