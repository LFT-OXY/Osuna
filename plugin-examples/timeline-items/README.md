...
calls return `undefined`, leaving Osuna's original timeline entry unchanged.

The transformer is a pure client contribution. It receives each source item and its streaming phase
while Osuna builds the render model, then returns plain plugin item objects. The renderer validates
`data` before Osuna mounts the component.
