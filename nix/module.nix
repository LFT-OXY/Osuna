...
let
  cfg = config.services.osuna;
in
{
  imports = [
    (lib.mkRenamedOptionModule [ "services" "osuna" "allowedHosts" ] [ "services" "osuna" "hostnames" ])
  ];

  options.services.osuna = {
    enable = lib.mkEnableOption "Osuna, a self-hosted daemon for AI coding agents";

    package = lib.mkPackageOption pkgs "osuna" { };

    user = lib.mkOption {
      type = lib.types.str;
      default = "osuna";
      description = "User account under which Osuna runs.";
    };

    group = lib.mkOption {
      type = lib.types.str;
      default = "osuna";
      description = "Group under which Osuna runs.";
    };
    ...
    dataDir = lib.mkOption {
      ...
      default =
        if cfg.user == "osuna"
        then "/var/lib/osuna"
        else "/home/${cfg.user}/.osuna";
      defaultText = lib.literalExpression ''
        if cfg.user == "osuna"
        then "/var/lib/osuna"
        else "/home/''${cfg.user}/.osuna"
      '';
      description = "Directory for Osuna state (OSUNA_HOME). Stores agent data, config, and logs.";
    };

    port = lib.mkOption {
      type = lib.types.port;
      default = 6777;
      description = "Port for the Osuna daemon to listen on.";
    };

    listenAddress = lib.mkOption {
      type = lib.types.str;
      default = "127.0.0.1";
      description = "Address for the Osuna daemon to bind to.";
    };

    openFirewall = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Whether to open the firewall for the Osuna daemon port.";
    };
    ...
    hostnames = lib.mkOption {
      ...
      description = ''
        Hostnames the Osuna daemon accepts in the Host header (DNS rebinding protection).
    ...
    inheritUserEnvironment = lib.mkOption {
      type = lib.types.bool;
      default = cfg.user != "osuna";
      defaultText = lib.literalExpression ''cfg.user != "osuna"'';
      description = ''
        Whether to include the user's profile PATH in the service environment.

        When Osuna runs as a real user (not the default system user), AI agents
      ...
      example = lib.literalExpression ''
        {
          ...
          log.file = { level = "info"; path = "/var/lib/osuna/daemon.log"; };
        }
      '';
      description = ''
        Declarative content for `$OSUNA_HOME/config.json`. Rendered to JSON
        and installed on every service start.

        Runtime mutations to `config.json` (e.g. via `osuna daemon set-password`
  ...
  config = lib.mkIf cfg.enable (
    let
      settingsFile = (pkgs.formats.json { }).generate "osuna-config.json" cfg.settings;
    in
    {
    assertions = [
      {
        ...
        message = ''
          services.osuna.relay.host must be set when relay.mode = "remote".
        '';
      }
    ];

    users.users.${cfg.user} = lib.mkIf (cfg.user == "osuna") {
      isSystemUser = true;
      group = cfg.group;
      home = cfg.dataDir;
    };

    users.groups.${cfg.group} = lib.mkIf (cfg.group == "osuna") { };

    systemd.tmpfiles.rules = [
      "d ${cfg.dataDir} 0700 ${cfg.user} ${cfg.group} - -"
    ];

    systemd.services.osuna = {
      ...
      environment = {
        OSUNA_HOME = cfg.dataDir;
        OSUNA_LISTEN = "${cfg.listenAddress}:${toString cfg.port}";
        ...
        OSUNA_HOSTNAMES = "true";
      } // lib.optionalAttrs (lib.isList cfg.hostnames && cfg.hostnames != [ ]) {
        OSUNA_HOSTNAMES = lib.concatStringsSep "," cfg.hostnames;
      } // lib.optionalAttrs (cfg.relay.enable && cfg.relay.mode == "remote") {
        OSUNA_RELAY_ENDPOINT = "${cfg.relay.host}:${toString cfg.relay.port}";
        OSUNA_RELAY_USE_TLS = if cfg.relay.useTls then "true" else "false";
      } // lib.optionalAttrs (cfg.relay.enable && cfg.relay.mode == "remote" && cfg.relay.publicUseTls != null) {
        OSUNA_RELAY_PUBLIC_USE_TLS = if cfg.relay.publicUseTls then "true" else "false";
      } // cfg.environment;
