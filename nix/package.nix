...
buildNpmPackage rec {
  ...
  installPhase = ''
    ...
    mkdir -p $out/lib/osuna
    node scripts/trace-daemon.mjs > daemon-files.txt

    while IFS= read -r path; do
      [ -z "$path" ] && continue
      mkdir -p "$out/lib/osuna/$(dirname "$path")"
      cp -a "$path" "$out/lib/osuna/$path"
    done < daemon-files.txt

    # Root package.json lets node resolve the workspace layout when the
    # CLI/server bin starts from $out.
    cp package.json $out/lib/osuna/

    # Web UI Assets
    cp -r packages/server/dist/server/web-ui $out/lib/osuna/packages/server/dist/server/

    # Create wrapper for the server entry point (for systemd / direct use)
    mkdir -p $out/bin
    # Keep Osuna's runtime mode separate from NODE_ENV, which belongs to spawned agents.
    makeWrapper ${nodejs}/bin/node $out/bin/osuna-server \
      --add-flags "$out/lib/osuna/packages/server/dist/scripts/supervisor-entrypoint.js" \
      --set OSUNA_NODE_ENV production

    # Create wrapper for the CLI
    makeWrapper ${nodejs}/bin/node $out/bin/osuna \
      --add-flags "$out/lib/osuna/packages/cli/dist/index.js" \
      --set NODE_PATH "$out/lib/osuna/node_modules"
