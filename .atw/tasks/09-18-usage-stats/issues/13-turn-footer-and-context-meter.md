...
- [ ] 接缝 2：对齐函数用例——`turnId` 命中、`turnId` 为 null 但 `messageId` 命中、都不命中；秒表叠加函数；`formatTokenCount` 新格式（`14.3K`、`1.2M`、`999`）。
- [ ] `*.real.spec.ts`：`CLAUDE_CONFIG_DIR` 指向临时目录，跑一轮真实 Claude，轮结束后 footer 出现 `↑ ↓ $` 段，弹层含模型名与耗时；环形表旁出现 `已用 / 上限`，弹层本会话合计的 Token 与 footer 该轮一致。
- [ ] 接缝 3（非 real）：旧 daemon 夹具下 footer 只显示 "Worked for"，弹层本段显示「需要更新主机」。
- [ ] 手机宽度下 footer 用量段换到第二行、环形表旁无文字（截图证据）。
- [ ] 资源测试通过；`npm run typecheck`、`npm run lint`，改动的测试与 spec 通过。
