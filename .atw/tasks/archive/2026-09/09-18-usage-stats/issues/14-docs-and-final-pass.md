...
- 每日细目行高从 `spacing[2]` 提到 `spacing[3]`（行高 33.5 → 41.5）。
- 明细卡内部纵向滚动条紧贴最后一列。卡内滚动容器加 12px 右内边距，
  最后一列与滚动条之间留出 23px（12 + 8px 滚动条槽）。
- 设置页价格表七列显示不全。列宽原来是 220 + 4×92 + 80 + 150，加间距和内边距共 898，
  而设置栏的阅读宽度上限是 720（`settings-screen.tsx` 的 `content.maxWidth`），卡片只有
  688 —— 「来源」被截、「操作」整列在卡外，而 `showsHorizontalScrollIndicator={false}`
  又把横向滚动条藏了，看上去就是内容丢了。列宽改到 154 + 4×70 + 64 + 120（共 678），
  滚动条不再隐藏；「无价格数据」徽章从模型名旁边挪到它下面一行，长模型 id 不再被徽章挤没。
  列几何抽到 `src/price-table/price-columns.ts`，表头和数据行共用一份。
  弹性列试过，在横向滚动容器里行宽由内容决定，`flex-shrink` 根本不会触发，已记在该文件注释里。
