...
- **`session.ts`'s dispatch chain and `bootstrap.ts`'s `createPaseoDaemon` both sit at the `eslint(complexity)` ceiling of 20.** One more `??` link in the chain, or one more `?.`/`??` inside `createPaseoDaemon`, fails lint. Put a new RPC in an existing domain dispatcher, and push option defaulting into the service's own constructor or a module-level factory rather than into the bootstrap expression.
