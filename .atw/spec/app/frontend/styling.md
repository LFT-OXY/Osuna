...
**Signature.** `LightThemeConfig` / `DarkThemeConfig` extend `ThemeRoleOverrides` (`styles/theme.ts`): optional `surfaceWorkspace`, `surfaceChrome`, `surfaceCard`, `surfaceMessage`, `surfaceSidebarHover`, `surfaceSidebarActive`, `surfaceSidebarSelected`, `borderSidebarSelected`, `borderInput`, `shadowComposer`, `insetHighlight`. Both semantic builders spread `deriveThemeRoles(base, overrides)` (module-private), which returns those eleven plus `surfaceTabHover` / `surfaceTabActive` (always derived, below), `diffAdditionBackground`, `diffDeletionBackground`, `diffAdditionBar`, `diffDeletionBar` (always derived from the status colors, matching `git/diff-document/palette.ts`) and the overlay roles from `deriveOverlayRoles` (always derived, never overridden):
