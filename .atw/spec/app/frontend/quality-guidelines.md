...
- **A renamed identifier carries its value.** `const DIRECT_OSUNA_TOOL_PREFIX = "osuna_"` typechecks, lints, and lies — the constant was renamed and the string it holds was not, so the classifier it feeds matched nothing real while reading as done. When a rename sweep touches a constant, a translation key, or a test fixture, check the right-hand side too, and check the i18n copy behind the key: a field renamed to `osunaCallCount` whose locale string still says "called Osuna" is the same half-change one layer out.
- **"Is this ours?" is answered in one place.** Tool-name classification lives in `packages/protocol/src/tool-name-normalization.ts`, but bare names with no `__` or `.` fall through it and are decided by `DIRECT_OSUNA_TOOL_PREFIX` in `tool-calls/detail-level/overview/model.ts`. Two holders of one policy means a rename has to move both; before adding a third, fold it into the protocol module.
