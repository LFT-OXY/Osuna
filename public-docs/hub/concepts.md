---
title: How Hub works
description: How a provider event reaches a workflow and an Osuna daemon.
nav: How it works
order: 62
category: Hub
---
...
- A **daemon** is a registered machine running the Osuna daemon.
...
The configuration lives in `.osuna/hub.yml` plus convention-discovered `.osuna/workflows/*.yml` files when the project uses a GitHub source. A project has one active configuration revision at a time.
...
- Prompt partials must resolve below `.osuna/workflows/partials/` at the exact commit.
