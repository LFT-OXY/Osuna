...
organization. Set the Hub origin in `OSUNA_HUB_URL` below, for example
`https://hub.example.com`.

## API reference

- Interactive API reference and the OpenAPI 3.1 document are served by the Hub deployment itself, at `<hub-origin>/api/reference` and `<hub-origin>/api/openapi.json`.

These are the canonical reference endpoints for the hosted Osuna Hub. A self-hosted Hub exposes the same `/api/reference` and `/api/openapi.json` paths on its own origin.

## Authentication

Run `osuna hub login [origin]` for interactive CLI access. After browser approval, Osuna stores a durable, revocable organization credential under `OSUNA_HOME` for that exact origin. Without an explicit origin, the CLI uses `OSUNA_HUB_URL`, then the active stored login; with neither configured it fails and says what to set.
...
```http
Authorization: Bearer osuna_pk_...
...
CLI credentials have the current CLI operation scopes and remain revocable independently of daemon relationships. `osuna hub logout` deletes the active local CLI credential; it does not revoke or disconnect the daemon identity.
...
```json
{
  "type": "<hub-origin>/problems/unauthorized",
  "title": "Authentication required",
  "status": 401,
  "detail": "Provide an active Osuna organization credential in the Authorization: Bearer header.",
  "code": "unauthorized",
...
`osuna hub deploy --dry-run` validates each `.osuna/triggers/*.yml` file through `POST /api/v1/triggers/validate`. `osuna hub deploy` validates all files first, then installs each through `POST /api/v1/triggers/install`.
...
`GET /api/v1/projects` returns active projects in the bearer credential's organization. `osuna hub projects` renders the projects as a table. With `--json`, it returns `{ "origin": "...", "projects": [...] }` so even an empty result records the resolved Hub.
...
`osuna hub deploy --project <slug> --dry-run` calls this endpoint with the identical locally resolved payload that a deployment would send.
...
```json
{
  ...
  "files": [
    ...
    {
      "path": ".osuna/workflows/deploy.yml",
      "content": "name: deploy\non: manual.run\nmax_runtime: 2h\nfilters:\n  from_users: [automation]\nsteps:\n  - id: deploy\n    environment: production\n    max_runtime: 90m\n    idle_timeout: 10m\n    agent: codex\n    prompt:\n      - include: partials/safety.md\n"
    },
    {
      "path": ".osuna/workflows/partials/safety.md",
...
`projectSlug` picks the target project; the bearer credential fixes the organization. `files` contains `.osuna/hub.yml`, every direct workflow `.yml`, and each referenced workflow partial. Hub rejects missing, extra, duplicate, unsafe, or noncanonical paths.
...
```bash
curl --fail-with-body -sS -X POST "$OSUNA_HUB_URL/api/v1/configurations/install" \
  -H "Authorization: Bearer $OSUNA_HUB_API_KEY" \
  -H "Content-Type: application/json" \
  --data @configuration-install.json
```

`osuna hub deploy -p <project>` selects this legacy endpoint with the discovered local bundle. The command uses an exact-origin stored login when flags and environment credentials are absent. See [Deploy from the CLI](/docs/hub/configuration#deploy-from-the-cli).
...
```bash
curl --fail-with-body -sS -X POST "$OSUNA_HUB_URL/api/v1/manual-runs" \
  -H "Authorization: Bearer $OSUNA_HUB_API_KEY" \
  -H "Content-Type: application/json" \
  --data '{
    "projectSlug": "my-project",
...
`osuna hub connect [origin]` performs this request with `--api-key`, `OSUNA_HUB_API_KEY`, or the matching stored login, then passes the one-time token to the daemon's enrollment operation. The daemon generates and keeps its own relationship credential.

```bash
curl --fail-with-body -sS -X POST \
  "$OSUNA_HUB_URL/api/v1/daemons/enrollment-tokens" \
  -H "Authorization: Bearer $OSUNA_HUB_API_KEY"
```

Direct API consumers can pass the returned token to the daemon enrollment protocol. The Osuna CLI intentionally does not accept raw enrollment tokens; `connect` owns the authenticated single-flow exchange.
