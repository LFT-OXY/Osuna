...
A daemon runs agents on one machine, for you. Osuna Hub is the layer above your daemons. You register your daemons with it, and it gives them capabilities they do not have on their own.
...
`osuna hub init` creates one self-contained starter trigger:
...
[Hosted Hub](/docs/hub/hosted) uses the same triggers, daemons, and activity model. This fork operates no Hub of its own — see that page for which origin to point at.
