---
title: Hub FAQ
description: Common questions about projects, connections, configuration, and daemons in Osuna Hub.
nav: FAQ
order: 78
category: Hub
---
...
## Do I need Hub to use Osuna?

No. Osuna runs agents on your machines without it. Hub adds what a single daemon cannot do on its own: starting agents from external activity, versioned configuration, a shared record of what ran, and team access.
...
Yes. `filters.repo` can name any repository the organization can reach, so a private repository can hold the `.osuna` bundle for workflows that watch public repositories. Protect push access because the bundle selects the organization's connections, daemons, agents, and outputs.

## Where do triggers go now?

Each trigger and its ordered steps live in one direct `.osuna/workflows/*.yml` file. `hub.yml` contains only named environments and agents. Hub rejects monolithic `triggers` configuration instead of translating it.
