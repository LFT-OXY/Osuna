...
Run Hub on your machine, connect it to Slack without a public server, and answer a mention with an agent in your repository. Hub's browser setup hands off to your terminal, and `osuna hub init` writes and deploys a starter trigger for you.

You need [Osuna installed and running](/docs), Node.js, and a Slack workspace where you can create an app.
...
```sh
osuna hub login http://localhost:3000
```
...
After approving login, answer **Yes** to **Connect this daemon to Osuna Hub?** and **Allow Hub automations to run agents on this daemon?**. Execution permission defaults to no, so enable it explicitly for this setup.
...
If `slack-help.yml` already exists, setup asks before replacing that file. Other files remain in place. If you decline deployment, run `osuna hub deploy` from this repository when ready.
...
Hub keeps its local state in your user data directory, normally `~/.local/share/osuna-hub`. [Self-hosting](/docs/hub/self-hosting) covers deployment and advanced configuration when you outgrow the local run.
