...
By default, Osuna uses `~/.osuna` as its home directory. The configuration file is:

```bash
~/.osuna/config.json
```

You can change the home directory by setting `OSUNA_HOME` or selecting `--home` on a CLI command.
...
```bash
osuna daemon config get daemon.listen --home ~/osuna-test
osuna daemon config set daemon.listen 127.0.0.1:6800 --home ~/osuna-test
osuna daemon config unset features.webUi.enabled --home ~/osuna-test
```
...
Agent providers, both the first-class ones Osuna ships with and custom entries you add under `agents.providers`, are documented on their own page.

See [Providers](/docs/providers) for the mental model and [Supported providers](/docs/supported-providers) for the full list of agents Osuna can launch. For pointing Claude at Anthropic-compatible endpoints (Z.AI, Alibaba/Qwen), multiple profiles, custom binaries, ACP agents, and the `additionalModels` merge behavior, see [Custom providers](/docs/custom-providers). The full field reference lives on GitHub at [docs/custom-providers.md](https://github.com/LFT-OXY/Osuna/blob/main/docs/custom-providers.md).

## Worktrees

New worktrees are created under `$OSUNA_HOME/worktrees` by default. To place new worktrees somewhere else, set `worktrees.root`:

```json
{
  "worktrees": {
    "root": "/mnt/fast/osuna-worktrees"
  }
}
```

Relative paths are resolved against `OSUNA_HOME`. Existing worktrees remain where they are; changing this setting only changes where Osuna creates and discovers Osuna-managed worktrees going forward.
...
- File (`$OSUNA_HOME/daemon.log`): `trace` and above
...
1. The `password` query parameter on a `tcp://` host URI:
   ...
   ```bash
   osuna --host "tcp://192.168.1.10:6777?password=my-secret" ls
   ```

2. The `OSUNA_PASSWORD` environment variable, used as a fallback when the host carries no embedded password (works for `localhost:6777`, bare `host:port`, or `tcp://` hosts without a `password=` query):

   ```bash
   OSUNA_PASSWORD=my-secret osuna ls
   OSUNA_PASSWORD=my-secret osuna --host 192.168.1.10:6777 ls
   ```

A `password=` in the URI always wins over the env var, so you can keep `OSUNA_PASSWORD` set globally and still target a different daemon by spelling its password into the URI.

In the mobile app, enter the password in the direct connection setup screen.

## Relay

New homes write `daemon.relay.enabled: false`. Osuna asks before enabling relay when you pair a device; existing homes keep their saved value. See [Connectivity](/docs/connectivity) to choose and configure a connection method, and [Security](/docs/security) for the relay encryption model.
...
`OSUNA_RELAY_ENABLED=true|false` overrides the file for a foreground deployment. Managed `start` uses the file. End and relaunch a deployment to remove its override before changing relay from the app or `osuna daemon pair --relay`.

## Common env vars

- `OSUNA_HOME`, set Osuna home directory
- `OSUNA_HOST`, set the daemon target for CLI commands
- `OSUNA_PASSWORD`, on the daemon, the password to require (plaintext, hashed at startup); on the CLI, the password used to connect when the host URI doesn't include one
- `OSUNA_LISTEN`, override `daemon.listen`
- `OSUNA_RELAY_ENABLED`, enable or disable the outbound relay for this daemon launch
- `OSUNA_HOSTNAMES`, override/extend `daemon.hostnames`
- `OSUNA_ALLOWED_HOSTS`, deprecated alias for `OSUNA_HOSTNAMES`
- `OSUNA_WEB_UI_ENABLED`, enable or disable the daemon-served web UI
- `OSUNA_WEB_UI_DIST_DIR`, override the daemon web UI build directory
- `OSUNA_TRUSTED_PROXIES`, configure trusted reverse proxy ranges for `X-Forwarded-*` headers
- `OSUNA_LOG_CONSOLE_LEVEL`, override `log.console.level`
- `OSUNA_LOG_FILE_LEVEL`, override `log.file.level`
- `OSUNA_LOG_FILE_PATH`, override `log.file.path`
- `OSUNA_LOG_FILE_ROTATE_SIZE`, override `log.file.rotate.maxSize`
- `OSUNA_LOG_FILE_ROTATE_COUNT`, override `log.file.rotate.maxFiles`
- `OSUNA_LOG`, `OSUNA_LOG_FORMAT`, legacy log overrides (still supported)
...
- `OSUNA_VOICE_LLM_PROVIDER`, override voice LLM provider (`claude`, `codex`, `opencode`)
- `OSUNA_DICTATION_STT_PROVIDER`, `OSUNA_VOICE_STT_PROVIDER`, `OSUNA_VOICE_TTS_PROVIDER`, override voice provider selection (`local` or `openai`)
- `OSUNA_LOCAL_MODELS_DIR`, control local model directory
- `OSUNA_DICTATION_LOCAL_STT_MODEL`, override local dictation STT model
- `OSUNA_VOICE_LOCAL_STT_MODEL`, `OSUNA_VOICE_LOCAL_TTS_MODEL`, override local voice STT/TTS models
- `OSUNA_DICTATION_LANGUAGE`, `OSUNA_VOICE_LANGUAGE`, override dictation and voice STT language
- `OSUNA_VOICE_LOCAL_TTS_SPEAKER_ID`, `OSUNA_VOICE_LOCAL_TTS_SPEED`, optional local voice TTS tuning
