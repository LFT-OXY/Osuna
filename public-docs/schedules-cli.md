...
description: Create and manage Osuna schedules with osuna schedule.
nav: CLI
order: 27
category: Schedules
---
...
`osuna schedule` creates and manages new-agent [schedules](/docs/schedules) from your terminal, useful for headless boxes and scripts. Every run starts a fresh agent.
...
Inside a running Osuna agent, create a heartbeat for that same conversation:

```bash
osuna heartbeat create \
  --cron "*/20 * * * *" \
  --name heartbeat \
...
```bash
osuna heartbeat update <id> --cron "*/10 * * * *"
osuna heartbeat delete <id>
```

Updating a heartbeat changes only its cron cadence and optional time zone. Its target and prompt stay fixed. Heartbeat commands require `OSUNA_AGENT_ID`, which Osuna sets inside agent sessions.
...
```bash
osuna schedule ls
osuna schedule inspect <id>
osuna schedule logs <id>
osuna schedule pause <id>
osuna schedule resume <id>
osuna schedule run-once <id>
osuna schedule update <id> --every 10m --max-runs 6
osuna schedule delete <id>
```
...
Start the command with `osuna --host <target> schedule create ...` when targeting a remote daemon. Pass `--cwd`; your local working directory may not exist on the remote machine.
