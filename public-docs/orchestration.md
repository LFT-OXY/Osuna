...
Osuna lets your coding agents coordinate other agents, split work across providers and machines, and keep tasks moving automatically.
...
- **Work across machines:** use the [CLI](/docs/cli#connecting-to-a-remote-daemon) to launch and manage agents on another reachable Osuna host.
...
Use built-in Osuna tools or the CLI. Both let an agent launch and coordinate workers.

### Built-in Osuna tools (MCP)

Enable Osuna tools so agents running inside Osuna can manage agents and workspaces on their host directly.

1. Open **Settings → your host → Agents**.
2. Turn on **Enable Osuna tools**. Tool injection is off by default.
3. Start a new agent, or reload an existing agent so it receives the tools.
4. Ask:

> Use Osuna to launch a second agent to review this branch. Have it report potential bugs without changing files, then summarize its findings.
...
Agents with shell access can also use the Osuna CLI. This route does not require enabling tool injection. With Osuna installed, a running host, and Codex configured:

```bash
osuna run --provider codex --background \
