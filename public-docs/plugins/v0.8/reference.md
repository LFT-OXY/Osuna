...
> **For Osuna v0.8 beta.** Return to the [v0.8 quickstart](/docs/plugins/v0.8).

Migrating an existing plugin? Follow the standalone [runtime-entry migration guide](/docs/plugins/v0.8/migration).

Local plugins are directory sources installed into one Osuna daemon. A plugin can contribute:

- React Native surfaces and sidebar items to Osuna clients;
...
- normal Osuna operations through the TypeScript SDK;
- searchable external resources in the message composer.

Plugin code is trusted and unsandboxed. Client surfaces run in the Osuna app. Backend contributions run in a subprocess with access to the daemon machine, including its files, processes, credentials, and network.

## Project files

`osuna plugin init /absolute/path/to/my-plugin` creates:
...
The required root manifest is `osuna-plugin.json`. It contains the default plugin ID and supported Osuna versions:
...
plugin release. Osuna 0.8 and later reject it with a link to the [migration guide](migration).
...
Prerelease Osuna versions also satisfy a range their stable core (`major.minor.patch`) satisfies, so `0.8.0-beta.1` satisfies `>=0.8.0` but not `<0.8.0`.

`osuna plugin init` writes `>=` followed by the current CLI version and pins the matching SDK
...
For example: `Plugin "review" requires Osuna >=0.8.0. Your daemon is 0.7.2.` Use a compatible plugin
...
| ------------------ | --------------------- | --------------------- | ------------------------------------------------------------------------------- |
| `index.client.tsx` | Osuna app, per client | `PluginClientContext` | When the plugin has any UI, callback, theme, or attachment source               |
...
Osuna builds each bundle from its matching entry. An import from `client/` into the daemon bundle,
...
The SDK root (`@osuna/plugin`) contains shared data, schemas, and runtime-neutral helpers only.
...
Do not import `lucide-react-native`, `react-native-svg`, or DOM libraries. Set contribution `icon` fields to a [Lucide icon name](https://lucide.dev/icons/); Osuna validates the name and renders the icon.
...
Osuna provides `@osuna/plugin`, `@osuna/plugin/server`,
`@osuna/plugin/server/provider`, `@osuna/plugin/server/acp`, and `zod` to server code. Backend
...
`@osuna/plugin/server/provider`. Its connection accepts inputs with `send()` and emits complete state
...
Provider settings are toggle/select descriptors that Osuna renders in the composer. Keep
provider-private JSON under `providerOptions`. Host tools arrive as MCP servers in the complete
session config.

Osuna refreshes an agent by closing its current provider session and opening it with current
configuration and persistence. Providers re-read external state during `session.open`.

Use `runAcpProvider()` from `@osuna/plugin/server/acp` to adapt a command-backed ACP. Add transformer
...
which return `{ update, remove }` handles. The entry cleanup runs before Osuna removes remaining registrations.
...
Cleanup can be async. Release timers, watchers, sockets, and other resources created by the plugin. Osuna also removes registrations, unmounts surfaces, rejects pending RPCs, closes the plugin's daemon session, and stops its subprocess on reload, disable, removal, disconnect, or daemon shutdown.
...
| ------------------------------- | ---------------------------------- | ------------------------------------------------------------ |
| `server.on(name, callback)`     | `(event, { osuna, signal })`       | `void` or `Promise<void>`                                    |
| `server.before(name, callback)` | `({ request }, { osuna, signal })` | Modified request, or `undefined` to keep it; async supported |
...
export default function contribute(server: PluginServerContext) {
  server.on("agent.turn_ended", async (event, context) => {
    ...
    if (/out of credits/i.test(text)) {
      await context.osuna.agents.ref(event.agent.id).send("Try again.");
    }
  });
...
Using `shellCommand` from the same [helper file](https://github.com/LFT-OXY/Osuna/blob/main/plugin-examples/lifecycle-actions/server/inspect.ts):
...
**Shared payload shapes** (`@osuna/plugin/server`):
...
Creation request
  ...
  → derive launch configuration with Osuna runtime tools and daemon prompt
  → agent.session_open hooks (same ordering; env only)
  → set OSUNA_AGENT_ID and OSUNA_AGENT_CWD
...
| Contract                           | Behavior                                                                                                  |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `context.osuna`                    | Existing SDK connected to this daemon                                                                     |
...
Read logger output with `osuna plugin logs lifecycle-logger` or the host's `daemon.log`.
...
| Field        | Meaning                                                                                                                      |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| `theme`      | Typed `PluginTheme` color tokens for the active Osuna theme.                                                                 |
...
Osuna owns the route, header, close action, host picker, error boundary, and query client. The plugin owns the surface body.

## Host UI

Import Osuna-owned UI from `@osuna/plugin/client/react-native` in client code. This example
...
Import `ScrollView` and `FlatList` from `@osuna/plugin/client/react-native` when content can appear in a
Osuna modal. They accept React Native props and refs and integrate with the sheet's gestures. Outside
...
```tsx
import { FlatList, Modal } from "@osuna/plugin/client/react-native";
...
```tsx
import { copyText, useToast } from "@osuna/plugin/client/react-native";
...
for long-press selection and OS Copy. Import `TextInput` from `@osuna/plugin/client/react-native` for modal forms. It accepts React Native
...
The runnable [modal UI example](https://github.com/LFT-OXY/Osuna/tree/main/plugin-examples/modal-ui)
...
`Icon` renders a [Lucide icon](https://lucide.dev/icons/) from Osuna's installed icon set. Plugin bundles do not import
...
registrations are client contributions. Osuna applies the transformer while building the render
model, including every live streaming update.

```tsx
import type { PluginClientContext, PluginTimelineItemProps } from "@osuna/plugin/client";
import { Text } from "react-native";
import { z } from "zod";
...
paragraph separators. Osuna invokes transformers before splitting native Markdown or grouping
...
Each replacement may set an optional plugin-local `id`; otherwise Osuna uses its index within that
source item's output.

Renderers receive `agentId`, `item`, `timestamp`, `theme`, `host`, and `layout`. Osuna validates
`item.data` with the registered schema before rendering. Keep transformers synchronous and
deterministic. Osuna memoizes results by source-item reference and derives replacement identity from
the source row, so updates to one streaming item do not remount its renderer. Use the exported
`useRevealedText(text, phase)` hook when a renderer should pace streaming text like Osuna's built-in
assistant rows.
...
same plugin and `id`. If its renderer is missing, Osuna shows the existing unavailable row. Daemons
...
Plugin UI runs on desktop, browser, iOS, and Android, across every Osuna theme. `theme` is a typed `PluginTheme` mapped from the active host theme. Color and spacing must come from those props. Hardcoded colors and unstyled `Text` break when the host theme changes.
...
Every color is a hex string; anything else fails to load. Osuna expands the palette into the full
...
`appearance` is `"light"` or `"dark"`. Osuna uses it to select the matching surface, status,
...
later disabled or removed, Osuna falls back to the default theme rather than leaving the app
unpainted.
...
The component receives `PluginSurfaceProps`. Osuna owns the header, back navigation, safe areas,
...
Import settings components from `@osuna/plugin/client/ui`. They work with your own state and RPCs;
no form wrapper or storage binding is required.

```tsx
import { useState } from "react";
import { SettingsCard, SettingsSection, SettingsSwitch } from "@osuna/plugin/client/ui";
...
| `SettingsSelect`                   | Row props plus required string `value`, `options: { label, value }[]`, `onValueChange`; optional `disabled`. Uses Osuna's adaptive menu. |
...
See the complete [settings example](https://github.com/LFT-OXY/Osuna/tree/main/plugin-examples/settings)
...
```tsx
import { type PluginAgentPanelProps, useAgent, useWorkspace } from "@osuna/plugin/client";
import { useMemo } from "react";
import { Text, View } from "react-native";
...
Read cached state with `useWorkspace(workspaceId, selector)` and `useAgent(agentId, selector)`. A selector is required. Osuna compares its result shallowly, so selecting `{ name, status }` does not re-render when unrelated fields change. Select every field the component renders in one call; do not select the whole snapshot.
...
Osuna owns tab focus, splitting, closing, persistence, query state, the API/RPC providers, and the render error boundary. A restored tab whose plugin, panel, context, workspace, or agent is unavailable stays open with an unavailable message instead of crashing the workspace.
...
client.addCommandCenterItem({
  ...
  keywords: ["inspect"],
  context: "agent",
  async onSelect({ osuna, rpc, workspace, agent, openPanel }) {
    await osuna.workspaces.ref(workspace.id).setTitle(`Review ${agent.id}`);
...
Register a command that runs in the Osuna client when the user submits `/name args` from the
...
`args` is `"src"`; Osuna trims only the remainder's leading and trailing whitespace and leaves
parsing to the plugin. Osuna owns the autocomplete row, input clearing, and the error toast. It
...
Try the [button example](https://github.com/LFT-OXY/Osuna/tree/main/plugin-examples/buttons) for
...
Compact header buttons use icons without labels or chevrons. Osuna moves excess contributions
...
The [local plugin example](https://github.com/LFT-OXY/Osuna/blob/main/plugin-examples/local-plugin/client/main.tsx)
replaces registrations on each snapshot and aborts the observation during entry cleanup, including pending bootstrap.

## Button descriptor

These contracts are exported from `@osuna/plugin/client`.
...
An action runs on the client. Osuna marks the button busy until its promise settles, blocks repeated
...
Osuna removes leading, trailing, and consecutive separators after filtering hidden items.
...
Render a React Native icon or indicator within the supplied size. Osuna bounds the icon slot and
...
Render the body only; Osuna owns anchoring, scrolling, padding, and sheet presentation. Content can
...
`remove()` is idempotent. Updates after removal do nothing. Osuna removes outstanding buttons when
...
```tsx
import { type PluginSurfaceProps, useOsuna } from "@osuna/plugin/client";
...
The returned API covers projects, workspaces, agents, terminals, providers, and daemon config. See the [SDK API reference](/docs/sdk/reference) for its methods. Connection lifecycle methods are intentionally absent because Osuna owns the connection.

## Add plugin-specific backend behavior

Use plugin RPC only for work that is not a normal Osuna operation: reading a vendor API, accessing daemon-local resources, or keeping credentials off the client.
...
Osuna adds `[osuna]` entries when the plugin starts loading, becomes ready, starts stopping, and has
stopped. It records compilation and load failures as stderr entries, including failures that happen
before the plugin subprocess starts. Osuna also captures output emitted during initialization, RPC
...
```bash
osuna plugin logs my-plugin
osuna plugin logs my-plugin --json
osuna --host <url> plugin logs my-plugin
```
...
Osuna retains up to 500 entries and 256 KiB per plugin in memory. Individual lines are capped at
...
also written to the daemon log at `$OSUNA_HOME/daemon.log`.
...
Osuna owns the composer menu, search picker, selected pill, draft state, and submission. The `text` value is the complete snapshot sent to the agent.

## Hosts and lifecycle

Plugins are installed per daemon. When the same contribution exists on several connected hosts, Osuna shows one sidebar item and adds a host picker. The selected host supplies the bundle, Osuna API, RPC transport, and query cache. Calls never fall through to another host when the selected host is offline.
...
```bash
osuna plugin init /absolute/path/to/plugin
...
osuna plugin add https://git.example.com/owner/repository.git --ref main
osuna plugin add owner/monorepo:plugins/review
osuna plugin ls [id]
osuna plugin update <id>
osuna plugin update --all
osuna plugin reload my-plugin
osuna plugin logs my-plugin
osuna plugin disable my-plugin
osuna plugin enable my-plugin
osuna plugin remove my-plugin
```

`ls` reports runtime state, source details, and the installed commit without contacting the remote.
Use `update` when you want Osuna to contact a tracked Git remote and install an available update.
...
> **Trust every plugin you add.** `osuna plugin add` and `osuna plugin install` mean “I trust this codebase.” Server code and Git preparation commands run unsandboxed with the daemon user's access on the daemon host; client contributions run inside Osuna. Dependencies and future updates are part of that decision. With the global `--host` option, commands run on the remote daemon host.
...
`build` is a list of non-empty argv arrays. Osuna runs each executable directly, without a shell,
...
The switch is the root `pluginsEnabled` field in `config.json`. After changing it, run `osuna reload --json`. Enabling starts every configured plugin whose own `enabled` value is not `false`; disabling tears down all plugins. No daemon restart is required. Manual edits to plugin source entries are not reloaded; use the plugin lifecycle commands for those.

## Load failures

Use `osuna plugin ls` to read the current status and error.

| Symptom                                                               | Check                                                                                                                                   |
| --------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `This plugin was made for an older version of Osuna`                  | The directory has only an `index.ts` entry. Follow the [migration guide](/docs/plugins/v0.8/migration).                                 |
