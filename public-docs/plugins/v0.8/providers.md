...
description: Add a coding agent to Osuna directly or adapt an ACP agent.
...
- [`provider-direct`](https://github.com/LFT-OXY/Osuna/tree/main/plugin-examples/provider-direct): sessions, settings, prompts, persistence, child sessions, and a provider-owned timeline renderer;
- [`provider-acp-transformer`](https://github.com/LFT-OXY/Osuna/tree/main/plugin-examples/provider-acp-transformer): an ACP command template with a Zod-validated vendor edit transformer. Replace `example-acp --stdio` with an installed ACP agent before loading it;
- [`inline-thinking`](https://github.com/LFT-OXY/Osuna/tree/main/plugin-examples/inline-thinking): a renderer-only plugin that does not implement a provider.
...
Osuna requests the catalog before creating a session. Return the choices needed by the agent form:
...
the user timeline item so Osuna replaces its optimistic message. Every started turn needs one
...
Osuna renders these controls in the composer. A user change arrives as `session.configure`; apply it
...
for provider-specific configuration that Osuna does not render. A provider-specific token budget
...
Return an opaque persistence value in `session.opened` or `session.persistence`; Osuna stores it
...
Osuna refreshes an agent by closing its current provider session and opening it again with current
...
Use `restoration: "core"` when Osuna can reopen a session from persistence. A provider-owned child
...
Advertise only capabilities you implement. If steering is unsupported, omit `prompt.steer`; Osuna
...
a running tool, or a todo list. Osuna derives the live delta and keeps the normal timeline.
...
```ts
import type { AcpTransformer } from "@osuna/plugin/server/acp";
import { z } from "zod";
...
osuna plugin update my-provider-plugin
```

Keep vendor compatibility and releases in that repository. Osuna core should only change when the
