---
title: MCP reference
description: Reference for the Osuna tools agents use to manage agents, workspaces, scripts, terminals, and schedules.
nav: MCP reference
order: 34
...
[Enable Osuna tools](/docs/orchestration#get-started) to give agents this catalog. Ask for an outcome in natural language, or use the tool interfaces below.
...
| `daemon.mcp.injectIntoAgents` | `false` | Give agents launched by Osuna access to its tools. |

Depending on the provider, Osuna delivers tools through its native tool interface or MCP. The capabilities are the same. Start a new agent or reload an existing one after changing injection settings.

## Limit Osuna tools by provider

Use provider policies when different agent profiles should receive different Osuna tools. Enable
tool injection globally, then add `osunaTools` to the exact provider IDs you launch:
...
Run `osuna reload` after editing `~/.osuna/config.json`, then start a new agent or reload an
existing one. A running session keeps the catalog it received at launch.

Omitting `osunaTools` enables the complete catalog. Set `enabled` to `false` to remove the catalog,
...
These tools manage scripts configured in a workspace's `osuna.json`. Each requires an explicit `workspaceId`; start and stop also require the configured `scriptName`.

| Tool                     | Function                                                                                |
| ------------------------ | --------------------------------------------------------------------------------------- |
| `list_workspace_scripts` | List configured scripts with lifecycle, terminal, port, proxy URL, and health metadata. |
| `start_workspace_script` | Start a configured script through Osuna's managed launcher.                             |
| `stop_workspace_script`  | Stop a running script through its supervised terminal.                                  |

See [Git worktrees](/docs/worktrees#scripts-and-services) for `osuna.json` configuration.
