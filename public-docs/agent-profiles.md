...
When creating an agent, open the model picker and select a saved profile under **Profiles**. Osuna applies its settings together; you can still adjust them before sending your prompt.
...
The **When to use** field is the profile's notes. Osuna exposes these notes through its tools so an orchestrating agent can read them before choosing how to launch a worker.
...
After [enabling Osuna tools](/docs/orchestration#get-started), ask:

> Check my Osuna profiles and their notes. Choose one for the settings-page UI change, then use a review profile for an independent review of the diff.
...
The bundled [Osuna skill](/docs/skills#osuna-osuna-reference) instructs agents to check profiles and read their notes before delegating. If none fit, it directs the agent to discover configured providers and models and tell you about the fallback.
