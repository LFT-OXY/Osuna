...
export async function runDaemonReloadCommand(
  ...
  try {
    ...
    return {
      type: "single",
      data: {
        restartCommand: `osuna daemon restart ${options.daemonTarget.kind === "instance" ? `--home ${JSON.stringify(options.daemonTarget.home)}` : `--host ${JSON.stringify(describeDaemonTarget(options.daemonTarget))}`}`,
