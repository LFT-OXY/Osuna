...
describe("Hub deployment bundle discovery", () => {
  it("discovers the canonical bundle in deterministic path order", async () => {
    ...
    await writeFile(
      path.join(cwd, ".osuna", "workflows", "z-last.yml"),
    ...
    await expect(discoverHubBundle({ cwd, project: "studio-api" })).resolves.toEqual({
      ...
      workflowCount: 2,
      files: [
        { path: ".osuna/hub.yml", content: hubResource },
        {
          path: ".osuna/workflows/answer.yml",
        ...
        { path: ".osuna/workflows/z-last.yml", content: workflow("z-last", "codex-safe") },
      ],
    });
  });
  ...
  it.each([
    {
      ...
      arrange: async (cwd: string) => {
        await mkdir(path.join(cwd, ".osuna", "workflows"), { recursive: true });
      },
      code: "HUB_RESOURCE_MISSING",
      message: ".osuna/hub.yml does not exist",
    },
    {
      name: "missing workflow directory",
      arrange: async (cwd: string) => {
        await mkdir(path.join(cwd, ".osuna"), { recursive: true });
        await writeFile(path.join(cwd, ".osuna", "hub.yml"), hubResource);
      },
      code: "HUB_WORKFLOW_DIRECTORY_MISSING",
      message: ".osuna/workflows does not exist",
    },
    {
      ...
      message: ".osuna/workflows must contain at least one direct-child .yml workflow",
    },
    {
      ...
      arrange: async (cwd: string) => {
        await mkdir(path.join(cwd, ".osuna", "workflows"), { recursive: true });
        await writeFile(path.join(cwd, ".osuna", "hub.yml"), hubResource);
        await writeFile(path.join(cwd, ".osuna", "workflows", "answer.yaml"), "name: answer\n");
      },
      code: "HUB_WORKFLOW_EXTENSION_UNSUPPORTED",
      message: ".osuna/workflows/answer.yaml must use the .yml extension",
    },
  ...
  it("rejects prompt partial traversal with a path-specific diagnostic", async () => {
    const cwd = await canonicalProject();
    await writeFile(
      path.join(cwd, ".osuna", "workflows", "answer.yml"),
    ...
    await expect(discoverHubBundle({ cwd, project: "studio-api" })).rejects.toMatchObject({
      ...
      message: expect.stringContaining(".osuna/workflows/answer.yml"),
    });
  });
  ...
  it("reports a referenced partial that is missing from the bundle", async () => {
    ...
    await expect(discoverHubBundle({ cwd, project: "studio-api" })).rejects.toMatchObject({
      ...
      message: expect.stringContaining(".osuna/workflows/partials/missing.md"),
    });
  });

  it("rejects nested workflow files instead of discovering a second layout", async () => {
    const cwd = await canonicalProject();
    await mkdir(path.join(cwd, ".osuna", "workflows", "nested"));
    await writeFile(
      path.join(cwd, ".osuna", "workflows", "nested", "other.yml"),
      workflow("other", "codex-safe"),
    );

    await expect(discoverHubBundle({ cwd, project: "studio-api" })).rejects.toMatchObject({
      code: "HUB_WORKFLOW_PATH_UNSUPPORTED",
      message: expect.stringContaining(".osuna/workflows/nested"),
    });
  });
  ...
  it("rejects symlinked workflow and partial paths without reading their targets", async () => {
    ...
    await symlink(outside, path.join(cwd, ".osuna", "workflows", "linked.yml"));

    await expect(discoverHubBundle({ cwd, project: "studio-api" })).rejects.toMatchObject({
      code: "HUB_BUNDLE_UNSAFE_PATH",
      message: expect.stringContaining(".osuna/workflows/linked.yml"),
    });
  });
