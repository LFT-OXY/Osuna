...
describe("Hub CLI credentials", () => {
  it("stores multiple normalized origins privately and selects the latest login", () => {
    ...
    const store = new PrivateHubCredentialStore({ OSUNA_HOME: home });
  ...
  it("rejects malformed storage without exposing persisted secrets", () => {
    ...
    assert.throws(
      () => new PrivateHubCredentialStore({ OSUNA_HOME: home }).active(),
      (error) => {
  ...
  it("resolves origin from explicit, environment, then active login", () => {
    ...
    assert.equal(
      resolveHubOrigin({
        ...
        env: { OSUNA_HUB_URL: "https://env.example.com" },
        credentials: store,
      }),
  ...
  it("resolves credential from explicit, environment, then exact-origin stored login", () => {
    ...
    assert.equal(
      resolveHubCredential({
        ...
        env: { OSUNA_HUB_API_KEY: "env-secret" },
        credentials: store,
      }),
      "explicit-secret",
    );
