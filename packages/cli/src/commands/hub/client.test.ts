  ...
  it("renders file-aware Hub validation issues without exposing credentials or response bodies", async () => {
    ...
    const origin = await startServer(
      () => ({
        ...
        body: {
          ...
          issues: [
            {
              path: [".osuna/workflows/answer.yml", "steps", "work", "agent"],
    ...
    await assert.rejects(
      hub.validateConfiguration({
        ...
        projectSlug: "studio",
        files: [{ path: ".osuna/hub.yml", content: "sensitive bundle content" }],
      }),
      (error: unknown) => {
        ...
        assert.equal(
          error.details,
          ".osuna/workflows/answer.yml: steps.work.agent: unknown named agent [redacted]",
        );
        return true;
      },
    );
