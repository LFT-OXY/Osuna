...
export async function runHubGuidedSetup(
  ...
  if (deploy) {
    ...
    reportMessage(environment, "Skipped deployment. Run `osuna hub deploy` when ready.");
  }
  ...
  if (connection.kind === "connected") {
    if (permissions.includes("hub.execute") && !status.permissions.includes("hub.execute")) {
      throw new HubCommandError(
        ...
        "This daemon is connected to Hub but cannot run Hub automations. Run `osuna hub permissions grant hub.execute`, then run Hub init again.",
      );
    }
      ...
      while (true) {
        ...
        if (Date.now() >= deadline) {
          throw new HubCommandError(
            ...
            "The daemon did not connect within 60 seconds. Check `osuna hub status`, then run Hub init again.",
          );
        }
...
async function resolveStarterTriggerConnections(
  ...
  if (connections.length === 0) {
    throw new HubInitCancelledError(
      "No Hub app connection is ready for this trigger.\nConnect GitHub, Slack, or Discord in Hub → Apps, then run `osuna hub init` again.",
    );
  }
  return connections;
}
...
function reportStarterTriggerConnections(
  ...
  const details = `${connections.map(({ label }) => label).join("\n")}\n\nOnly configured connections are shown. To add another, open Hub → Apps, then run \`osuna hub init\` again.`;
...
async function chooseStarterTriggerConnection(
  ...
  if (connection === undefined) {
    throw new HubCommandError(
      ...
      "The selected Hub app connection is no longer available. Run osuna hub init again.",
    );
  }
  return connection;
}
...
async function chooseStarterAgentRuntime(
  ...
  if (runtime?.mode === undefined) {
    throw new HubCommandError(
      ...
      "The selected starter agent runtime is no longer available. Run osuna hub init again.",
    );
  }
      ...
      while (true) {
        ...
        if (state.kind === "ready") {
          ...
          throw new HubCommandError(
            ...
            "No agent runtime with an execution mode is available from this daemon. Configure one, then run osuna hub init again.",
          );
        }
        if (state.kind === "unavailable") {
          throw new HubCommandError(
            "HUB_AGENT_RUNTIME_REQUIRED",
            "No usable agent runtime is available from this daemon. Configure an enabled provider with a selectable model, then run osuna hub init again.",
          );
        }
        if (Date.now() >= deadline) {
          throw new HubCommandError(
            "HUB_AGENT_RUNTIME_TIMEOUT",
            "Agent runtime discovery did not finish within 60 seconds. Check the daemon's provider configuration, then run osuna hub init again.",
          );
        }
...
function requireInteractiveTerminal(
  ...
  if (!(environment.isInteractive?.() ?? (process.stdin.isTTY && process.stdout.isTTY))) {
    throw new HubCommandError("HUB_INIT_INTERACTIVE_REQUIRED", "osuna hub init requires a TTY.");
  }
}
