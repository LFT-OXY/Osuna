...
import { resolveHookActivity, type AgentHookActivityState } from "@osuna/server/agent-hooks";

interface HookEnvironment {
  OSUNA_TERMINAL_ID?: string;
  OSUNA_ACTIVITY_TOKEN?: string;
  OSUNA_TERMINAL_ACTIVITY_URL?: string;
}
...
function resolveTarget(env: HookEnvironment) {
  const terminalId = env.OSUNA_TERMINAL_ID;
  const token = env.OSUNA_ACTIVITY_TOKEN;
  const url = env.OSUNA_TERMINAL_ACTIVITY_URL;
