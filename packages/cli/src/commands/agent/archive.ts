...
export async function runArchiveCommand(
  ...
  if (!agentIdArg || agentIdArg.trim().length === 0) {
    const error: CommandError = {
      ...
      details: "Usage: osuna agent archive <id-or-name>",
    };
    throw error;
  }
  ...
  try {
    ...
    if (agent.status === "running" && !options.force) {
      const error: CommandError = {
        ...
        details:
          "Use --force to archive a running agent (it will interrupt the active run), or stop it first with: osuna agent stop. Use osuna agent delete to hard-delete it.",
      };
      throw error;
    }
