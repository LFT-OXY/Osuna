...
function launchDesktop(args: string[]): void {
  if (process.env.OSUNA_DESKTOP_CLI === "1") {
    throw new Error("Cannot open Osuna Desktop while running in desktop CLI passthrough mode.");
  }

  const desktopApp = findDesktopApp();
  if (!desktopApp) {
    throw new Error(
      "Osuna desktop app not found. Install it from https://github.com/getpaseo/paseo/releases",
    );
  }
