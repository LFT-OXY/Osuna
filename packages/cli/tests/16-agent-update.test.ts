...
try {
  // Test 1: agent update --help shows options
  {
    ...
    const result = await $`npx osuna agent update --help`.nothrow();
  ...
  // Test 2: agent update requires ID argument
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx osuna --host localhost:${port} agent update --name "New Name"`.nothrow();
  ...
  // Test 3: agent update requires at least one update field
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx osuna --host localhost:${port} agent update abc123`.nothrow();
  ...
  // Test 4: agent update handles daemon not running
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx osuna --host localhost:${port} agent update abc123 --name "Renamed Agent"`.nothrow();
  ...
  // Test 5: agent update accepts multiple/comma-separated labels
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx osuna --host localhost:${port} agent update abc123 --label surface=workspace,area=frontend --label priority=high`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: agent --help shows update subcommand
  {
    ...
    const result = await $`npx osuna agent --help`.nothrow();
  ...
  // Test 7: agent update accepts thinking as an update field
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx osuna --host localhost:${port} agent update abc123 --thinking high`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: thinking cannot be combined with metadata updates
  {
    ...
    const result =
      await $`PASEO_HOME=${paseoHome} npx osuna --host localhost:${port} agent update abc123 --name renamed --thinking high`.nothrow();
