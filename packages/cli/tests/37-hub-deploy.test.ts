...
import { runLocalOsuna } from "./helpers/local-cli.js";

const cwd = await mkdtemp(path.join(tmpdir(), "osuna-hub-installed-"));
...
try {
  ...
  const validate = await runLocalOsuna(
    [
      "hub",
      "deploy",
      "-p",
  ...
  const install = await runLocalOsuna(
