...
try {
  ...
  const renamed = await ctx.osuna(["workspace", "rename", workspaceId, "Auth rework", "--json"]);
  ...
  const renamedList = await ctx.osuna(["workspace", "ls", "--json"]);
