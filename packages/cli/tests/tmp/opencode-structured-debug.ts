...
async function main() {
  ...
  try {
    ...
    const ls = await ctx.osuna(["ls", "--json"]);
    console.log("LS\n" + ls.stdout);
    const agents = JSON.parse(ls.stdout);
    const agentId = agents[0]?.id;
    if (agentId) {
      const inspect = await ctx.osuna(["inspect", agentId]);
      console.log("INSPECT\n" + inspect.stdout);
      const logs = await ctx.osuna(["logs", "--tail", "100", agentId], { timeout: 30000 });
      console.log("LOGS\n" + logs.stdout);
    }
  } finally {
    await ctx.stop();
