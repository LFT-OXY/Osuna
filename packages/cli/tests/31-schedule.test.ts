...
try {
  {
    ...
    const listed = await ctx.osuna(["schedule", "ls", "--json"]);
    ...
    const inspected = await ctx.osuna(["schedule", "inspect", createdJson.id, "--json"]);
  ...
  await ctx.stop();
  await rm(ctx.osunaHome, { recursive: true, force: true });
