...
const promptFilePath = join(osunaHome, "send-prompt.txt");
...
try {
  // Test 1: send --help shows options
  {
    ...
    const result = await $`npx osuna send --help`.nothrow();
  ...
  // Test 2: send requires id argument
  {
    ...
    const result =
      await $`OSUNA_HOME=${osunaHome} npx osuna --host localhost:${port} send`.nothrow();
  ...
  // Test 3: send requires prompt argument
  {
    ...
    const result =
      await $`OSUNA_HOME=${osunaHome} npx osuna --host localhost:${port} send abc123`.nothrow();
  ...
  // Test 4: send handles daemon not running
  {
    ...
    const result =
      await $`OSUNA_HOME=${osunaHome} npx osuna --host localhost:${port} send abc123 "test prompt"`.nothrow();
  ...
  // Test 5: send --no-wait flag is accepted
  {
    ...
    const result =
      await $`OSUNA_HOME=${osunaHome} npx osuna --host localhost:${port} send --no-wait abc123 "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5b: send --prompt flag is accepted
  {
    ...
    const result =
      await $`OSUNA_HOME=${osunaHome} npx osuna --host localhost:${port} send --prompt "test prompt" abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5c: send --prompt-file flag is accepted
  {
    ...
    const result =
      await $`OSUNA_HOME=${osunaHome} npx osuna --host localhost:${port} send --prompt-file ${promptFilePath} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: send --host flag is accepted
  {
    ...
    const result =
      await $`OSUNA_HOME=${osunaHome} npx osuna send --host localhost:${port} abc123 "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: -q (quiet) flag is accepted with send
  {
    ...
    const result =
      await $`OSUNA_HOME=${osunaHome} npx osuna --host localhost:${port} -q send --no-wait abc123 "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: Combined flags work together
  {
    ...
    const result =
      await $`OSUNA_HOME=${osunaHome} npx osuna --host localhost:${port} -q send --no-wait abc123 "Run the linter"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8b: conflicting prompt sources are rejected
  {
    ...
    const result =
      await $`OSUNA_HOME=${osunaHome} npx osuna --host localhost:${port} send abc123 "positional prompt" --prompt "flag prompt"`.nothrow();
  ...
  // Test 9: osuna --help shows send command
  {
    console.log("Test 9: osuna --help shows send command");
    ...
    console.log("✓ osuna --help shows send command\n");
  }
