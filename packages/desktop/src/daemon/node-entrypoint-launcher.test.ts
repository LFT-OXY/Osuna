...
describe("node-entrypoint-launcher", () => {
  describe("createNodeEntrypointInvocation", () => {
    it("uses the packaged runner when the desktop app is packaged", () => {
      expect(
        createNodeEntrypointInvocation({
          execPath: "/Applications/Osuna.app/Contents/MacOS/Osuna",
          isPackaged: true,
          packagedRunnerPath:
            "/Applications/Osuna.app/Contents/Resources/app.asar/dist/daemon/node-entrypoint-runner.js",
        ...
        command: "/Applications/Osuna.app/Contents/MacOS/Osuna",
        args: [
    ...
    it("uses the entrypoint directly in development", () => {
      expect(
        createNodeEntrypointInvocation({
          ...
          argvMode: "node-script",
          args: ["ls"],
          baseEnv: { PATH: "/usr/bin", OSUNA_NODE_ENV: "production" },
        }),
      ).toEqual({
        command: "/opt/homebrew/bin/electron",
        args: ["--import", "tsx", "/tmp/osuna-cli.js", "ls"],
        env: {
          PATH: "/usr/bin",
          ELECTRON_RUN_AS_NODE: "1",
