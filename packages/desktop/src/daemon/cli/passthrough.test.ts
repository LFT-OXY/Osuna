...
const originalDesktopCli = process.env.OSUNA_DESKTOP_CLI;
...
describe("passthrough CLI", () => {
  afterEach(() => {
    ...
    if (originalDesktopCli === undefined) {
      delete process.env.OSUNA_DESKTOP_CLI;
    } else {
      process.env.OSUNA_DESKTOP_CLI = originalDesktopCli;
    }
  });
  ...
  it("returns null when no CLI args are provided", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Osuna.app/Contents/MacOS/Osuna"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("ignores macOS GUI launch arguments", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Osuna.app/Contents/MacOS/Osuna", "-psn_0_12345"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("ignores --no-sandbox injected by Linux wrapper", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/usr/bin/Osuna", "--no-sandbox", "status"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toEqual(["status"]);
    ...
    expect(
      parsePassthroughCliArgs({
        argv: [
          ...
          "/nix/store/osuna-desktop/share/osuna-desktop/electron-app",
          "--no-sandbox",
          "--class=osuna-desktop",
          "daemon",
          "status",
  ...
  it("ignores Electron remote debugging switches", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/usr/bin/Osuna", "--remote-debugging-port=9233"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("passes --open-project through as a normal CLI arg", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Osuna.app/Contents/MacOS/Osuna", "--open-project", "/tmp/project"],
        isDefaultApp: false,
        forceCli: false,
      }),
