...
declare global {
  ...
  var __OSUNA_NODE_ENTRYPOINT_RUNNER_FIXTURE__:
    | {
        argv: string[];
...
describe("node-entrypoint-runner", () => {
  it("preserves Electron node-mode env before loading the target entrypoint", async () => {
    ...
    writeFileSync(
      fixturePath,
      `
globalThis.__OSUNA_NODE_ENTRYPOINT_RUNNER_FIXTURE__ = {
  argv: process.argv,
...
`,
    ...
    try {
      ...
      globalThis.__OSUNA_NODE_ENTRYPOINT_RUNNER_FIXTURE__ = originalFixture;
