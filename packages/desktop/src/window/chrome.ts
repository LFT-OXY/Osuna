...
const WINDOW_CHROME_MODE_ARGUMENT_PREFIX = "--osuna-window-chrome-mode=";
...
export function resolveDesktopWindowChromeMode(input: {
  ...
  if (input.isPackaged) {
    throw new Error("OSUNA_DESKTOP_WINDOW_CONTROLS is only available in development builds");
  }
  if (override === "windows") return "custom-windows";
  if (override === "linux") return "custom-linux";
  throw new Error(
    `Unsupported OSUNA_DESKTOP_WINDOW_CONTROLS value: ${input.override}. Use windows or linux.`,
  );
}
