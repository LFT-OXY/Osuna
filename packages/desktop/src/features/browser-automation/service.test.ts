...
class FakeTab implements TabContents {
  ...
  public async executeJavaScript(code: string): Promise<unknown> {
    ...
    if (code.includes("__OSUNA_ARIA_SNAPSHOT__")) {
    ...
    if (code.includes("__OSUNA_BROWSER_EVALUATE__")) {
...
function snapshotResult(nodes: FakeTab["snapshotNodes"]) {
  ...
  return {
    marker: "__OSUNA_ARIA_SNAPSHOT__",
    root: {
      kind: "role",
      role: "document",
      name: "Fixture",
...
describe("executeAutomationCommand", () => {
  ...
  test("evaluate returns primitive JSON from the page context", async () => {
    ...
    expect(containsScript(browser.tab, "__OSUNA_BROWSER_EVALUATE__", "() => 42")).toBe(true);
  });
  ...
  test("evaluate passes the resolved ref element as the first argument", async () => {
    ...
    expect(containsScript(browser.tab, '"@e1"', "__OSUNA_BROWSER_AUTOMATION__?.resolve")).toBe(
      true,
    );
  });
  ...
  test("evaluate caps oversized results inside the page script", async () => {
    ...
    expect(
      containsScript(browser.tab, "__OSUNA_BROWSER_EVALUATE__", "resultJson.length <= 80000"),
    ).toBe(true);
