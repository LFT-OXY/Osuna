...
export function registerNotificationHandlers(): void {
  ipcMain.handle("osuna:notification:isSupported", () => {
    return Notification.isSupported();
  });

  ipcMain.handle("osuna:notification:send", async (event, rawInput?: NotificationInput) => {
    ...
    notification.on("click", () => {
      ...
      if (win && data && Object.keys(data).length > 0) {
        ...
        win.webContents.send("osuna:event:notification-click", payload);
      }
