...
import { getActiveOsunaBrowserWebContentsForHostWindow } from "./browser-webviews/index.js";
        ...
        { type: "separator" },
        {
          ...
          click: withBrowserWindow((win) => {
            reloadActiveBrowserOrWindow({
              win,
              getActiveBrowserContentsForHostWindow: getActiveOsunaBrowserWebContentsForHostWindow,
            });
          }),
        },
        {
...
export function setupApplicationMenu(options: ApplicationMenuOptions): void {
  ...
  ipcMain.handle("osuna:menu:showContextMenu", (event, input?: ShowContextMenuInput) => {
  ...
  ipcMain.handle("osuna:menu:set-capturing-shortcut", (_event, capturing?: boolean) => {
