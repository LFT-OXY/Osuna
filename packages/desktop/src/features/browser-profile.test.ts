...
describe("listOsunaBrowserProfileGuests", () => {
  test("returns every live webview and popup in the shared profile", () => {
    ...
    const guests = listOsunaBrowserProfileGuests({
      profileSession,
      webContents: [
        firstWindowGuest,
        secondWindowGuest,
...
describe("legacy browser profiles", () => {
  test("accepts only unique saved browser ids and resolves their old partitions", () => {
    ...
    const browserIds = readLegacyOsunaBrowserIds([uuid, fallbackId, uuid, "not-a-browser-id", 123]);
    const partitions: string[] = [];
    const sessions = getOsunaBrowserProfileSessions(
      {
    ...
    expect(partitions).toEqual([
      "persist:osuna-browser",
      `persist:osuna-browser-${uuid}`,
      `persist:osuna-browser-${fallbackId}`,
    ]);
  ...
  test("resolves one valid legacy profile for tab-close cleanup", () => {
    ...
    expect(getLegacyOsunaBrowserProfileSession(sessions, "1700000000000-abcd")).not.toBeNull();
    expect(getLegacyOsunaBrowserProfileSession(sessions, "invalid")).toBeNull();
    expect(partitions).toEqual(["persist:osuna-browser-1700000000000-abcd"]);
  });
});

describe("clearOsunaBrowserProfile", () => {
  test("clears site data, HTTP cache, and auth before reloading live guests", async () => {
    ...
    const clearing = clearOsunaBrowserProfile({
