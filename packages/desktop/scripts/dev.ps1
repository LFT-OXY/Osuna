...
$env:EXPO_PUBLIC_OSUNA_DEV_BUILD_LABEL = (git -C $RootDir branch --show-current).Trim()

$env:OSUNA_DEV_ROOT = $RootDir
$env:OSUNA_DEV_RUNTIME_FALLBACK_ROOT = $RootDir
$DevRuntime = node "$ScriptDir\dev-runtime.mjs" | ConvertFrom-Json
$env:OSUNA_ELECTRON_FLAGS = $DevRuntime.electronFlags
$env:OSUNA_ELECTRON_USER_DATA_DIR = $DevRuntime.userDataDir
Remove-Item Env:\OSUNA_DEV_RUNTIME_FALLBACK_ROOT -ErrorAction SilentlyContinue
...
# Fully isolate the dev instance from a production Osuna install so `npm run dev`
...
# OSUNA_HOME defaults to a script-managed dev home. If you override it (to point
...
if (-not $env:OSUNA_HOME) {
    $env:OSUNA_HOME = "$DevStateDir\osuna-home"
    $OsunaHomeManaged = $true
} else {
    $OsunaHomeManaged = $false
}
New-Item -ItemType Directory -Force -Path $env:OSUNA_HOME, $env:OSUNA_ELECTRON_USER_DATA_DIR | Out-Null

$DevDaemonPort = if ($env:OSUNA_DEV_DAEMON_PORT) { $env:OSUNA_DEV_DAEMON_PORT } else { "6788" }
if (-not $env:OSUNA_LISTEN) { $env:OSUNA_LISTEN = "127.0.0.1:$DevDaemonPort" }
...
# (it does NOT honor the OSUNA_LISTEN env var) and probing that address. Without
...
# ONLY seed the script-managed home: never rewrite a user-supplied OSUNA_HOME
# (that could clobber a production config.json with the dev port + wildcard CORS).
if ($OsunaHomeManaged) {
    $env:TMP_CFG_PATH = "$($env:OSUNA_HOME)/config.json"
...
"@
    ...
    Write-Host "  (custom OSUNA_HOME - leaving its config.json untouched)"
}

Write-Host @"
======================================================
  Osuna Desktop Dev (Windows)
======================================================
  Metro:      http://localhost:$($env:EXPO_PORT)
  Daemon:     $($env:OSUNA_LISTEN) (isolated)
  OSUNA_HOME: $($env:OSUNA_HOME)
  userData:   $($env:OSUNA_ELECTRON_USER_DATA_DIR)
======================================================
"@
...
concurrently `
    ...
    "cd `"$AppDir`" && cross-env OSUNA_WEB_PLATFORM=electron npx expo start --port $($env:EXPO_PORT)" `
