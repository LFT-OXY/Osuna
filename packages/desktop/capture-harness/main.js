...
const OUT_DIR = process.env.OSUNA_CAPTURE_HARNESS_OUT_DIR || path.join(ROOT, "out");
...
const SOAK_MS = Number(process.env.OSUNA_CAPTURE_HARNESS_SOAK_MS || 75000);
const HARNESS_GROUP = process.env.OSUNA_CAPTURE_HARNESS_GROUP || "permanent-parking";
const BROWSER_PROFILE_PHASE = process.env.OSUNA_CAPTURE_HARNESS_PHASE || "";
...
const PERMANENT_STATE_FILTER = new Set(
  (process.env.OSUNA_CAPTURE_HARNESS_STATES || "P1")
    .split(",")
    .map((state) => state.trim())
    .filter(Boolean),
);
const PERMANENT_VARIANT_FILTER = new Set(
  (process.env.OSUNA_CAPTURE_HARNESS_VARIANTS || "default")
    .split(",")
...
const AUTOMATION_SNAPSHOT_PROBE = String.raw`(() => {
  ...
  function runtime() {
    ...
    Object.defineProperty(window, '__OSUNA_BROWSER_AUTOMATION__', { configurable: true, value: api });
    return api;
  }
  ...
  const result = await guest.executeJavaScript(
    String.raw`(async () => {
      ...
      while (performance.now() <= deadline) {
        const resolved = window.__OSUNA_BROWSER_AUTOMATION__.resolve(ref, fingerprint);
...
async function automationEvaluate(guest, functionSource, refEntry) {
  return guest.executeJavaScript(
    String.raw`(async () => {
      ...
      ${
        refEntry
          ? `const resolved = window.__OSUNA_BROWSER_AUTOMATION__.resolve(${JSON.stringify(refEntry.ref)}, ${JSON.stringify(refEntry.fingerprint)});
...
const AUTOMATION_PROMPT_SHIM_INSTALL = String.raw`(() => {
  const stateKey = "__OSUNA_BROWSER_AUTOMATION_DIALOG_STATE__";
...
const AUTOMATION_PROMPT_SHIM_DRAIN = String.raw`(() => {
  const state = window.__OSUNA_BROWSER_AUTOMATION_DIALOG_STATE__;
...
async function runAutomationGroup() {
  ...
  try {
    ...
    const pushStateResult = await guest.executeJavaScript(
      `window.__OSUNA_BROWSER_AUTOMATION__.resolve(${JSON.stringify(saveRef.ref)}, ${JSON.stringify(saveRef.fingerprint)}).ok`,
      true,
    );
    ...
    const rerenderResult = await guest.executeJavaScript(
      `window.__OSUNA_BROWSER_AUTOMATION__.resolve(${JSON.stringify(saveRef.ref)}, ${JSON.stringify(saveRef.fingerprint)}).reason`,
      true,
    );
