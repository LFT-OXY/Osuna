...
declare global {
  interface Window {
    __osunaStartupPresentationTrace?: StartupPresentation[];
  }
}
...
async function observeStartupPresentation(page: Page): Promise<void> {
  await page.addInitScript(() => {
    ...
    window.__osunaStartupPresentationTrace = trace;
...
async function getStartupPresentation(page: Page): Promise<StartupPresentation[]> {
  return page.evaluate(() => window.__osunaStartupPresentationTrace?.slice() ?? []);
}
