...
describe("plugin selector shallow equality", () => {
  it("keeps equivalent selector objects and arrays stable", () => {
    ...
    expect(shallowEqual({ name: "Osuna", shared }, { name: "Osuna", shared })).toBe(true);
    expect(shallowEqual(["Osuna", shared], ["Osuna", shared])).toBe(true);
    expect(shallowEqual({ name: "Osuna" }, { name: "Changed" })).toBe(false);
  });
