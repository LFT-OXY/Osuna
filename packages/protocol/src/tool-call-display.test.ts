...
describe("shared tool-call display mapping", () => {
  ...
  it("builds display model for worktree setup detail", () => {
    const display = buildToolCallDisplayModel({
      ...
      detail: {
        type: "worktree_setup",
        worktreePath: "/tmp/repo/.osuna/worktrees/repo/branch",
        ...
        commands: [
          {
            index: 1,
            command: "npm install",
            cwd: "/tmp/repo/.osuna/worktrees/repo/branch",
            log: "==> [1/1] Running: npm install\n",
            status: "running",
            exitCode: null,
          },
  ...
  it("humanizes Osuna MCP tool names (Claude Code format)", () => {
    const display = buildToolCallDisplayModel({
  ...
  it("humanizes Osuna MCP tool names (Codex format)", () => {
    const display = buildToolCallDisplayModel({
  ...
  it("humanizes list_agents Osuna tool", () => {
    const display = buildToolCallDisplayModel({
