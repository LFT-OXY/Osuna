...
const DIRECTORY_COUNT = readPositiveInteger("OSUNA_WATCH_BENCH_DIRS", 500);
const ROOT_COUNT = readPositiveInteger("OSUNA_WATCH_BENCH_ROOTS", 1);
const IGNORED_FILE_COUNT = readPositiveInteger("OSUNA_WATCH_BENCH_IGNORED_FILES", 2_000);
const PREEXISTING_FILE_COUNT = readPositiveInteger("OSUNA_WATCH_BENCH_PREEXISTING_FILES", 1_000);
const EDIT_COUNT = readPositiveInteger("OSUNA_WATCH_BENCH_EDITS", 100);
const SUSTAINED_EDIT_COUNT = readPositiveInteger("OSUNA_WATCH_BENCH_SUSTAINED_EDITS", 20);
const SUSTAINED_EDIT_INTERVAL_MS = readPositiveInteger(
  "OSUNA_WATCH_BENCH_SUSTAINED_INTERVAL_MS",
  500,
);
const REPETITIONS = readPositiveInteger("OSUNA_WATCH_BENCH_REPETITIONS", 1);
...
async function measure(run: number): Promise<Measurement> {
  const base = await mkdtemp(join(tmpdir(), "osuna-watch-"));
