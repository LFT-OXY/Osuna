...
const ITERATIONS = readPositiveInteger("OSUNA_WATCH_REPRO_ITERATIONS", 200);
const CHILD_TIMEOUT_MS = readPositiveInteger(
  "OSUNA_WATCH_REPRO_TIMEOUT_MS",
...
async function runChild(): Promise<ChildResult> {
  const base = await mkdtemp(join(tmpdir(), "osuna-watch-repro-"));
