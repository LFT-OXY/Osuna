...
const MEASUREMENT_CWD = "/tmp/osuna-agent-tools-measurement";
...
Measures the MCP tools/list payload for Osuna agent tools.
...
async function measureCatalog(params: {
  ...
  const client = new Client({ name: "osuna-agent-tools-measurement", version: "0.0.0" });
...
function formatMarkdown(measurements: CatalogMeasurement[], top: number): string {
  ...
  const delta =
    withoutBrowser && withBrowser
      ...
      : null;
  const lines: string[] = [];
  lines.push("# Osuna Agent Tool Catalog Context");
  lines.push("");
