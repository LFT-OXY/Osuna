...
OSUNA_HOME=~/.osuna        # Runtime state directory (agents/, etc.)
OSUNA_LISTEN=127.0.0.1:6777  # Listen address (host:port or /path/to/socket)
```

`OSUNA_HOME` defaults to `~/.osuna` and isolates runtime artifacts like `agents/`. `OSUNA_LISTEN` controls the daemon listen address. For blue/green testing you can run a parallel server without touching production state:

```bash
OSUNA_HOME=~/.osuna-blue OSUNA_LISTEN=127.0.0.1:7777 npm run dev
```
