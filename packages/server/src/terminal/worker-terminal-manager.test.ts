...
it("delivers rapid small writes complete and in order through worker coalescing", async () => {
  ...
  const session = trackTerminal(
    await manager.createTerminal({
      workspaceId: "ws-test",
      cwd,
      env: { OSUNA_TERMINAL_BURST_GATE: burstGatePath },
      ...nodeTerminalCommand(`
      const fs = require("node:fs");
      const gatePath = process.env.OSUNA_TERMINAL_BURST_GATE;
...
it("keeps registered cwd env inheritance behind the worker manager interface", async () => {
  ...
  manager.registerCwdEnv({
    cwd,
    env: { OSUNA_WORKER_TERMINAL_TEST: "worker-env" },
  });
  trackTerminal(
    await manager.createTerminal({
      workspaceId: "ws-test",
      ...
      require("node:fs").writeFileSync(
        ...
        process.env.OSUNA_WORKER_TERMINAL_TEST ?? "",
      );
      setInterval(() => {}, 1000);
    `),
    }),
...
it("injects parent-minted terminal activity env through the worker", async () => {
    ...
    await manager.createTerminal({
      ...
      ...nodeTerminalCommand(`
        require("node:fs").writeFileSync(
          ...
          JSON.stringify({
            terminalId: process.env.OSUNA_TERMINAL_ID,
            token: process.env.OSUNA_ACTIVITY_TOKEN,
            url: process.env.OSUNA_TERMINAL_ACTIVITY_URL,
            hookCli: process.env.OSUNA_HOOK_CLI,
  ...
  const osunaCliBinDir = resolveOsunaCliBinDir();
  const osunaCliPath = resolveOsunaCliExecutablePath();
  expect(osunaCliBinDir).not.toBeNull();
  expect(osunaCliPath).not.toBeNull();
  ...
  expect(env.hookCli).toBe(osunaCliPath);
  ...
  expect(env.path?.split(delimiter)[0]).toBe(osunaCliBinDir);
});
