...
function hookPaths(root: string) {
  return {
    ...
    opencode: join(root, "opencode", "plugins", "osuna-terminal-activity.js"),
  };
}

function createStore(osunaHome: string, enableTerminalAgentHooks: boolean): DaemonConfigStore {
...
describe("applyTerminalAgentHookSetting", () => {
  it("leaves agent configs untouched when the setting is disabled", () => {
    const root = createTempDir("osuna-hook-setting-");
    const store = createStore(createTempDir("osuna-hook-setting-home-"), false);
  ...
  it("installs agent hooks when the setting is enabled", () => {
    const root = createTempDir("osuna-hook-setting-");
    const store = createStore(createTempDir("osuna-hook-setting-home-"), true);
