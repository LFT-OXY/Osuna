...
export const codexHooksFormat: AgentHookConfigFormat<CodexHooksFile> = {
  ...
  install(config, provider) {
    ...
    for (const event of provider.events) {
      const userEntries = removeOsunaHooks(hooks[event.event], install.hookMarker);
      hooks[event.event] = [
        ...userEntries,
        {
          matcher: "",
  ...
  uninstall(config, provider) {
    ...
    const hooks = normalizeHooks(config.hooks);
    for (const event of provider.events) {
      const entries = removeOsunaHooks(hooks[event.event], install.hookMarker);
      if (entries.length > 0) {
    ...
    return provider.events.every((event) =>
      normalizeMatchers(hooks[event.event]).some((entry) =>
        normalizeCommandHooks(entry.hooks).some((hook) =>
          hasOsunaCommands(hook, install.hookMarker),
        ),
      ),
    );
  },
...
function removeOsunaHooks(value: unknown, marker: string): CodexMatcherGroup[] {
...
function hasOsunaCommands(hook: CodexCommandHook, marker: string): boolean {
  return (
