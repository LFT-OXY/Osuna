...
async function makeSandbox(): Promise<Sandbox> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "osuna-skill-sync-"));
...
describe("syncSkills", () => {
  ...
  it("overwrites on-disk skill content when the bundle differs", async () => {
    await writeBundleSkill(sandbox.sourceDir, "osuna", {
      "SKILL.md": "new osuna content",
    });
    const onDiskSkill = path.join(sandbox.agentsDir, "osuna");
    await fs.mkdir(onDiskSkill, { recursive: true });
    await fs.writeFile(path.join(onDiskSkill, "SKILL.md"), "old osuna content");
    ...
    const agentsContent = await fs.readFile(
      path.join(sandbox.agentsDir, "osuna", "SKILL.md"),
      "utf-8",
    );
    expect(agentsContent).toBe("new osuna content");
    const claudeContent = await fs.readFile(
      path.join(sandbox.claudeDir, "osuna", "SKILL.md"),
      "utf-8",
    );
    expect(claudeContent).toBe("new osuna content");
    const codexContent = await fs.readFile(
      path.join(sandbox.codexDir, "osuna", "SKILL.md"),
      "utf-8",
    );
    expect(codexContent).toBe("new osuna content");
  });

  it("installs new bundled skills, including references/, when not present on disk", async () => {
    await writeBundleSkill(sandbox.sourceDir, "osuna-committee", {
    ...
    await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["osuna-committee"],
    });

    expect(
      await fs.readFile(path.join(sandbox.agentsDir, "osuna-committee", "SKILL.md"), "utf-8"),
    ).toBe("committee content");
    expect(
      await fs.readFile(
        path.join(sandbox.agentsDir, "osuna-committee", "references", "roles.md"),
        "utf-8",
      ),
    ).toBe("roles content");
    expect(
      await fs.readFile(
        path.join(sandbox.codexDir, "osuna-committee", "references", "roles.md"),
        "utf-8",
      ),
    ).toBe("roles content");

    const claudeSkillDir = path.join(sandbox.claudeDir, "osuna-committee");
  ...
  it("leaves on-disk skills not in the bundle untouched", async () => {
    await writeBundleSkill(sandbox.sourceDir, "osuna", { "SKILL.md": "new" });
  ...
  it("removes stale files previously written to managed skill dirs", async () => {
    ...
    await fs.rm(path.join(sandbox.sourceDir, "osuna"), { recursive: true, force: true });
    await writeBundleSkill(sandbox.sourceDir, "osuna", { "SKILL.md": "new" });
  ...
  it("skips skills listed in skillNames that are missing from the bundle without raising", async () => {
    await writeBundleSkill(sandbox.sourceDir, "osuna", { "SKILL.md": "content" });
    // "osuna-removed" is in skillNames but not in the bundle on disk.
    ...
    const result = await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["osuna", "osuna-removed"],
      onSkillError: (name, error) => errors.push({ name, error }),
    });

    expect(errors).toEqual([]);
    expect(result.processedSkills).toBe(1);
    expect(await fs.readFile(path.join(sandbox.agentsDir, "osuna", "SKILL.md"), "utf-8")).toBe(
      "content",
    );
    await expect(fs.access(path.join(sandbox.agentsDir, "osuna-removed"))).rejects.toThrow();
  });

  it("leaves on-disk skill content alone when the skill has been removed from the bundle", async () => {
    await writeBundleSkill(sandbox.sourceDir, "osuna", { "SKILL.md": "current" });
    const deprecatedDir = path.join(sandbox.agentsDir, "osuna-deprecated");
    ...
    await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["osuna", "osuna-deprecated"],
    });
  ...
  it("reports per-skill errors via onSkillError without throwing", async () => {
    ...
    await fs.writeFile(path.join(sandbox.agentsDir, "osuna"), "blocking file");
...
describe("removeSkill", () => {
  ...
  it("removes the skill from all three targets when present", async () => {
    ...
    await removeSkill("osuna", {
      agentsDir: sandbox.agentsDir,
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
    });

    await expect(fs.access(path.join(sandbox.agentsDir, "osuna"))).rejects.toThrow();
    await expect(fs.access(path.join(sandbox.claudeDir, "osuna"))).rejects.toThrow();
    await expect(fs.access(path.join(sandbox.codexDir, "osuna"))).rejects.toThrow();
  });
