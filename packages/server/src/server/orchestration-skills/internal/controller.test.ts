...
const BUNDLED_SKILLS = ["osuna", "osuna-advisor", "osuna-loop"];

async function makeHarness(selectionStore?: SkillSelectionStore): Promise<Harness> {
  const root = await mkdtemp(path.join(os.tmpdir(), "osuna-skills-controller-"));
...
describe("skills controller", () => {
  it("imports a legacy selection only while daemon selection is absent", async () => {
    ...
    await expect(
      harness.controller.importLegacySelectionIfUnset({
        mode: "custom",
        skills: ["osuna", "osuna-loop"],
      }),
    ).resolves.toEqual({
      imported: true,
      selection: { mode: "custom", skills: ["osuna", "osuna-loop"] },
    });
  ...
  it("reports one snapshot with catalog, selection, status, and pending work", async () => {
    expect(await harness.controller.status()).toEqual({
      ...
      ops: [
        { kind: "add", name: "osuna" },
        { kind: "add", name: "osuna-advisor" },
        { kind: "add", name: "osuna-loop" },
      ],
  ...
  it("installs every bundled skill while the selection is all", async () => {
    ...
    expect(await isInstalled(harness.targets, "osuna-advisor")).toBe(true);
  });

  it("does not remove deselected directories during install", async () => {
    await harness.controller.save({ mode: "custom", skills: ["osuna"] });
    await writeUserFile(harness.targets, "osuna-loop", "notes/mine.md", "keep this");

    await harness.controller.install();

    expect(await readUserFile(harness.targets, "osuna-loop", "notes/mine.md")).toEqual([
      "keep this",
      "keep this",
      "keep this",
    ]);
  ...
  it("saves a custom selection, converges disk, and returns the refreshed snapshot", async () => {
    ...
    expect(snapshot).toEqual({
      ...
      ops: [],
      available: BUNDLED_SKILLS,
      installed: ["osuna", "osuna-loop"],
      selection: { mode: "custom", skills: ["osuna", "osuna-loop"] },
      confirmationRequired: null,
    });
    expect(await isInstalled(harness.targets, "osuna")).toBe(true);
    expect(await isInstalled(harness.targets, "osuna-loop")).toBe(true);
    expect(await isInstalled(harness.targets, "osuna-advisor")).toBe(false);
  });
  ...
  it("removes a skill from disk when it is dropped from the selection", async () => {
    ...
    await harness.controller.save({
      mode: "custom",
      skills: ["osuna"],
      confirmedRemovals: ["osuna-advisor", "osuna-loop"],
    });

    expect(await isInstalled(harness.targets, "osuna")).toBe(true);
    expect(await isInstalled(harness.targets, "osuna-advisor")).toBe(false);
    expect(await isInstalled(harness.targets, "osuna-loop")).toBe(false);
  });
  ...
  it("treats an empty custom selection as uninstall while keeping the preference", async () => {
    ...
    expect(await isInstalled(harness.targets, "osuna")).toBe(false);
  });
  ...
  it("saves no selection at all when the very first save fails", async () => {
    ...
    await expect(harness.controller.save({ mode: "custom", skills: ["osuna"] })).rejects.toThrow();
    await rm(harness.targets.agentsDir, { force: true });
  ...
  it("restores deleted directories byte for byte when the selection cannot be committed", async () => {
    ...
    await writeUserFile(readOnly.targets, "osuna-loop", "notes/mine.md", "hand written");

    // Deselects osuna-loop and adds osuna-advisor, then fails to commit.
    await expect(
      readOnly.controller.save({
        mode: "custom",
        skills: ["osuna", "osuna-advisor"],
    ...
    expect(await readUserFile(readOnly.targets, "osuna-loop", "notes/mine.md")).toEqual([
      "hand written",
      "hand written",
      "hand written",
    ]);
  ...
  it("preserves files added by another writer before rollback", async () => {
    const selection: SkillSelection = { mode: "custom", skills: ["osuna"] };
    const gated = createGatedUnwritableSelectionStore(selection);
    const readOnly = await makeHarness(gated.store);
    await readOnly.controller.install();
    await writeFile(path.join(readOnly.targets.sourceDir, "osuna", "SKILL.md"), "osuna-v2");

    const save = readOnly.controller.save(selection);
    await gated.persistenceStarted;
    await writeUserFile(readOnly.targets, "osuna", "notes/concurrent.md", "keep this");
    gated.failPersistence();
    await expect(save).rejects.toThrow("selection store is read-only");

    expect(await readUserFile(readOnly.targets, "osuna", "notes/concurrent.md")).toEqual([
      "keep this",
      "keep this",
      "keep this",
    ]);
  ...
  it("does not automatically delete files preserved from a rolled-back add", async () => {
    const previous: SkillSelection = { mode: "custom", skills: ["osuna"] };
    ...
    await gated.persistenceStarted;
    await writeUserFile(readOnly.targets, "osuna-loop", "notes/concurrent.md", "keep this");
    gated.failPersistence();
    await expect(save).rejects.toThrow("selection store is read-only");

    await readOnly.controller.autoUpdate();
    expect(await readUserFile(readOnly.targets, "osuna-loop", "notes/concurrent.md")).toEqual([
      "keep this",
      "keep this",
      "keep this",
    ]);
  ...
  it("merges a deleted directory backup into files another writer recreated", async () => {
    ...
    const readOnly = await makeHarness(gated.store);
    await readOnly.controller.install();
    await writeUserFile(readOnly.targets, "osuna-loop", "notes/before.md", "restore this");
    ...
    expect(await readUserFile(readOnly.targets, "osuna-loop", "notes/before.md")).toEqual([
      "restore this",
      "restore this",
      "restore this",
    ]);
  ...
  it("atomically stages a deletion before another writer can recreate its path", async () => {
    ...
    const next: SkillSelection = { mode: "custom", skills: ["osuna"] };
    await harness.controller.save(previous);

    const transaction = await beginSkillsTransaction(harness.targets, previous, next, [
      { kind: "delete", name: "osuna-loop" },
    ]);

    expect(await isInstalled(harness.targets, "osuna-loop")).toBe(false);
    await writeUserFile(harness.targets, "osuna-loop", "notes/concurrent.md", "keep this");
    await transaction.rollback();
    expect(await readUserFile(harness.targets, "osuna-loop", "notes/concurrent.md")).toEqual([
      "keep this",
      "keep this",
      "keep this",
    ]);
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      ].map((root) => path.join(root, "osuna-loop"));
  ...
  it("finishes rollback when an external deletion leaves no live or staged path", async () => {
    ...
    await rm(path.join(harness.targets.codexDir, codexStage!, "osuna-loop"), {
      recursive: true,
      force: true,
    });
    ...
    expect(await readUserFile(harness.targets, "osuna-loop", "SKILL.md")).toEqual([
      "osuna-loop-v1",
      "osuna-loop-v1",
      null,
    ]);
  ...
  it("quarantines a staged directory when an external file takes its live path", async () => {
    ...
    const live = path.join(harness.targets.codexDir, "osuna-loop");
  ...
  it("quarantines staged files that collide with a recreated directory", async () => {
    ...
    const next: SkillSelection = { mode: "custom", skills: ["osuna"] };
    await harness.controller.save(previous);
    await writeUserFile(harness.targets, "osuna-loop", "notes/mine.md", "staged notes");
    ...
    const recovered = (await readdir(harness.targets.codexDir)).find((entry) =>
      entry.startsWith(".osuna-skills-recovered-osuna-loop-"),
    );
    expect(recovered).toBeDefined();
    expect(
  ...
  it("preserves incompatible live paths while rolling back adds and updates", async () => {
    ...
    await harness.controller.save(previous);
    await writeUserFile(harness.targets, "osuna", "notes/mine.md", "keep this");

    const transaction = await beginSkillsTransaction(harness.targets, previous, next, [
      { kind: "update", name: "osuna" },
      { kind: "add", name: "osuna-advisor" },
    ]);
    const replacedUpdate = path.join(harness.targets.agentsDir, "osuna");
    await rm(replacedUpdate, { recursive: true, force: true });
    await writeFile(replacedUpdate, "external update replacement");
    const replacedAdd = path.join(harness.targets.codexDir, "osuna-advisor");
    ...
    const recovered = (await readdir(recoveryParent)).find((entry) =>
      entry.startsWith(".osuna-skills-recovered-osuna-"),
    );
    expect(recovered).toBeDefined();
    expect(await readFile(path.join(recoveryParent, recovered!, "notes", "mine.md"), "utf8")).toBe(
      "keep this",
    );
    expect(await readUserFile(harness.targets, "osuna", "notes/mine.md")).toEqual([
      null,
      "keep this",
      "keep this",
    ]);
  ...
  it("recovers after an update backup was quarantined before transaction cleanup", async () => {
    ...
    await harness.controller.save(previous);
    await writeUserFile(harness.targets, "osuna", "notes/mine.md", "captured notes");
    ...
    const entry = manifest.entries.find(
      (candidate) => candidate.livePath === path.join(harness.targets.agentsDir, "osuna"),
    );
    expect(entry?.backupPath).toBeTruthy();
    const backup = path.join(transactionDir, entry!.backupPath!);
    const recovered = path.join(
      transactionParent,
      `.osuna-skills-recovered-osuna-${transactionName!.replace(".osuna-skills-transaction-", "")}`,
    );
  ...
  it("preserves a directory that replaces a captured file before recovery", async () => {
    ...
    await harness.controller.save(previous);
    const live = path.join(harness.targets.agentsDir, "osuna");
  ...
  it.skipIf(process.platform !== "linux")(
    ...
    async () => {
      const crossFilesystemRoot = await mkdtemp("/dev/shm/osuna-skills-controller-");
      try {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const next: SkillSelection = { mode: "custom", skills: ["osuna"] };
      await harness.controller.save(previous);
      const shared = path.join(harness.root, "home", "shared", "osuna-loop");
      await mkdir(shared, { recursive: true });
      await writeFile(path.join(shared, "SKILL.md"), "shared target");
      const live = path.join(harness.targets.claudeDir, "osuna-loop");
      await rm(live, { recursive: true, force: true });
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const readOnly = await makeHarness(gated.store);
      await readOnly.controller.install();
      const shared = path.join(readOnly.root, "home", "shared", "osuna");
      await mkdir(shared, { recursive: true });
      await writeFile(path.join(shared, "SKILL.md"), "osuna-v1");
      const live = path.join(readOnly.targets.claudeDir, "osuna");
      await rm(live, { recursive: true, force: true });
      ...
      expect((await lstat(live)).isSymbolicLink()).toBe(true);
      expect(await readFile(path.join(shared, "SKILL.md"), "utf8")).toBe("osuna-v1");
      await rm(readOnly.root, { recursive: true, force: true });
    },
  );
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      for (const root of [
        ...
        const notes = path.join(root, "osuna-loop", "notes");
      ...
      for (const root of [
        ...
        const restored = path.join(root, "osuna-loop", "notes", "latest.md");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const readOnly = await makeHarness(gated.store);
      await readOnly.controller.install();
      await writeUserFile(readOnly.targets, "osuna-loop", "hooks/run.sh", "#!/bin/sh\n");
      for (const root of [
        readOnly.targets.agentsDir,
        readOnly.targets.claudeDir,
        readOnly.targets.codexDir,
      ]) {
        await chmod(path.join(root, "osuna-loop", "hooks", "run.sh"), 0o751);
      }
      ...
      for (const root of [
        ...
        const restored = await lstat(path.join(root, "osuna-loop", "hooks", "run.sh"));
  ...
  it("recovers an interrupted save before the next controller operation", async () => {
    ...
    await harness.controller.save(previous);
    await writeUserFile(harness.targets, "osuna-loop", "notes/mine.md", "hand written");
    await beginSkillsTransaction(harness.targets, previous, next, [
      { kind: "delete", name: "osuna-loop" },
    ]);
    for (const root of [
      ...
      await rm(path.join(root, "osuna-loop"), { recursive: true, force: true });
    }
  ...
  it("asks for confirmation naming the directories a save would delete", async () => {
    await harness.controller.save({ mode: "custom", skills: ["osuna", "osuna-loop"] });
    // Something puts a managed directory back after the UI took its snapshot.
    await writeUserFile(harness.targets, "osuna-advisor", "SKILL.md", "external");
    ...
    expect(result.confirmationRequired).toEqual({ removals: ["osuna-advisor"] });
    expect(await installedEverywhere(harness.targets)).toEqual([
      ["osuna", "osuna-advisor", "osuna-loop"],
      ["osuna", "osuna-advisor", "osuna-loop"],
      ["osuna", "osuna-advisor", "osuna-loop"],
    ]);
    expect(result.selection).toEqual({ mode: "custom", skills: ["osuna", "osuna-loop"] });
  });
  ...
  it("applies the save once the removals are confirmed", async () => {
    ...
    expect(result.confirmationRequired).toBeNull();
    expect(result.selection).toEqual({ mode: "custom", skills: ["osuna"] });
    expect(await installedEverywhere(harness.targets)).toEqual([["osuna"], ["osuna"], ["osuna"]]);
  });

  it("asks again when another directory appears before the retry", async () => {
    await harness.controller.install();
    await writeUserFile(harness.targets, "osuna-chat", "SKILL.md", "retired but present");
    ...
    expect(result.confirmationRequired).toEqual({
      removals: ["osuna-advisor", "osuna-chat", "osuna-loop"],
    });
    expect(await installedEverywhere(harness.targets)).toEqual([
      ["osuna", "osuna-advisor", "osuna-chat", "osuna-loop"],
      ["osuna", "osuna-advisor", "osuna-chat", "osuna-loop"],
      ["osuna", "osuna-advisor", "osuna-chat", "osuna-loop"],
    ]);
  });
  ...
  it("does not commit when a new removal appears while the frozen plan is applying", async () => {
    ...
    await writeFile(path.join(harness.targets.sourceDir, "osuna", "SKILL.md"), "osuna-v2");
    ...
    await writeUserFile(harness.targets, "osuna-chat", "notes/mine.md", "hand written");

    const result = await save;

    expect(result.confirmationRequired).toEqual({ removals: ["osuna-chat"] });
    expect(result.selection).toEqual(selection);
    expect(await readUserFile(harness.targets, "osuna-chat", "notes/mine.md")).toEqual([
      "hand written",
      "hand written",
      "hand written",
    ]);
  ...
  it("saves without asking when nothing would be deleted", async () => {
    const result = await harness.controller.save({ mode: "custom", skills: ["osuna"] });
  ...
  it("preserves a regular file at a skill path when save convergence fails", async () => {
    ...
    const collision = path.join(harness.targets.agentsDir, "osuna");
  ...
  it("serializes startup convergence with an interactive save", async () => {
    ...
    await rm(path.join(harness.targets.claudeDir, "osuna-loop"), {
      recursive: true,
      force: true,
    });
    ...
    expect(saved.selection).toEqual({ mode: "custom", skills: ["osuna"] });
    expect(await installedEverywhere(harness.targets)).toEqual([["osuna"], ["osuna"], ["osuna"]]);
    expect(await harness.controller.status()).toEqual({
      state: "up-to-date",
      ops: [],
  ...
  it("updates a drifted install without touching the saved selection", async () => {
    await harness.controller.save({ mode: "custom", skills: ["osuna"] });
    await writeFile(path.join(harness.targets.agentsDir, "osuna", "SKILL.md"), "stale");
