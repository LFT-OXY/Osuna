...
import type { ProjectCheckoutLitePayload, ProjectPlacementPayload } from "@osuna/protocol/messages";
...
export type MutableWorkspacePlacement = Pick<
  PersistedWorkspaceRecord,
  "kind" | "branch" | "worktreeRoot" | "isOsunaOwnedWorktree" | "mainRepoRoot"
>;
...
export function initialWorkspacePlacement(
  ...
  return {
    ...
    baseBranch: null,
    isOsunaOwnedWorktree: input.checkout.isGit && input.checkout.isOsunaOwnedWorktree,
...
export function reconcileWorkspacePlacement(input: {
  ...
  if (input.workspace.isOsunaOwnedWorktree !== observed.isOsunaOwnedWorktree)
    fields.isOsunaOwnedWorktree = observed.isOsunaOwnedWorktree;
...
export function checkoutFromPersistedWorkspacePlacement(input: {
  ...
  if (workspace.isOsunaOwnedWorktree && workspace.mainRepoRoot) {
    return {
      ...checkout,
      isGit: true,
      isOsunaOwnedWorktree: true,
...
export function checkoutLiteFromGitSnapshot(
  ...
  if (git.isOsunaOwnedWorktree && git.mainRepoRoot) {
    return {
      cwd,
      isGit: true,
