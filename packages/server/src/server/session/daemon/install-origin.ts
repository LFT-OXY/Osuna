...
export function validateDaemonInstallOrigin(
  ...
  if (install.isLinked) {
    return `The global ${OSUNA_CLI_PACKAGE} install is linked; self-update only supports normal npm global installs.`;
  }

  if (daemonVersion && install.version !== daemonVersion) {
    return `This daemon is not running from the npm global ${OSUNA_CLI_PACKAGE} install (global npm has ${install.version}, daemon is ${daemonVersion}).`;
  }
  ...
  if (!isCurrentServerUnderNpmInstall(currentServerPackageRoot, install)) {
    return `This daemon is not running from the npm global ${OSUNA_CLI_PACKAGE} install.`;
  }
...
function resolveCurrentServerPackageRoot(): string | null {
  return resolvePackageRootFrom(fileURLToPath(import.meta.url), "@osuna/server");
}
