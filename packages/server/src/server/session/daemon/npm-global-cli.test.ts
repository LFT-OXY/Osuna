...
import { DefaultNpmGlobalOsunaCli } from "./npm-global-cli.js";
...
const cliPackagePath = path.join(globalNodeModules, "@osuna", "cli");

function npmGlobalOsunaCliJson(version: string, options?: { linked?: boolean }): string {
...
describe("DefaultNpmGlobalOsunaCli", () => {
  test("inspects the npm global cli install with npm -g ls", async () => {
    const calls: CommandCall[] = [];
    const cli = new DefaultNpmGlobalOsunaCli(async (command, args, options) => {
      calls.push({
        command,
        args,
      ...
      return { exitCode: 0, stdout: npmGlobalOsunaCliJson("0.1.15"), stderr: "" };
    });
    ...
    expect(calls).toEqual([
      {
        command: "npm",
        args: ["-g", "ls", "@osuna/cli", "--json", "--depth=0", "--long"],
        timeout: 10_000,
  ...
  test("runs the global install command for the latest cli", async () => {
    ...
    expect(calls).toEqual([
      {
        command: "npm",
        args: ["install", "-g", "@osuna/cli@latest"],
        timeout: 300_000,
        maxBuffer: 10 * 1024 * 1024,
      },
    ]);
  ...
  test("reports missing npm when npm exits without JSON", async () => {
    const cli = new DefaultNpmGlobalOsunaCli(async () => ({
      exitCode: 127,
      stdout: "",
  ...
  test("reports missing global cli when npm output has no cli dependency", async () => {
    ...
    await expect(cli.inspect()).rejects.toThrow(
      "@osuna/cli is not installed with npm -g on this host",
    );
  });
});
