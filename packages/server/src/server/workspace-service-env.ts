...
export function buildWorkspaceServiceEnv(
  ...
  const env: Record<string, string> = {
    ...
    OSUNA_PORT: String(selfPeer.port),
  };
  ...
  if (selfProxyUrl) {
    env.OSUNA_URL = selfProxyUrl;
  }

  for (const peer of options.peers) {
    const envName = normalizeServiceEnvName(peer.scriptName);
    env[`OSUNA_SERVICE_${envName}_PORT`] = String(peer.port);
    ...
    if (peerProxyUrl) {
      env[`OSUNA_SERVICE_${envName}_URL`] = peerProxyUrl;
    }
  }
