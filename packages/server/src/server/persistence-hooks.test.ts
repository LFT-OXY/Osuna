...
describe("persistence hooks", () => {
  test("buildConfigOverrides preserves the complete private launch config", () => {
    const record = createRecord({
      ...
      config: {
        ...
        toolPolicy: {
          preapproved: [{ kind: "mcp", server: "osuna", tool: "report_status" }],
        },
  ...
  test("buildConfigOverrides drops persisted internal osuna MCP server", () => {
    const record = createRecord({
      config: {
        modeId: "default",
        model: "gpt-5.4-mini",
  ...
  test("buildConfigOverrides preserves user-provided osuna MCP server", () => {
    const record = createRecord({
      config: {
        modeId: "default",
        model: "gpt-5.4-mini",
