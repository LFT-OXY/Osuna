...
async function runLoggerFixture(source: string): Promise<{ stdout: string; stderr: string }> {
  const tempDir = await mkdtemp(path.join(tmpdir(), "osuna-logger-fixture-"));
...
describe("resolveLogConfig", () => {
  const osunaHome = "/tmp/osuna-logger-tests";

  it("defaults to stdout JSON without file logging", () => {
    const result = resolveLogConfig(undefined, { osunaHome });
  ...
  it("keeps legacy level and format as stdout configuration", () => {
    const result = resolveLogConfig({ level: "warn", format: "pretty" }, { osunaHome });
  ...
  it("enables file output only when log.file is present", () => {
    ...
    expect(resolveLogConfig(config, { osunaHome })).toEqual({
      level: "debug",
      console: {
        level: "warn",
        format: "json",
      ...
      file: {
        level: "debug",
        path: path.resolve(osunaHome, "logs", "programmatic.log"),
      },
    });
  });
  ...
  it("defaults file output to info when log.file is present without a level", () => {
    ...
    expect(resolveLogConfig(config, { osunaHome })).toEqual({
      ...
      file: {
        level: "info",
        path: path.resolve(osunaHome, "daemon.log"),
      },
    });
  });
});
...
describe("loadConfig logger config", () => {
  it("applies log format env at the config boundary", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "osuna-logger-config-"));
    ...
    const config = loadConfig(osunaHome, {
      env: { OSUNA_LOG_FORMAT: "pretty" },
    });

    expect(config.log?.format).toBe("pretty");
    expect(resolveLogConfig(config, { osunaHome }).console.format).toBe("pretty");
  });
});

describe("createRootLogger", () => {
  it("includes the daemon version in child logger entries", async () => {
    const osunaHome = await mkdtemp(path.join(tmpdir(), "osuna-logger-version-"));

    const { stdout } = await runLoggerFixture(`
      const logger = createRootLogger(undefined, { osunaHome: ${JSON.stringify(osunaHome)} });
  ...
  it("writes JSON to stdout by default and does not initialize file logging", async () => {
    const osunaHome = await mkdtemp(path.join(tmpdir(), "osuna-logger-default-"));
    const missingLogDir = path.join(osunaHome, "logs");
    ...
    expect(existsSync(path.join(osunaHome, "daemon.log"))).toBe(false);
    expect(existsSync(missingLogDir)).toBe(false);
  });

  it("writes to an explicit file target without creating rotation files", async () => {
    const osunaHome = await mkdtemp(path.join(tmpdir(), "osuna-logger-file-"));
    const logPath = path.join(osunaHome, "logs", "programmatic.log");

    await runLoggerFixture(`
      const logger = createRootLogger(
        { log: { file: { path: ${JSON.stringify(logPath)} } } },
        { osunaHome: ${JSON.stringify(osunaHome)} },
      );
  ...
  it("can disable file output for supervised workers", async () => {
    const osunaHome = await mkdtemp(path.join(tmpdir(), "osuna-logger-no-worker-file-"));
    const logPath = path.join(osunaHome, "daemon.log");

    const { stdout } = await runLoggerFixture(`
      const logger = createRootLogger(
        { log: { file: { path: ${JSON.stringify(logPath)} } } },
        { osunaHome: ${JSON.stringify(osunaHome)}, file: false },
      );
  ...
  it("keeps pretty output available as a format choice", async () => {
    const osunaHome = await mkdtemp(path.join(tmpdir(), "osuna-logger-pretty-"));
