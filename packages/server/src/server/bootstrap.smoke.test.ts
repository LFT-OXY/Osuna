...
describe("osuna daemon bootstrap", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  test("starts and serves health endpoint", async () => {
    const daemonHandle = await createTestOsunaDaemon({
  ...
  test("keeps timeline activity in memory and removes obsolete timeline files at startup", async () => {
    ...
    const obsoleteTimelineDirectory = path.join(osunaHome, "agent-timelines");
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "osuna-timeline-agent-"));
    await mkdir(obsoleteTimelineDirectory, { recursive: true });
    await writeFile(path.join(obsoleteTimelineDirectory, "obsolete.json"), "{}\n", "utf-8");

    const daemonHandle = await createTestOsunaDaemon({ osunaHomeRoot, cleanup: false });
    try {
      ...
      await daemonHandle.close();
      await Promise.all([
        rm(osunaHomeRoot, { recursive: true, force: true }),
  ...
  test("does not create a timeline directory for live timeline activity", async () => {
    const osunaHomeRoot = await mkdtemp(path.join(os.tmpdir(), "osuna-timeline-memory-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "osuna-timeline-agent-"));
    const daemonHandle = await createTestOsunaDaemon({ osunaHomeRoot, cleanup: false });
    const timelineDirectory = path.join(daemonHandle.osunaHome, "agent-timelines");
    try {
      const agent = await daemonHandle.daemon.agentManager.createAgent(
        { provider: "codex", cwd: agentCwd },
        undefined,
  ...
  test("reload applies live HTTP, MCP, Git, provider, relay, and app policies", async () => {
    const osunaHomeRoot = await mkdtemp(path.join(os.tmpdir(), "osuna-config-reload-runtime-"));
    const osunaHome = path.join(osunaHomeRoot, ".osuna");
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "osuna-static-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "osuna-config-reload-agent-"));
    ...
    config.agentStoragePath = path.join(osunaHome, "agents");
    config.isDev = true;
    config.speech = {
      providers: {
  ...
  test("rolls back daemon listener when standalone service proxy startup fails", async () => {
    ...
    const osunaHomeRoot = await mkdtemp(path.join(os.tmpdir(), "osuna-standalone-rollback-"));
    const osunaHome = path.join(osunaHomeRoot, ".osuna");
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "osuna-static-"));
    await mkdir(osunaHome, { recursive: true });
  ...
  test("relay config changes during Hub enrollment reach the live runtime", async () => {
    const osunaHomeRoot = await mkdtemp(path.join(os.tmpdir(), "osuna-relay-startup-"));
    const osunaHome = path.join(osunaHomeRoot, ".osuna");
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "osuna-static-"));
    await mkdir(osunaHome, { recursive: true });
    await writeFile(
      path.join(osunaHome, "hub-relationship.json"),
      `${JSON.stringify({
        version: 1,
        state: "pending",
        relationship: {
  ...
  test("rolls back standalone listener without starting plugins when main listen fails", async () => {
    ...
    const osunaHomeRoot = await mkdtemp(path.join(os.tmpdir(), "osuna-main-rollback-"));
    const osunaHome = path.join(osunaHomeRoot, ".osuna");
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "osuna-static-"));
    const pluginDirectory = path.join(osunaHomeRoot, "plugin");
...
export default function contribute(plugin: unknown) {
  ...
  test("starts when OpenAI speech provider is configured without credentials", async () => {
    const osunaHomeRoot = await mkdtemp(path.join(os.tmpdir(), "osuna-openai-config-"));
    const osunaHome = path.join(osunaHomeRoot, ".osuna");
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "osuna-static-"));
    await mkdir(osunaHome, { recursive: true });
  ...
  test("does not block daemon start on local speech model downloads", async () => {
    ...
    const daemonHandle = await createTestOsunaDaemon({
      speech: {
        ...
        local: {
          modelsDir: path.join(os.tmpdir(), `osuna-missing-models-${Date.now()}`),
          models: {
  ...
  test("rejects Windows absolute paths that are not named pipes", () => {
    ...
    expect(() => parseListenString(String.raw`D:\Users\foo\.osuna\daemon.sock`)).toThrow();
  ...
  test("parses Windows named pipes as managed IPC listen targets", () => {
    expect(parseListenString(String.raw`\\.\pipe\osuna-managed-test`)).toEqual({
      type: "pipe",
      path: String.raw`\\.\pipe\osuna-managed-test`,
    });
    expect(parseListenString(`pipe://${String.raw`\\.\pipe\osuna-managed-test`}`)).toEqual({
      type: "pipe",
      path: String.raw`\\.\pipe\osuna-managed-test`,
    });
  });
  ...
  test.skipIf(isPlatform("win32"))(
    ...
    async () => {
      const osunaHomeRoot = await mkdtemp(path.join(os.tmpdir(), "osuna-socket-relay-"));
      const osunaHome = path.join(osunaHomeRoot, ".osuna");
      const staticDir = await mkdtemp(path.join(os.tmpdir(), "osuna-static-"));
      const socketPath = path.join(osunaHomeRoot, "run", "osuna.sock");
...
async function beginDaemonShutdownWithAgentClosing(): Promise<BlockedDaemonShutdown> {
  ...
  const agentCwd = await mkdtemp(path.join(os.tmpdir(), "osuna-shutdown-agent-"));
  ...
  return {
    ...
    async finish() {
      ...
      await Promise.all([
        rm(path.dirname(daemonHandle.osunaHome), { recursive: true, force: true }),
