...
const localModelsDir =
  process.env.OSUNA_LOCAL_MODELS_DIR ?? path.join(homedir(), ".osuna", "models", "local-speech");
  ...
  try {
    ...
    expect(result).toEqual({
      ...
      file: {
        ...
        mimeType: "text/plain",
        size: 11,
        path: path.join(daemon.osunaHome, "uploads", "upload_req-upload-e2e", "notes.txt"),
      },
      error: null,
    });
...
test("returns home-scoped directory suggestions", async () => {
  const insideHomeDir = mkdtempSync(path.join(homedir(), "osuna-dir-suggestion-"));
  const rootBrowseDir = mkdtempSync(path.join(homedir(), "000-osuna-root-browse-"));
  const outsideHomeDir = mkdtempSync(path.join(tmpdir(), "osuna-dir-suggestion-outside-"));
...
test("returns typed relative suggestions within a requested directory", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "osuna-workspace-suggestion-"));
...
test("finds workspace files inside the OpenCode directory", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "osuna-opencode-suggestion-"));
...
test("opens an exact gitignored workspace path without offering it as a suggestion", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "osuna-gitignored-suggestion-"));
...
test("a Desktop-managed daemon does not advertise npm self-update", async () => {
  const daemon = await createTestOsunaDaemon({ desktopManaged: true });
  const client = new DaemonClient({
    url: `ws://127.0.0.1:${daemon.port}/ws`,
