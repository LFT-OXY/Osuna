...
interface RegisteredTool {
  ...
  handler: (args: unknown, context: OsunaToolExecutionContext) => Promise<OsunaToolResult>;
}
...
class BrowserToolHarness {
  ...
  public async execute(name: string, input: unknown): Promise<OsunaToolResult> {
...
function schemaFor(inputSchema: OsunaToolConfig["inputSchema"]): z.ZodType {
  if (!inputSchema) {
...
const routedToolCases = [
  ...
  content: OsunaToolResult["content"];
}>;
...
describe("registerBrowserTools", () => {
  ...
  test("list tabs sends workspace in the request envelope", async () => {
    ...
    expect(response.content).toEqual([
      {
        type: "text",
        text: `Found 1 Osuna browser tab. Use these browserId values for tab-scoped browser tools.\n- browserId=${BROWSER_ID} active title="Example" url=https://example.com`,
      },
    ]);
  });
