...
async function createPlugin(id: string, source: string): Promise<string> {
  const directory = await mkdtemp(path.join(tmpdir(), "osuna-plugin-service-"));
...
function createPausedRuntime() {
  ...
  const runtime: TestPluginRuntime = {
    ...
    subscribe: () => () => undefined,
    bindOsunaSessionHost: () => undefined,
  };
...
describe("PluginService", () => {
  it("resolves a provider icon path to sanitized inline SVG", async () => {
    const home = await mkdtemp(path.join(tmpdir(), "osuna-plugin-home-"));
    roots.push(home);
  ...
  it("keeps the running commit when a Git update build command fails", async () => {
    const home = await mkdtemp(path.join(tmpdir(), "osuna-plugin-home-"));
    roots.push(home);
    const repository = await mkdtemp(path.join(tmpdir(), "osuna-plugin-repository-"));
    roots.push(repository);
