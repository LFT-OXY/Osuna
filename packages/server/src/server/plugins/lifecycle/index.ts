...
import { z } from "zod";
import { CreateAgentRequestMessageSchema } from "@osuna/protocol/messages";
import type {
  PluginHookAgent,
  PluginHookContext,
  PluginLifecycleRegistration,
} from "@osuna/plugin/server";
...
import { WorkspaceCreateRequestSchema } from "@osuna/protocol/messages";
      ...
      if (kind === "before") {
        if (
          !beforeHookNames.includes(
            name as keyof import("@osuna/plugin/server").PluginBeforeRequests,
          )
        ) {
          throw new Error(`Unknown before hook: ${name}`);
        }
        const hookName = name as keyof import("@osuna/plugin/server").PluginBeforeRequests;
