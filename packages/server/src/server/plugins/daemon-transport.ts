import type { DaemonTransportFactory } from "@osuna/client/internal/daemon-client-transport-types";
...
export function createPluginDaemonTransportFactory(
  ...
  return () => {
    ...
    const unsubscribe = port.onMessage((message) => {
      ...
      if (message.type === "osuna_close") closeTransport();
    });
    ...
    return {
      send(data) {
        ...
        port.send({ type: "osuna_frame", data: frame, isBinary: typeof frame !== "string" });
      },
      close() {
        if (!isOpen) return;
        port.send({ type: "osuna_close" });
        closeTransport();
      },
      onMessage(handler) {
