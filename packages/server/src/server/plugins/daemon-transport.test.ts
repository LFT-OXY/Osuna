...
describe("plugin daemon IPC transport", () => {
  it("preserves incoming text and binary frame order", async () => {
    ...
    await opened;
    receive?.({ type: "osuna_frame", data: "first", isBinary: false });
    receive?.({ type: "osuna_frame", data: new Uint8Array([7, 8]), isBinary: true });
    ...
    expect(sent).toEqual([{ type: "osuna_frame", data: "outbound", isBinary: false }]);
  });
});
