...
import { z } from "zod";
import { PluginIdSchema, type PluginSourceStatusItem } from "@osuna/protocol/messages";
...
export class ManagedPluginSources {
  ...
  constructor(osunaHome: string) {
    this.root = path.join(osunaHome, "plugins");
...
async function clone(remote: string, checkoutRoot: string): Promise<void> {
  ...
  const cloneRemote = publicRemote === remote ? remote : "https://osuna.invalid/plugin.git";
  const envOverlay =
    cloneRemote === remote
      ? GIT_ENV
      : {
