...
/**
 * Stable daemon identifier scoped to a given $OSUNA_HOME.
 *
 * - Persisted to `$OSUNA_HOME/server-id`
  ...
  if (envOverride) {
    ...
    if (!existsSync(serverIdPath)) {
      try {
        ...
        log?.info({ serverId: envOverride }, "Persisted OSUNA_SERVER_ID override");
      } catch (error) {
        log?.warn({ error }, "Failed to persist OSUNA_SERVER_ID override");
      }
    } else {
