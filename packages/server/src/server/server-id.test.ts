...
function tmpHome(): string {
  return mkdtempSync(path.join(tmpdir(), "osuna-server-id-"));
}
...
describe("getOrCreateServerId", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.OSUNA_SERVER_ID;
    home = tmpHome();
  });
  ...
  it("creates and persists a stable id per OSUNA_HOME", () => {
  ...
  it("respects and persists OSUNA_SERVER_ID override", () => {
    process.env.OSUNA_SERVER_ID = "test-daemon-id";
