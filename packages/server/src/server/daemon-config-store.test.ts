...
describe("DaemonConfigStore", () => {
  ...
  test("patch persists relay state and emits its field change", () => {
    const osunaHome = mkdtempSync(path.join(tmpdir(), "osuna-daemon-config-store-"));
    tempDirs.push(osunaHome);
    const store = new DaemonConfigStore(osunaHome, {
      relay: { enabled: false },
    ...
    expect(loadPersistedConfig(osunaHome).daemon?.relay?.enabled).toBe(true);
  });
  ...
  test("patch round-trips agent profiles through the strictly-parsed persisted config", () => {
    ...
    expect(loadPersistedConfig(osunaHome).daemon?.agentProfiles).toEqual([
      {
        id: "profile_ui",
        name: "UI work",
        icon: "🎨",
  ...
  test("patch replaces the whole agent profile list rather than merging entries", () => {
    ...
    expect(loadPersistedConfig(osunaHome).daemon?.agentProfiles).toHaveLength(1);
  });
  ...
  test("rolls back config when a field transition fails", () => {
    ...
    expect(loadPersistedConfig(osunaHome).daemon?.relay?.enabled).toBe(false);
  });
  ...
  test("rolls back live owners when a later transactional owner fails", () => {
    ...
    expect(loadPersistedConfig(osunaHome).daemon?.browserTools?.enabled).toBeUndefined();
  });
  ...
  test("unrelated patches persist only requested file intent", () => {
    const osunaHome = mkdtempSync(path.join(tmpdir(), "osuna-daemon-config-store-"));
    tempDirs.push(osunaHome);
    const before = loadPersistedConfig(osunaHome);
    const store = new DaemonConfigStore(
      osunaHome,
      {
        relay: { enabled: true },
    ...
    expect(loadPersistedConfig(osunaHome)).toEqual({
      ...before,
  ...
  test("patch persists provider enabled flags into config.json", () => {
    ...
    const initial = loadPersistedConfig(osunaHome);
  ...
  test("patch persists provider Osuna-tool policy without changing availability", () => {
    const osunaHome = mkdtempSync(path.join(tmpdir(), "osuna-daemon-config-store-"));
    tempDirs.push(osunaHome);
    writeFileSync(
      path.join(osunaHome, "config.json"),
    ...
    expect(loadPersistedConfig(osunaHome).agents?.providers?.claude).toEqual({
      enabled: false,
      osunaTools: { enabled: true, disabledTools: ["create_agent"] },
    });
  });
  ...
  test("patch persists custom ACP provider overrides into config.json", () => {
    ...
    const persisted = loadPersistedConfig(osunaHome);
    expect(persisted.agents?.providers?.["osuna-e2e-acp"]).toEqual({
      extends: "acp",
      label: "Osuna E2E ACP",
      description: "E2E ACP provider fixture",
      command: ["npx", "-y", "--version"],
...
describe("DaemonConfigStore reload", () => {
  ...
  function createReloadableStore(
    ...
    const osunaHome = mkdtempSync(path.join(tmpdir(), "osuna-daemon-config-reload-"));
    tempDirs.push(osunaHome);
    ...
    const store = new DaemonConfigStore(osunaHome, initialMutable, undefined, {
      reloadSource: {
    ...
    return { osunaHome, store, persisted };
  }

  function writeConfig(osunaHome: string, config: unknown): void {
    writeFileSync(path.join(osunaHome, "config.json"), `${JSON.stringify(config, null, 2)}\n`);
  }

  test("applies mutable edits and reports startup-only edits", () => {
    const { osunaHome, store, persisted } = createReloadableStore();
    writeConfig(osunaHome, {
      ...persisted,
      daemon: {
        ...persisted.daemon,
  ...
  test("applies the global plugin switch in both directions", () => {
    ...
    writeConfig(osunaHome, { ...persisted, pluginsEnabled: true });
    expect(store.reload()).toEqual({
    ...
    writeConfig(osunaHome, { ...persisted, pluginsEnabled: false });
    expect(store.reload()).toEqual({
      appliedPaths: ["pluginsEnabled"],
      restartRequiredPaths: [],
      overrideControlledPaths: [],
  ...
  test("classifies every leaf when a parent subtree is added", () => {
    const { osunaHome, store } = createReloadableStore({
  ...
  test("classifies every leaf when the daemon subtree is removed", () => {
    ...
    writeConfig(osunaHome, { version: 1 });
  ...
  test("invalid JSON and invalid schema apply nothing", () => {
    const { osunaHome, store } = createReloadableStore();
    writeFileSync(path.join(osunaHome, "config.json"), "{ nope\n");
    expect(() => store.reload()).toThrow("Invalid JSON");
    expect(store.get().browserTools.enabled).toBe(false);

    writeConfig(osunaHome, { daemon: { browserTools: { enabled: "yes" } } });
  ...
  test("removing providers and optional profiles clears live state", () => {
    ...
    writeConfig(osunaHome, persisted);
