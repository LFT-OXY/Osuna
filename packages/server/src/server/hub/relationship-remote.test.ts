...
afterEach(async () => {
  ...
  await Promise.all(openOsunaHomes.splice(0).map((home) => rm(home, { recursive: true })));
});
...
test.each([
  ...
  async ({ acknowledge, expected }) => {
    ...
    if (acknowledge) {
      webSockets.on("headers", (headers) => headers.push("x-osuna-session-protocol: 1"));
    }
    let offeredProtocol: string | string[] | undefined;
    server.on("upgrade", (request, socket, head) => {
      offeredProtocol = request.headers["x-osuna-session-protocol"];
...
async function connectController(
  ...
  const osunaHome = await mkdtemp(path.join(tmpdir(), "osuna-hub-socket-"));
  openOsunaHomes.push(osunaHome);
