...
export class DaemonExecutions implements HubExecutionAgents {
  ...
  private async createOrResolve(
    ...
    let createdWorktree: CreateOsunaWorktreeWorkflowResult | null = null;
...
function requireHubMcpNamespace(mcpServers: Record<string, McpServerConfig> | undefined): void {
  if (mcpServers && Object.hasOwn(mcpServers, "osuna")) {
    throw new Error('Hub execution MCP server name "osuna" is reserved by the daemon');
  }
}
...
function ownedCreatedWorktree(
  worktree: CreateOsunaWorktreeWorkflowResult | null,
): CreateOsunaWorktreeWorkflowResult | null {
