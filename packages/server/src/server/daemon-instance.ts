  ...
  while (true) {
    ...
    if (!instance)
      throw new DaemonInstanceError(
        "DAEMON_NOT_RUNNING",
        `Daemon is not running for ${home}. Start with: osuna daemon start --home ${JSON.stringify(home)}`,
      );
...
function notReady(home: string, instance: PidLockInfo): DaemonInstanceError {
  return new DaemonInstanceError(
    "DAEMON_NOT_READY",
    `Daemon PID ${instance.pid} remains running but is not ready for ${home}.\nLogs: ${daemonLogPath(home)}\nStatus: osuna daemon status --home ${JSON.stringify(home)}\nStop: osuna daemon stop --home ${JSON.stringify(home)}`,
  );
}
  ...
  async function waitUntilReady() {
    while (true) {
      ...
      if (Date.now() >= deadline) {
        ...
        throw new DaemonInstanceError(
          "DAEMON_NOT_READY",
          `Supervisor PID ${child.pid} remains running but has not published its lock for ${input.home}. Logs: ${daemonLogPath(input.home)}. Check osuna daemon status --home ${JSON.stringify(input.home)}. Stop with osuna daemon stop --home ${JSON.stringify(input.home)} once its lock is published, or signal this PID.`,
        );
      }
