...
async function createHarness(): Promise<{
  client: DaemonClient;
  daemon: Awaited<ReturnType<typeof createTestOsunaDaemon>>;
}> {
  ...
  test("custom agents defined in opencode.json appear in availableModes", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        agent: {
          "osuna-e2e-custom": {
            description: "Custom agent for Osuna daemon E2E test",
            mode: "primary",
          },
        },
      }),
    ...
    try {
      ...
      const custom = snapshot.availableModes.find((m) => m.id === "osuna-e2e-custom");
      expect(custom).toBeDefined();
      expect(custom!.description).toBe("Custom agent for Osuna daemon E2E test");
