...
import type { PullRequestMergeable } from "@osuna/protocol/messages";
  ...
  testWithGitHubCliAuth(
    ...
    async () => {
      ...
      try {
        ...
        const archiveResult = await ctx.client.archiveOsunaWorktree({
          worktreePath: worktree.worktreePath,
        });
        ...
        if (worktreePath) {
          await ctx.client.archiveOsunaWorktree({ worktreePath }).catch(() => undefined);
        }
        if (agentId) {
