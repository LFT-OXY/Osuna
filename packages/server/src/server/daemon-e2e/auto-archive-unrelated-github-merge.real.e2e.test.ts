...
githubTest(
  ...
  async () => {
    const repository = realpathSync(mkdtempSync(path.join(tmpdir(), "osuna-github-merge-")));
    cleanupPaths.add(repository);
    run(repository, "git", ["init", "-b", "main"]);
    run(repository, "git", ["config", "user.email", "osuna-test@example.com"]);
    run(repository, "git", ["config", "user.name", "Osuna Test"]);
    ...
    const merged = await createWorktree({
      ...
      runSetup: false,
      osunaHome: context.daemon.osunaHome,
    });
