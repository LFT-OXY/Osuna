...
test("openProject preserves a worktree's exact-root project without rehoming it", async () => {
  ...
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "osuna-open-project-repo-")));
    const worktreeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "osuna-open-project-worktree-")),
    );
    const osunaHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "osuna-open-project-home-")),
    );
    ...
    const workspacesPath = path.join(osunaHome, "projects", "workspaces.json");
