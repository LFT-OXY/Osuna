...
export async function createOpenCodeOmoRealRuntime(): Promise<OpenCodeOmoRealRuntime> {
  ...
  const openCodeVersion =
    process.env.OSUNA_REAL_OPENCODE_VERSION?.trim() || PINNED_OPENCODE_VERSION;
  const omoVersion = process.env.OSUNA_REAL_OMO_VERSION?.trim() || PINNED_OMO_VERSION;
  ...
  if (model === NO_AUTH_MODEL) {
    await runCommand({
      command: openCodeCommand,
      args: ["run", "--model", model, "Reply with exactly: OSUNA_BIG_PICKLE_PROBE_OK"],
      cwd: paths.workspace,
      env: runtimeEnv,
      artifactName: "big-pickle-probe.log",
      artifacts: paths.artifacts,
      secrets,
      requiredOutput: "OSUNA_BIG_PICKLE_PROBE_OK",
    });
  }
...
function resolveModel(openRouterApiKey: string | null): string {
  const explicitModel = process.env.OSUNA_REAL_OPENCODE_MODEL?.trim();
  if (explicitModel) {
