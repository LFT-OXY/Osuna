...
import type { TerminalCell, TerminalState } from "@osuna/protocol/messages";
import { renderTerminalSnapshotToAnsi } from "@osuna/protocol/terminal-snapshot";
import type { TerminalStreamEvent } from "@osuna/client/internal/daemon-client";
...
async function createPackageLockTerminalCwd(): Promise<PackageLockTerminalCwd> {
  const cwd = await mkdtemp(path.join(tmpdir(), "osuna-byte-package-lock-"));
  return {
    path: cwd,
