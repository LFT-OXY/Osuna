...
test("an empty project becomes Git without changing its identity or creating a workspace", async () => {
  const projectRoot = realpathSync(
    mkdtempSync(path.join(os.tmpdir(), "osuna-project-becomes-git-")),
  );
  const osunaHomeRoot = realpathSync(
    mkdtempSync(path.join(os.tmpdir(), "osuna-project-becomes-git-home-")),
  );
  ...
  const persistedProjects = JSON.parse(
    await readFile(path.join(daemon.osunaHome, "projects", "projects.json"), "utf8"),
