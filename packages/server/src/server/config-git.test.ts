...
async function createHome(config: object = {}): Promise<string> {
  const home = await mkdtemp(path.join(os.tmpdir(), "osuna-config-git-"));
  roots.push(home);
...
describe("daemon Git process config", () => {
  ...
  test("new environment variables override config.json", async () => {
    ...
    expect(
      loadConfig(home, {
        env: {
          OSUNA_GIT_MAX_PROCESSES_PER_SECOND: "12",
          OSUNA_GIT_MAX_PROCESS_CONCURRENCY: "6",
        },
      }).git,
    ).toEqual({
  ...
  test("accepts legacy OSUNA_GIT_CONCURRENCY below the renamed variable", async () => {
    const home = await createHome();

    expect(
      loadConfig(home, {
        env: {
          OSUNA_GIT_CONCURRENCY: "3",
        },
    ...
    expect(
      loadConfig(home, {
        env: {
          OSUNA_GIT_CONCURRENCY: "3",
          OSUNA_GIT_MAX_PROCESS_CONCURRENCY: "7",
        },
      }).git?.maxProcessConcurrency,
    ).toBe(7);
  });
