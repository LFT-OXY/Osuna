...
describe("STTManager", () => {
  ...
  it("uses OSUNA_VOICE_LANGUAGE over OSUNA_DICTATION_LANGUAGE", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
        OSUNA_VOICE_LANGUAGE: "pt",
        OSUNA_DICTATION_LANGUAGE: "es",
  ...
  it("uses OSUNA_DICTATION_LANGUAGE when OSUNA_VOICE_LANGUAGE is unset", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
  ...
  it("uses streaming segmentation for batch transcription and concatenates segment finals", async () => {
    const original = process.env.OSUNA_STT_BATCH_COMMIT_EVERY_SECONDS;
    process.env.OSUNA_STT_BATCH_COMMIT_EVERY_SECONDS = "1";
    ...
    try {
      ...
      if (original === undefined) {
        delete process.env.OSUNA_STT_BATCH_COMMIT_EVERY_SECONDS;
      } else {
        process.env.OSUNA_STT_BATCH_COMMIT_EVERY_SECONDS = original;
      }
    }
  });
});
