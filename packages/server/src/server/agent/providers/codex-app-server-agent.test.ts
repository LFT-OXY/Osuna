...
async function startPublicSteeringSession(
...
): Promise<{ session: AgentSession; osunaTurnId: string }> {
  const session = new CodexAppServerAgentSession(
  ...
  return { session, osunaTurnId: started.turnId };
}
...
describe("Codex active-turn steering admission", () => {
  test("a steer without the clearing contract leaves permissions open", async () => {
    ...
    const { session, osunaTurnId } = await startPublicSteeringSession(appServer);
    castInternals<{ emitSyntheticPlanApprovalRequest: (planText: string) => void }>(
      session,
    ).emitSyntheticPlanApprovalRequest("Ship the thing");

    await expect(
      session.steerActiveTurn!("background notification", { expectedTurnId: osunaTurnId }),
  ...
  test("does not steer B when A completes while command resolution is pending", async () => {
    ...
    const appServer = createFakeCodexAppServer();
    const { session, osunaTurnId } = await startPublicSteeringSession(appServer, async (prompt) => {
    ...
    expect(startedB.turnId).not.toBe(osunaTurnId);
...
const capturePath = process.env.OSUNA_FAKE_CODEX_CAPTURE;
let buffer = "";
    ...
    providerOverrides: {
      [providerId]: {
        ...
        env: {
          ...
          OSUNA_FAKE_CODEX_CAPTURE: capturedRequestsPath,
        },
      },
    },
  });
...
describe("Codex app-server provider", () => {
  ...
  test("initializes Codex app-server without making Osuna the request originator", async () => {
  ...
  test("unarchives Codex when an active Osuna agent resumes an archived thread", async () => {
    const threadRequests: string[] = [];
  ...
  test("resolves Codex skill slash commands into app-server skill input", async () => {
          ...
          data: [
            {
              ...
              skills: [
                {
                  name: "osuna-implement",
                  description: "Execute an existing Osuna plan.",
                  path: "/tmp/skills/osuna-implement/SKILL.md",
                },
              ],
              errors: [],
            },
    ...
    await session.startTurn("/osuna-implement in a worktree, remember to use Claude for the UI");
    ...
    expect(turnStartCall?.[1]).toEqual(
      expect.objectContaining({
        input: [
          ...
          {
            type: "text",
            text: "$osuna-implement in a worktree, remember to use Claude for the UI",
            text_elements: [],
          },
        ],
      }),
    ...
    const commands = await listCommandsFromFakeCodex([
      {
        ...
        path: "/Users/test/.agents/skills/osuna/SKILL.md",
      },
      {
        name: "osuna",
        description: "Shared orchestration skill.",
        path: "/Users/test/.codex/skills/osuna/SKILL.md",
      },
    ]);

    expect(commands.filter((command) => command.name === "osuna")).toEqual([
      {
        name: "osuna",
        description: "Shared orchestration skill.",
        argumentHint: "",
    ...
    const input = await codexAppServerTurnInputFromPrompt(
      [
        { type: "text", text: "Please review this" },
        {
          ...
          url: "https://github.com/lft-oxy/osuna/issues/456",
        },
      ],
      logger,
    );
  ...
  test("builds app-server env from launch-context env overrides", () => {
    const launchContext: AgentLaunchContext = {
      env: {
        OSUNA_AGENT_ID: "00000000-0000-4000-8000-000000000301",
        OSUNA_TEST_FLAG: "codex-launch-value",
      },
    };
    const env = buildCodexAppServerEnv(
      {
        env: {
          OSUNA_AGENT_ID: "runtime-value",
          OSUNA_TEST_FLAG: "runtime-test-value",
        },
      },
      launchContext.env,
    );

    expect(env.OSUNA_AGENT_ID).toBe(launchContext.env?.OSUNA_AGENT_ID);
    expect(env.OSUNA_TEST_FLAG).toBe(launchContext.env?.OSUNA_TEST_FLAG);
  });
    ...
    expect(events).toEqual([
      {
        ...
        item: {
          type: "assistant_message",
          text: "![Image](file:///tmp/osuna%20image.png)",
        },
      },
    ]);
  });
  ...
  test("materializes imageGeneration base64 results before rendering markdown", () => {
    ...
    expect(source).toMatch(/osuna-attachments(?:-[^\\/]+)?[\\/].+\.png$/);
    expect(existsSync(source)).toBe(true);
    rmSync(source, { force: true });
  });
