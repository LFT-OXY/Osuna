...
const ROOT_PROMPT = `You are ROOT_OWNER. Use Claude Code's native Agent tool exactly once, never Osuna tools.
...
test("attributes a nested Claude child and its background notification to their direct owners", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "osuna-claude-nested-ownership-"));
