...
test("logs redacted query summary and never leaks sentinel secrets", async () => {
  const envSecret = "OSUNA_ENV_SENTINEL_SECRET";
  const runtimeSecret = "OSUNA_RUNTIME_SENTINEL_SECRET";
  const systemSecret = "OSUNA_SYSTEM_PROMPT_SENTINEL_SECRET";
  const previousEnv = process.env.OSUNA_TEST_SENTINEL_SECRET;
  process.env.OSUNA_TEST_SENTINEL_SECRET = envSecret;
  ...
  const client = new ClaudeAgentClient({
    ...
    runtimeSettings: {
      env: {
        OSUNA_RUNTIME_SENTINEL_SECRET: runtimeSecret,
      },
    },
    resolveBinary: async () => "/test/claude/bin",
  });
  ...
  try {
    ...
    if (previousEnv === undefined) {
      delete process.env.OSUNA_TEST_SENTINEL_SECRET;
    } else {
      process.env.OSUNA_TEST_SENTINEL_SECRET = previousEnv;
    }
  }
});
