...
describe("Claude SDK env", () => {
  test("forwards launch-context env through Claude process env", async () => {
    ...
    const launchContext: AgentLaunchContext = {
      env: {
        OSUNA_AGENT_ID: "00000000-0000-4000-8000-000000000201",
        OSUNA_TEST_FLAG: "launch-value",
      },
    };
    ...
    try {
      ...
      expect(capturedEnv?.OSUNA_AGENT_ID).toBe(launchContext.env?.OSUNA_AGENT_ID);
      expect(capturedEnv?.OSUNA_TEST_FLAG).toBe(launchContext.env?.OSUNA_TEST_FLAG);
  ...
  test("forwards launch-context env through Claude resume env", async () => {
    ...
    const launchContext: AgentLaunchContext = {
      env: {
        OSUNA_AGENT_ID: "00000000-0000-4000-8000-000000000202",
        OSUNA_TEST_FLAG: "resume-launch-value",
      },
    };
    const queryFactory = vi.fn(({ options }: ClaudeQueryInput) => {
      capturedEnv = options.env;
