...
export class OpenCodeBridge {
  ...
  private manifestCatalog: OsunaToolCatalog | null = null;
  ...
  setManifestCatalog(catalog: OsunaToolCatalog | null): void {
  ...
  decorateServerEnv(env: Record<string, string>): Record<string, string> {
    ...
    const withoutBridge = plugins.filter((entry) => {
      ...
      return !specifier.includes("/osuna-") || !specifier.endsWith(".mjs");
    });
    return {
      ...env,
  ...
  private async materializePlugin(): Promise<string> {
    ...
    const destination = path.join(this.osunaHome, "runtime", "opencode", `osuna-${digest}.mjs`);
  ...
  private async route(request: IncomingMessage, response: ServerResponse): Promise<void> {
    try {
      ...
      if (request.method === "GET" && contextMatch) {
        ...
        if (!binding) {
          sendJson(response, 404, { error: "OpenCode session is not bound to an Osuna agent" });
          return;
        }
  ...
  private serializeManifest(): Array<Record<string, unknown>> {
    ...
    return [...catalog.tools.values()].map((tool) => {
      const definition: Record<string, unknown> = {
        ...
        inputSchema: serializeOsunaToolInputParameters(tool),
      };
  ...
  private async executeTool(input: {
    ...
    if (!binding) {
      sendJson(input.response, 404, { error: "OpenCode session is not bound to an Osuna agent" });
      return;
    }
    if (!binding.tools) {
      sendJson(input.response, 403, { error: "Osuna tools are disabled for this session" });
      return;
    }
