...
// CLI identity instead of showing up as Osuna in provider usage logs.
...
// Osuna v0.1.105; remove after 2027-01-09 once the supported Codex floor is >=0.143.
  ...
  private async ensureThreadLoaded(): Promise<void> {
    ...
    try {
      ...
      if (isArchivedCodexThreadResumeError(error, threadId)) {
        ...
        const response = await this.client.request("thread/resume", params);
        this.rememberResolvedSandboxPolicy(response);
        this.logger.info({ threadId }, "Unarchived Codex thread to restore active Osuna agent");
        return;
      }
