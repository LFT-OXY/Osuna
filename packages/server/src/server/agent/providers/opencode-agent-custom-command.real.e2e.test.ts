...
describe("opencode custom command Big Pickle E2E (real)", () => {
  ...
  test("executes a global custom command through Osuna using Big Pickle", async () => {
    ...
    const projectDir = await mkdtemp(path.join(tmpdir(), "osuna-opencode-big-pickle-"));
    ...
    try {
      ...
      await writeFile(
        ...
        [
          "---",
          "description: Osuna issue 903 Big Pickle custom command",
          "agent: build",
          "---",
          "",
