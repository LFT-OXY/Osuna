...
describe("codex agent commands E2E", () => {
  ...
  test("lists available slash commands for a codex agent", async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "---",
        "",
        "Say hello to $NAME and then output exactly OSUNA_OK.",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    writeFileSync(
      ...
      [
        ...
        "---",
        "",
        "When invoked, respond with exactly OSUNA_SKILL_OK.",
        "",
      ].join("\n"),
      "utf8",
    );
  ...
  test("executes a custom prompt via normal sendMessage path (prompts:*)", async () => {
    ...
    expect(state.lastMessage).toContain("OSUNA_OK");
  ...
  test("sendMessage keeps unknown slash input as plain prompt text", async () => {
    ...
    expect(state.lastMessage).not.toContain("OSUNA_SKILL_OK");
  }, 30_000);
