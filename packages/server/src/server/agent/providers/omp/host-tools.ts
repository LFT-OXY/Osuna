...
import {
  ...
  serializeOsunaToolInputParameters,
} from "../../tools/osuna-tool-serialization.js";
import type { OsunaToolCatalog, OsunaToolResult } from "../../tools/types.js";
...
interface OmpHostToolRouterInput {
  ...
  catalog: OsunaToolCatalog;
  logger: Logger;
}

const routersByRuntimeSession = new WeakMap<OmpRuntimeSession, OmpHostToolRouter>();

export function serializeOmpHostTools(catalog: OsunaToolCatalog): OmpRpcHostToolDefinition[] {
  return [...catalog.tools.values()].map((tool) => {
    const definition: OmpRpcHostToolDefinition = {
      name: tool.name,
      description: tool.description,
      loadMode: "essential",
      parameters: serializeOsunaToolInputParameters(tool),
    };
    if (tool.title) {
...
function sendMissingCatalogResult(
  ...
  runtimeSession.sendHostToolResult(
    toOmpHostToolErrorResult(
      request.id,
      `Host tool "${request.toolName}" was called before Osuna tools were registered`,
    ),
  );
}
...
class OmpHostToolRouter {
  ...
  private sendUpdate(callId: string, result: OsunaToolResult): void {
...
function toOmpHostToolResult(id: string, result: OsunaToolResult): OmpRpcHostToolResult {
...
function toOmpAgentToolResult(result: OsunaToolResult): OmpAgentToolResult {
