...
function createToolCatalog(): OsunaToolCatalog {
  return {
    tools: new Map([
      [
        "create_agent",
...
describe("OMP agent client and session", () => {
  ...
  test("resumes an OMP session and replays its history", async () => {
    ...
    expect(omp.launchConfiguration()).toEqual({
      ...
      protocolMode: "rpc-ui",
      modeId: "ask",
      session: expect.stringMatching(/[\\/]osuna-omp-resume-.*[\\/]session\.jsonl$/),
      argv: [
        "omp",
        "--mode",
        "rpc-ui",
        ...
        "high",
        "--session",
        expect.stringMatching(/[\\/]osuna-omp-resume-.*[\\/]session\.jsonl$/),
      ],
    });
