...
export class OmpAgentSession implements AgentSession {
  ...
  constructor(options: OmpAgentSessionOptions) {
    ...
    this.osunaTools = options.osunaTools;
  ...
  private readonly osunaTools?: OsunaToolCatalog;
  ...
  private handleSessionEvent(event: OmpAgentSessionEvent): void {
    ...
    switch (event.type) {
      ...
      case "agent_end": {
        ...
        // so do not advertise Osuna idle until it reports that transition.
...
export class OmpAgentClient implements AgentClient {
  ...
  private async configureNativeOsunaTools(
    runtimeSession: OmpRuntimeSession,
    catalog: OsunaToolCatalog | undefined,
  ): Promise<void> {
    if (!catalog) {
      return;
    }
  ...
  async createSession(
    ...
    try {
      await this.configureNativeOsunaTools(runtimeSession, launchContext?.osunaTools);
      return new OmpAgentSession({
        ...
        osunaTools: launchContext?.osunaTools,
      });
    } catch (error) {
