...
import type { OsunaToolCatalog, OsunaToolDefinition, OsunaToolResult } from "../../tools/types.js";
import {
...
function createCatalog(tools: OsunaToolDefinition[]): OsunaToolCatalog {
...
class OmpHostToolHarness {
  ...
  private resolveControlledResult: ((result: OsunaToolResult) => void) | null = null;
  ...
  private constructor(
    private readonly catalog: OsunaToolCatalog,
    private readonly runtimeSession: Awaited<ReturnType<FakeOmp["startSession"]>>,
  ) {}

  static async withTools(tools: OsunaToolDefinition[]): Promise<OmpHostToolHarness> {
  ...
  static async cancellable(): Promise<OmpHostToolHarness> {
    ...
    const tool: OsunaToolDefinition = {
      name: "wait_for_agent",
      description: "Wait for an Osuna agent.",
      handler: async (_input, context) => {
        harness.controlledSignal = context.signal ?? null;
        harness.resolveControlledStart?.();
        return await new Promise<OsunaToolResult>((resolve) => {
  ...
  completeControlledCall(result: OsunaToolResult): void {
  ...
  private routerInput() {
    return { runtimeSession: this.runtimeSession, osunaTools: this.catalog, logger: this.logger };
  }
}

describe("OMP host tools", () => {
  test("marks every caller-scoped Osuna tool essential for direct invocation", () => {
    const catalog = createCatalog([
      {
        name: "create_agent",
        title: "Create agent",
        description: "Create an Osuna agent.",
