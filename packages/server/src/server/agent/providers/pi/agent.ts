...
function createPiPaseoExtensionFile(systemPrompt?: string): PiTempFile {
  ...
  writeFileSync(
	...
	function emitCommandResult(ctx, requestId, result) {
	  ctx.ui.notify(
	    "${OSUNA_PI_COMMAND_RESULT_MARKER} " + JSON.stringify({ requestId, ...result }),
	...
	export default function paseoIntegration(pi) {
	  ...
	  function emitSubmittedUserEntries(ctx) {
	    ...
	    for (let index = 0; index < submittedUserMessages.length; index += 1) {
	      ...
	      index -= 1;
	      ctx.ui.notify(
	        "${OSUNA_PI_SUBMITTED_USER_ENTRY_MARKER} " +
...
export class PiRpcAgentSession implements AgentSession {
  ...
  private async runPiTreeExtensionCommand(targetId: string): Promise<unknown> {
    ...
    await this.runtimeSession.prompt(`/${OSUNA_PI_TREE_EXTENSION_COMMAND} ${payload}`);
  ...
  private async requestEntryCapture(reason: string): Promise<void> {
    ...
    await this.runtimeSession.prompt(`/${OSUNA_PI_CAPTURE_EXTENSION_COMMAND} ${payload}`);
    await resultPromise;
  }
  ...
  private handleSubmittedUserEntryMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, OSUNA_PI_SUBMITTED_USER_ENTRY_MARKER);
    if (!payload) {
      return false;
    }
  ...
  private handleEntryCaptureMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, OSUNA_PI_ENTRY_CAPTURE_MARKER);
    if (!payload) {
      return false;
    }
  ...
  private handleCommandResultMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, OSUNA_PI_COMMAND_RESULT_MARKER);
    if (!payload) {
      return false;
    }
    if (typeof payload.requestId !== "string") {
