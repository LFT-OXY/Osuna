...
type OsunaExtensionListener = (event: unknown, context?: unknown) => unknown;

async function loadOsunaExtensionListeners(
  extensionPath: string,
): Promise<Map<string, OsunaExtensionListener>> {
  const listeners = new Map<string, OsunaExtensionListener>();
  const extension = (await import(pathToFileURL(extensionPath).href)) as {
    default: (piApi: {
      on: (event: string, listener: OsunaExtensionListener) => void;
...
async function applyOsunaExtensionSystemPrompt(
  extensionPath: string,
  systemPrompt: string,
): Promise<string | undefined> {
  const listeners = await loadOsunaExtensionListeners(extensionPath);
...
describe("PiRpcAgentSession", () => {
  ...
  test("reports the persisted Pi entry attached to the submitted message", async () => {
    ...
    expect(notifications).toEqual([
      'OSUNA_SUBMITTED_USER_ENTRY {"entry":{"id":"entry-new","parentId":"entry-old-assistant","text":"new prompt"}}',
    ]);
  ...
  test("appends agent and daemon prompts after Pi's discovered system prompt", async () => {
    ...
    await expect(
      applyOsunaExtensionSystemPrompt(actualLaunch.extensionPaths[0]!, "Pi project prompt"),
  ...
  test("materializes image prompts as text hints for text-only Pi models", async () => {
    ...
    try {
      ...
      expect(imagePath).toMatch(
        /osuna-attachments(?:-[^\\/]+)?[\\/](?:[^\\/]+[\\/])?[0-9a-f]{64}\.png$/,
      );
...
describe("PiRpcAgentClient", () => {
  test("lists JSONL persisted sessions from configured provider params", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "osuna-pi-sessions-"));
  ...
  test("lists JSONL persisted sessions from Pi's configured agent directory", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "osuna-pi-default-sessions-"));
    const cwd = path.join(root, "workspace");
  ...
  test("imports JSONL sessions with the recorded model and thinking level", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "osuna-pi-import-config-"));
    const cwd = path.join(root, "workspace");
    const sessionsDir = path.join(root, "sessions");
    mkdirSync(sessionsDir, { recursive: true });
  ...
  test("maps extension, prompt, and skill commands to Osuna slash commands", async () => {
    const { pi, session } = await createSession();
  ...
  test("injects MCP servers without replacing the Pi global MCP config", async () => {
    const agentDir = mkdtempSync(path.join(tmpdir(), "osuna-pi-agent-"));
  ...
  test("reports the path of a malformed Pi global MCP config", async () => {
    ...
    await expect(
      client.createSession(
        createConfig({
          mcpServers: {
            osuna: { type: "http", url: "http://127.0.0.1:6767/mcp/agents" },
          },
        }),
        { env: { PI_CODING_AGENT_DIR: agentDir } },
      ),
