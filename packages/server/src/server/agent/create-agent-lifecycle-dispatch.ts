...
interface CreateAgentLifecycleDispatchDependencies {
  ...
  createOsunaWorktreeWorkflow: CreateOsunaWorktreeWorkflowFn;
...
type AutoArchiveTarget =
  | { kind: "agent-only" }
  | { kind: "created-worktree"; result: CreateOsunaWorktreeWorkflowResult };
...
export class CreateAgentLifecycleDispatch {
  ...
  }): Promise<CreateOsunaWorktreeWorkflowResult | null> {
  ...
  private async createWorktreeForTarget(
    ...
    const baseInput = {
      ...
      firstAgentContext,
      runSetup: false,
      osunaHome: this.dependencies.osunaHome,
      worktreesRoot: this.dependencies.worktreesRoot,
    } as const;

    switch (target.mode) {
      case "branch-off":
        return this.dependencies.createOsunaWorktreeWorkflow(
          {
            ...baseInput,
  ...
  private async archiveAutoCreatedWorktree(options: {
    ...
    if (!ownership.allowed) {
      throw new Error("Auto-created worktree is not an Osuna-owned worktree");
    }

    await archiveByScope(
      {
        osunaHome: this.dependencies.osunaHome,
        osunaWorktreesBaseRoot: this.dependencies.worktreesRoot,
