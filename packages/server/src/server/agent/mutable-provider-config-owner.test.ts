...
describe("mutable provider config owner", () => {
  test("publishes provider changes after commit and emits nothing on rollback", () => {
    const osunaHome = mkdtempSync(path.join(tmpdir(), "osuna-provider-config-owner-"));
    tempDirs.push(osunaHome);
    const store = new DaemonConfigStore(osunaHome, mutableConfig({ version: 1 }));
  ...
  test("does no provider work for unrelated CORS, Git, and app URL reloads", () => {
    ...
    const store = new DaemonConfigStore(osunaHome, mutableConfig(initial), undefined, {
