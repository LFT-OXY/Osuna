...
let tempRoot: string;
let daemonHandle: TestOsunaDaemon;
...
beforeAll(async () => {
  ...
  daemonHandle = await createTestOsunaDaemon({
...
describe("Suite A: Core Fixes", () => {
  ...
  test("agentManager.createAgent injects osuna MCP using the daemon listen target", async () => {
    let agentId: string | null = null;
    try {
      ...
      expect(snapshot.config.mcpServers?.osuna).toBeUndefined();

      const liveAgent = daemonHandle.daemon.agentManager.getAgent(agentId);
      expect(liveAgent?.config.mcpServers?.osuna).toBeUndefined();
    } finally {
      await archiveAgentIfPresent(agentId);
    }
  });
