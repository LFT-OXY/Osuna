...
import {
  ...
  type CreateOsunaWorktreeWorkflowFn,
...
import { MutableDaemonConfigSchema, type AgentProfile } from "@osuna/protocol/messages";
...
function configureOpenCodeProviderStub(
  ...
  const opencodeModes: AgentMode[] = [
    ...
    { id: "osuna-custom", label: "Osuna Custom", description: "Custom OpenCode agent" },
  ];
  ...
  const customOpenCodeModes: AgentMode[] = [
    ...opencodeModes,
    { id: "osuna-custom", label: "Osuna Custom" },
  ];
...
async function connectInMemoryMcpClient(server: Awaited<ReturnType<typeof createAgentMcpServer>>) {
  const client = new Client({ name: "osuna-test-client", version: "0.0.0" });
...
function createOsunaWorktreeForMcpTest(options: {
  osunaHome: string;
  broadcasts: string[];
    ...
    const result = await createOsunaWorktreeWorkflow(
      {
        ...
        createOsunaWorktree: (workflowInput, workflowOptions) =>
          createOsunaWorktreeService(workflowInput, {
            github,
...
describe("browser MCP tools", () => {
  ...
  it("applies provider policy after the browser-tools host gate", async () => {
    ...
    const server = await createAgentMcpServer({
      ...
      browserToolsBroker: broker as BrowserToolsBroker,
      callerAgentId: "agent-1",
      osunaToolPolicy: { disabledTools: ["browser_list_tabs"] },
      logger,
    });
  ...
  it("filters policy-disabled tools from MCP listing and calls", async () => {
    ...
    const server = await createAgentMcpServer({
      ...
      browserToolsBroker: broker as BrowserToolsBroker,
      callerAgentId: "agent-1",
      osunaToolPolicy: { disabledTools: ["list_agents", "browser_list_tabs"] },
      logger,
    });
    const client = await connectInMemoryMcpClient(server);
  ...
  it("wires browser tools through the browser tools broker", async () => {
    ...
    expect(response.content).toEqual([
      {
        type: "text",
        text: "No Osuna browser tabs are open. Call browser_new_tab to create one.",
      },
    ]);
    expect(response.structuredContent).toEqual({
      ok: true,
  ...
  it("tells browser callers without a workspace how to proceed before broker execution", async () => {
    ...
    expect(response.content).toEqual([
      {
        type: "text",
        text: "This browser tool needs a workspace. Start the agent from an Osuna workspace before calling browser_new_tab or browser_list_tabs.",
      },
    ]);
    expect(response.structuredContent).toEqual({
      ok: false,
...
describe("create_agent MCP tool", () => {
  ...
  it("registers and broadcasts a workspace when create_agent creates a worktree", async () => {
    const { agentManager, agentStorage, spies } = createTestDeps();
    const tempDir = await mkdtemp(join(tmpdir(), "osuna-mcp-worktree-"));
    const repoDir = join(tempDir, "repo");
    const osunaHome = join(tempDir, ".osuna");
    ...
    try {
      ...
      const server = await createAgentMcpServer({
        ...
        providerSnapshotManager: createOpenCodeManager().manager,
        osunaHome,
        createOsunaWorktree: createOsunaWorktreeForMcpTest({
          osunaHome,
          broadcasts,
          createdWorkspaceIds,
          setupContinuations,
  ...
  it("creates a create_agent branch-off worktree without invoking the legacy metadata branch rename", async () => {
    const { agentManager, agentStorage, spies } = createTestDeps();
    const tempDir = await mkdtemp(join(tmpdir(), "osuna-mcp-agent-worktree-name-context-"));
    const repoDir = join(tempDir, "repo");
    const osunaHome = join(tempDir, ".osuna");
    const broadcasts: string[] = [];
    ...
    try {
      ...
      const server = await createAgentMcpServer({
        ...
        providerSnapshotManager: createOpenCodeManager().manager,
        osunaHome,
        createOsunaWorktree: createOsunaWorktreeForMcpTest({ osunaHome, broadcasts }),
  ...
  it("auto-titles and renames an agent-created branch-off worktree from the initial prompt", async () => {
    const { agentManager, agentStorage, spies } = createTestDeps();
    const tempDir = await mkdtemp(join(tmpdir(), "osuna-mcp-agent-worktree-auto-title-"));
    const repoDir = join(tempDir, "repo");
    const osunaHome = join(tempDir, ".osuna");
    const broadcasts: string[] = [];
    const createdWorkspaceIds: string[] = [];
    ...
    try {
      ...
      const branchName = execFileSync("git", ["branch", "--show-current"], {
        ...
        .toString()
        .trim();
      const metadata = readOsunaWorktreeMetadata(agentCwd);
  ...
  it("keeps a manual workspace title when agent-created worktree naming finishes later", async () => {
    const { agentManager, agentStorage, spies } = createTestDeps();
    const tempDir = await mkdtemp(join(tmpdir(), "osuna-mcp-agent-worktree-manual-title-"));
    const repoDir = join(tempDir, "repo");
    const osunaHome = join(tempDir, ".osuna");
    const broadcasts: string[] = [];
    const createdWorkspaceIds: string[] = [];
  ...
  it("uses create_agent title for the agent while still auto-titling the worktree workspace", async () => {
    const { agentManager, agentStorage, spies } = createTestDeps();
    const tempDir = await mkdtemp(join(tmpdir(), "osuna-mcp-agent-title-workspace-title-"));
    const repoDir = join(tempDir, "repo");
    const osunaHome = join(tempDir, ".osuna");
    const broadcasts: string[] = [];
    const createdWorkspaceIds: string[] = [];
  ...
  it("auto-titles an agent-created directory workspace from the initial prompt", async () => {
    const { agentManager, agentStorage, spies } = createTestDeps();
    const tempDir = await mkdtemp(join(tmpdir(), "osuna-mcp-agent-directory-auto-title-"));
    const workspaceDir = join(tempDir, "workspace");
    const workspaceRecords = new Map<string, PersistedWorkspaceRecord>();
    const broadcasts: string[] = [];
    const workspaceGitService = new WorkspaceGitServiceImpl({
      logger: createTestLogger(),
      osunaHome: join(tempDir, ".osuna"),
  ...
  it("auto-titles without renaming a create_agent checkout worktree from the initial prompt", async () => {
    const { agentManager, agentStorage, spies } = createTestDeps();
    const tempDir = await mkdtemp(join(tmpdir(), "osuna-mcp-agent-checkout-name-context-"));
    const repoDir = join(tempDir, "repo");
    const osunaHome = join(tempDir, ".osuna");
    const broadcasts: string[] = [];
    const createdWorkspaceIds: string[] = [];
    ...
    try {
      ...
      expect(readOsunaWorktreeMetadata(agentCwd)).toMatchObject({
        version: 1,
  ...
  it("passes create_agent GitHub PR worktrees through workspace creation without metadata branch rename", async () => {
    ...
    const createOsunaWorktree = vi.fn(
      async (
        ...
        options?: Parameters<CreateOsunaWorktreeWorkflowFn>[1],
      ) => ({
        worktree: {
          branchName: "pr-123",
  ...
  it("creates a worktree-isolated workspace", async () => {
    const { agentManager, agentStorage } = createTestDeps();
    const tempDir = await mkdtemp(join(tmpdir(), "osuna-mcp-create-worktree-"));
    const repoDir = join(tempDir, "repo");
    const osunaHome = join(tempDir, ".osuna");
    const broadcasts: string[] = [];
    const setupContinuations: Array<"workspace" | "agent" | undefined> = [];
  ...
  it("creates a worktree workspace from a project root without a path", async () => {
    ...
    const createOsunaWorktree: CreateOsunaWorktreeWorkflowFn = async (input) => {
  ...
  it("archives a worktree-isolated workspace by workspace id", async () => {
    ...
    const tempDir = realpathSync.native(
      await mkdtemp(join(tmpdir(), "osuna-mcp-archive-worktree-")),
    );
    const repoDir = join(tempDir, "repo");
    const osunaHome = join(tempDir, ".osuna");
  ...
  it("keeps an owned worktree while another workspace still references it", async () => {
    const { agentManager, agentStorage } = createTestDeps();
    const tempDir = realpathSync.native(
      await mkdtemp(join(tmpdir(), "osuna-mcp-archive-worktree-multi-")),
    );
    const repoDir = join(tempDir, "repo");
    const osunaHome = join(tempDir, ".osuna");
  ...
  it("does not expose worktree path or slug operations", async () => {
    const { agentManager, agentStorage } = createTestDeps();
    const tempDir = realpathSync.native(
      await mkdtemp(join(tmpdir(), "osuna-mcp-archive-worktree-slug-")),
    );
    const repoDir = join(tempDir, "repo");
    const osunaHome = join(tempDir, ".osuna");
  ...
  it("lists active workspace descriptors", async () => {
    ...
    const workspace = createPersistedWorkspaceRecord({
      ...
      projectId: "project-1",
      cwd: "/tmp/osuna/worktrees/repo/feature",
      kind: "worktree",
      displayName: "feature",
  ...
  it("allows caller agents to override cwd and applies caller context labels", async () => {
    const { agentManager, agentStorage, spies } = createTestDeps();
    const baseDir = await mkdtemp(join(tmpdir(), "osuna-mcp-test-"));
