...
function createTempHome(): string {
  return mkdtempSync(path.join(tmpdir(), "osuna-config-"));
}
...
describe("PersistedConfigSchema worktrees config", () => {
  test("accepts optional worktree root", () => {
    ...
    expect(parsed.worktrees?.root).toBe("/mnt/fast/osuna-worktrees");
  });
...
describe("loadPersistedConfig", () => {
  test("materializes relay disabled for a new Osuna home", () => {
  ...
  test("accepts the documented config schema marker", () => {
    ...
    try {
      writeFileSync(
        ...
        `${JSON.stringify(
          {
            $schema: "https://example.com/schemas/osuna.config.v1.json",
            version: 1,
            daemon: {
