...
interface CreateServiceOptions {
  ...
  listOsunaWorktrees?: ReturnType<typeof vi.fn>;
  github?: ForgeService;
...
function buildDefaultServiceDeps() {
  return {
    ...
    listOsunaWorktrees: vi.fn(async () => []),
  ...
  test("GitHub self-heal polling uses the fork PR head branch instead of the owner-prefixed local branch", async () => {
    ...
    const getCheckoutSnapshotFacts = vi.fn(async (cwd: string) =>
      createCheckoutFacts(cwd, {
        ...
        branchRemoteName: "osuna-pr-1285",
...
describe("WorkspaceGitServiceImpl D2 read methods", () => {
  ...
  test("listWorktrees cold-loads, warms, forces, and coalesces per repo root", async () => {
    ...
    const listOsunaWorktrees = vi.fn().mockResolvedValue(worktrees);
    const service = createService({
      listOsunaWorktrees,
      now: () => new Date(nowMs),
    });
    ...
    expect(listOsunaWorktrees).toHaveBeenCalledTimes(1);
    ...
    expect(listOsunaWorktrees).toHaveBeenCalledTimes(2);
  ...
  test("listWorktrees shares one repo-root scoped read across sibling workspace cwds", async () => {
    ...
    const worktrees = [
      {
        path: join(tempDir, "osuna-home", "worktrees", "repo", "feature"),
        createdAt: "2026-04-12T00:00:00.000Z",
        branchName: "feature",
      },
    ];
    const listOsunaWorktrees = vi.fn(async () => worktrees);
    const service = createService({
      getCheckoutSnapshotFacts: getCheckoutSnapshotFactsUncached as never,
      getCheckoutStatus: getCheckoutStatusUncached as never,
      listOsunaWorktrees,
    ...
    try {
      ...
      expect(listOsunaWorktrees).toHaveBeenCalledTimes(1);
      expect(listOsunaWorktrees).toHaveBeenCalledWith({
  ...
  test("getProjectSlug derives the slug from the snapshot cache", async () => {
    ...
    await expect(service.getProjectSlug(REPO_CWD)).resolves.toBe("osuna");
