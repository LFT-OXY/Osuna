...
function seedFetchFixture(): {
  ...
  const fixtureRoot = mkdtempSync(join(tmpdir(), "osuna-noop-fetch-"));
  cleanupPaths.push(fixtureRoot);
  const repoRoot = join(fixtureRoot, "repo");
  const originRoot = join(fixtureRoot, "origin.git");
  const osunaHome = join(fixtureRoot, "osuna-home");
  const worktreesRoot = join(fixtureRoot, "worktrees");
  mkdirSync(repoRoot, { recursive: true });
  mkdirSync(osunaHome, { recursive: true });
  ...
  return { originRoot, osunaHome, repoRoot, worktrees };
}
