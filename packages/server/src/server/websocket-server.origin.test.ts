...
describe("isWebSocketSameOrigin", () => {
  test("accepts exact same-origin websocket upgrades", () => {
    ...
    expect(isWebSocketSameOrigin("https://osuna.example.com", "osuna.example.com")).toBe(true);
  });
  ...
  test("rejects non-loopback cross-origin upgrades", () => {
    ...
    expect(isWebSocketSameOrigin("http://127.0.0.1:32775", "osuna.example.com:32775")).toBe(false);
  });
});
