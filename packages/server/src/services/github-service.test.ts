...
import { CheckoutPrStatusResponseSchema } from "@osuna/protocol/messages";
    ...
    data: {
      repository: {
        pullRequest: {
          ...
          headRepository: {
            sshUrl: "git@github.com:therainisme/osuna.git",
            url: "https://github.com/therainisme/osuna",
          },
        },
      },
    },
...
describe("ForgeService", () => {
  ...
  it("loads pull request checkout target details through GraphQL", async () => {
    ...
    expect(runner.calls[1]?.args).toContain("owner=lft-oxy");
    expect(runner.calls[1]?.args).toContain("name=osuna");
      ...
      pullRequestTimelineJson({
        ...
        comments: {
          nodes: [
            {
              id: "IC_badge",
              body: "![build](https://img.shields.io/github/actions/workflow/status/lft-oxy/osuna/ci.yml)",
              bodyHTML:
  ...
  it("loads GitHub merge, auto-merge, permission, policy, and queue facts for PR 993 shape", async () => {
    const runner = createScriptedRunner([
      currentPullRequestJson({
        number: 993,
        url: "https://github.com/lft-oxy/osuna/pull/993",
        title: "Auto-merge UX",
        ...
        statusCheckRollup: [
          {
            ...
            status: "IN_PROGRESS",
            conclusion: null,
            detailsUrl: "https://github.com/lft-oxy/osuna/actions/runs/993",
          },
        ],
      }),
      currentPullRequestGithubFactsJson(),
      ...
      checks: [
        {
          name: "server tests",
          status: "pending",
          url: "https://github.com/lft-oxy/osuna/actions/runs/993",
          workflow: "CI",
        },
      ],
  ...
  it("treats a GitHub issue or PR URL as a search for that number", async () => {
    ...
    await service.searchIssuesAndPrs({
      cwd: "/repo",
      query: "https://github.com/lft-oxy/osuna/pull/793",
      limit: 5,
    });
  ...
  it("does not treat an unrelated tracker URL as a GitHub issue/PR number", async () => {
    ...
    await service.searchIssuesAndPrs({
      cwd: "/repo",
      query: "https://gitlab.com/lft-oxy/osuna/issues/793",
      limit: 5,
    });
    ...
    expect(runner.calls.map((call) => call.args)).toEqual([
      [
        ...
        "list",
        "--search",
        "https://gitlab.com/lft-oxy/osuna/issues/793",
        "--json",
        "number,title,url,state,body,labels,updatedAt",
        "--limit",
        "5",
  ...
  it("treats a GitHub Enterprise issue/PR URL as a search for that number", async () => {
    ...
    await service.searchIssuesAndPrs({
      cwd: "/repo",
      query: "https://github.acme.internal/lft-oxy/osuna/pull/793",
      limit: 5,
    });
