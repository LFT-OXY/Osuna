...
import { parseGitRemoteLocation, type GitRemoteLocation } from "@osuna/protocol/git-remote";
...
export function createForgeResolver(options: CreateForgeResolverOptions = {}): ForgeResolver {
  ...
  // from outside Osuna — `git remote add`/`set-url` run in a terminal — is
  // still picked up within a cycle instead of staying pinned forever; an Osuna
