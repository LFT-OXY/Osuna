...
const expected = JSON.parse(process.env.OSUNA_EXPECTED_ARGV_JSON);
const sliceFrom = process.env.OSUNA_ARGV_SLICE_FROM ? Number(process.env.OSUNA_ARGV_SLICE_FROM) : 2;
...
async function runFixture(params: {
  ...
  const child = spawnProcess(params.command, params.args, {
    env: {
      ...process.env,
      OSUNA_EXPECTED_ARGV_JSON: JSON.stringify(["--config", JSON_ARG]),
    },
...
async function findAndLaunch(fixture: LaunchFixture): Promise<{
  ...
  return withPathPrepended(fixture.root, async () => {
    ...
    const child = spawnProcess(found, fixture.args, {
      env: {
        ...process.env,
        OSUNA_EXPECTED_ARGV_JSON: fixture.expectedArgvJson,
        OSUNA_ARGV_SLICE_FROM: String(fixture.sliceFrom),
      },
      stdio: ["ignore", "pipe", "pipe"],
    });
