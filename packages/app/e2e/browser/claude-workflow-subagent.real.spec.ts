...
// opts in explicitly so it tests Osuna's integration rather than the account's rollout cohort.
...
test.describe("real Claude workflow subagent row", () => {
  ...
  test("shows a real Claude workflow running and completed through the generic subagent UI", async ({
    ...
    const cwd = realpathSync(mkdtempSync(path.join(tmpdir(), "osuna-claude-workflow-row-")));
