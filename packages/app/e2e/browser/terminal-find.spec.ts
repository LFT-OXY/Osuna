...
async function viewport(page: Page) {
  return page.evaluate(
    () =>
      (window as Window & { __osunaTerminal?: { buffer: { active: { viewportY: number } } } })
        .__osunaTerminal?.buffer.active.viewportY,
  );
}
...
async function readVimScreen(page: Page) {
  const rows = await page.evaluate(
    () => (window as Window & { __osunaTerminal: { rows: number } }).__osunaTerminal.rows,
  );
