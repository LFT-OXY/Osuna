...
interface OsunaOwnedWorktree {
  projectName: string;
  workspaceId: string;
  worktreeSlug: string;
}

async function withOsunaOwnedWorktree(
  run: (workspace: OsunaOwnedWorktree) => Promise<void>,
): Promise<void> {
  ...
  try {
    ...
    if (!created.workspace) {
      throw new Error(created.error ?? "Failed to create Osuna-owned worktree");
    }
...
test.describe("Sidebar workspace list", () => {
  ...
  test("Osuna-owned worktree hover card shows the worktree directory name", async ({ page }) => {
    await withOsunaOwnedWorktree(async ({ projectName, workspaceId, worktreeSlug }) => {
      await gotoAppShell(page);
      await waitForSidebarProject(page, projectName);
