...
test("keeps the selected workspace visible in Light", async ({ page }, testInfo) => {
  ...
  try {
    await page.addInitScript(() => {
      localStorage.setItem("@osuna:app-settings", JSON.stringify({ theme: "light" }));
    });
...
test("keeps the selected workspace visible in Pure black", async ({ page }, testInfo) => {
  ...
  try {
    await page.addInitScript(() => {
      localStorage.setItem("@osuna:app-settings", JSON.stringify({ theme: "pureBlack" }));
    });
    await gotoAppShell(page);
...
test("applies the interface font size to settings text", async ({ page }) => {
  await page.addInitScript(() => {
    localStorage.setItem("@osuna:app-settings", JSON.stringify({ uiBaseFontSize: 21 }));
  });
  await page.goto("/settings");
  await expect(page.getByTestId("settings-sidebar")).toBeVisible();
  await openSettingsSection(page, "appearance");
