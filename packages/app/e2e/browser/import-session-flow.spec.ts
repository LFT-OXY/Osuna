...
const SCREENSHOT_DIRECTORY = path.join(
  ...
  ".osuna/plans/import-session-ux",
);
const claudeConfigDirectory = mkdtempSync(path.join(tmpdir(), "osuna-import-flow-claude-"));
...
test.beforeAll(async () => {
  ...
  const repo = await createTempGitRepo("isf-", {
    originUrl: "https://github.com/osuna-e2e/import-session-fixture.git",
  });
