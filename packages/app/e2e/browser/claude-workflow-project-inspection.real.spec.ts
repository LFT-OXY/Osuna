...
const DESCRIPTION = "Inspect Osuna and explain what the project does";
...
test.describe("real Claude project-inspection workflow", () => {
  ...
  test("shows the workflow's actual report in the generic detail pane", async ({
    ...
    const cwd = realpathSync(mkdtempSync(path.join(tmpdir(), "osuna-project-inspection-")));
    ...
    try {
      ...
      await expect(
        panel.getByText("Inspect the Osuna repository at", { exact: false }),
      ).toHaveCount(0);
