...
const RUN_AGENT_STREAM_PERF = process.env.OSUNA_AGENT_STREAM_PERF_E2E === "1";
...
agentStreamPerfDescribe("Agent stream smoothness", () => {
  test("reveals bursty model output at a steady rate", async ({ page }, testInfo) => {
    ...
    await page.addInitScript(() => {
      Reflect.set(globalThis, "__OSUNA_RENDER_PROFILE_ENABLED__", true);
    });
    ...
    try {
      ...
      await page.evaluate(() => {
        const reset = Reflect.get(globalThis, "__OSUNA_RESET_RENDER_PROFILE__");
...
async function readStreamReactCommits(
  ...
  return await page.evaluate(() => {
    const samples = Reflect.get(globalThis, "__OSUNA_RENDER_PROFILE__");
