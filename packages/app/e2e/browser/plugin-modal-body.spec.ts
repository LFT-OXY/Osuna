...
async function copyAndPaste(page: Page) {
  ...
  expect(await page.evaluate(() => navigator.clipboard.readText())).toBe("Copied from Osuna");
  await page.getByRole("textbox", { name: "Paste here" }).focus();
  await page.keyboard.press("ControlOrMeta+A");
  await page.keyboard.press("ControlOrMeta+V");
  await expect(page.getByRole("textbox", { name: "Paste here" })).toHaveValue("Copied from Osuna");
}
