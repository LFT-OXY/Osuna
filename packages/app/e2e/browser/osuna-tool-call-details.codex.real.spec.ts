...
function verificationPrompt(workspacePath: string): string {
  return [
    "Use Osuna tools to perform this exact UI verification.",
    ...
    "Use each named Osuna tool exactly once except for the matching schedule and heartbeat deletion tools.",
  ].join(" ");
}
...
test.describe("real Codex Osuna tool-call presentation", () => {
  test.setTimeout(600_000);

  test("renders owned tool inputs and results as readable text", async ({ page }, testInfo) => {
    const cwd = realpathSync(mkdtempSync(path.join(tmpdir(), "osuna-tool-call-presentation-")));
