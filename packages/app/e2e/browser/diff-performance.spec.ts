...
const RUN_DIFF_PERF = process.env.OSUNA_DIFF_PERF_E2E === "1";
const RUN_HEADER_COMPARISON = process.env.OSUNA_DIFF_HEADER_COMPARISON === "1";
const diffPerfDescribe = RUN_DIFF_PERF ? test.describe : test.describe.skip;
const LINE_COUNT_PER_FILE = 1_200;
const MANY_FILE_COUNT = Number(process.env.OSUNA_DIFF_MANY_FILE_COUNT ?? 2_000);
const FILE_PATHS = ["src/large-a.ts", "src/large-b.ts"] as const;
const CPU_SLOWDOWN = Number(process.env.OSUNA_DIFF_PERF_CPU_SLOWDOWN ?? 6);
const CHANGES_PREFERENCES_KEY = "@osuna:changes-preferences";
const POC_MEDIAN_FRAME_MS = Number(
  process.env.OSUNA_DIFF_POC_MEDIAN_FRAME_MS ?? DIFF_POC_BASELINE.medianFrameMs,
);
const POC_P95_FRAME_MS = Number(
  process.env.OSUNA_DIFF_POC_P95_FRAME_MS ?? DIFF_POC_BASELINE.p95FrameMs,
);
  ...
  test("many small files keep canvas headers bounded and responsive", async ({
    ...
    try {
      ...
      const headerCanvasMetrics = await page
        ...
        .catch(() => null);
      const capturePath = process.env.OSUNA_DIFF_HEADER_CAPTURE_PATH;
      if (capturePath) {
  ...
  await page.addInitScript(
    ({ preferencesKey }) => {
      const scope = globalThis as typeof globalThis & {
        __OSUNA_DIFF_REACT_STATS__?: { commits: number };
      };
      scope.__OSUNA_DIFF_REACT_STATS__ = { commits: 0 };
...
async function startRendererInstrumentation(page: Page): Promise<void> {
  await page.getByTestId("git-diff-canvas-root").evaluate((root) => {
    const scope = globalThis as typeof globalThis & {
      __OSUNA_DIFF_REACT_STATS__?: { commits: number };
      __OSUNA_DIFF_RENDERER_STATS__?: {
        initialCommits: number;
        addedElements: number;
        removedElements: number;
      };
    };
    const reactStats = scope.__OSUNA_DIFF_REACT_STATS__ ?? { commits: 0, unmounts: 0 };
    const stats = {
      initialCommits: reactStats.commits,
      addedElements: 0,
      removedElements: 0,
    };
    scope.__OSUNA_DIFF_RENDERER_STATS__ = stats;
...
async function readRendererInstrumentation(page: Page): Promise<{
  ...
  return page.evaluate(() => {
    ...
    const react = scope.__OSUNA_DIFF_REACT_STATS__!;
    const renderer = scope.__OSUNA_DIFF_RENDERER_STATS__!;
    return {
