...
const SHARED_PROJECT_NAME = "Osuna";
const SHARED_REMOTE_URL = "https://github.com/lft-oxy/osuna.git";
...
const test = base.extend<{ crossHostProject: CrossHostProjectScenario }>({
  crossHostProject: async ({ page: _page }, provide) => {
    ...
    const primarySharedRepo = await createTempGitRepo("new-workspace-osuna-primary-", {
      originUrl: SHARED_REMOTE_URL,
    });
    const primaryDuplicateRepo = await createTempGitRepo("new-workspace-osuna-duplicate-", {
      originUrl: SHARED_REMOTE_URL,
    });
    const secondaryFallbackRepo = await createTempGitRepo("new-workspace-fallback-");
    const secondarySharedRepo = await createTempGitRepo("new-workspace-osuna-secondary-", {
      originUrl: SHARED_REMOTE_URL,
    });
    ...
    try {
      ...
      const primarySharedProject = await createProject(primaryClient, {
        ...
        workspaceName: "Primary Osuna workspace",
      });
      ...
      const primaryDuplicateProject = await createProject(primaryClient, {
        ...
        projectName: "Osuna duplicate",
        workspaceName: "Duplicate Osuna workspace",
      });
      ...
      const secondarySharedProject = await createProject(secondaryClient, {
        ...
        projectName: SHARED_PROJECT_NAME,
        workspaceName: "Secondary Osuna workspace",
      });
      ...
      if (
        ...
        throw new Error("Expected the Osuna clones to have the same equivalence key");
      }
