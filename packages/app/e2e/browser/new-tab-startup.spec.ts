...
test("a workspace with an existing agent never presents New during startup", async ({ page }) => {
  ...
  try {
    await page.addInitScript(() => {
      Reflect.set(globalThis, "__osunaSawVisibleNewTab", false);
      function recordVisibleNewTab() {
        ...
        if (panel?.getClientRects().length) {
          Reflect.set(globalThis, "__osunaSawVisibleNewTab", true);
        }
      }
    ...
    expect(await page.evaluate(() => Reflect.get(globalThis, "__osunaSawVisibleNewTab"))).toBe(
      false,
    );
  } finally {
