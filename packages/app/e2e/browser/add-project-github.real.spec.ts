...
test.describe("Add Project GitHub flow", () => {
  ...
  test("searches the host's repositories and clones into a clearly shown final path", async ({
    ...
    const parentDirectory = await mkdtemp(path.join(tmpdir(), "osuna-e2e-github-clone-"));
    ...
    try {
      ...
      await addProjectFlowInput(page).fill("lft-oxy/osuna");
      await expect(addProjectFlow(page).getByText("lft-oxy/osuna", { exact: true })).toBeVisible({
        timeout: 30_000,
      });
