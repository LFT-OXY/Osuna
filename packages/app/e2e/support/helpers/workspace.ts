...
export const createTempGitRepo = async (
  ...
  execSync('git config user.email "e2e@osuna.test"', { cwd: repoPath, stdio: "ignore" });
  execSync('git config user.name "Osuna E2E"', { cwd: repoPath, stdio: "ignore" });
  ...
  if (options?.osunaConfig) {
    await writeFile(
      ...
      JSON.stringify(options.osunaConfig, null, 2),
    );
  }
 ...
 */
export async function createTempDirectory(prefix = "osuna-e2e-dir-"): Promise<TempDirectory> {
...
export async function trackForkUpstream(repoPath: string): Promise<string> {
  ...
  const upstreamClone = await mkdtemp(path.join(await resolveTempRoot(), "osuna-e2e-upstream-"));
  ...
  execSync('git config user.email "e2e@osuna.test"', { cwd: upstreamClone, stdio: "ignore" });
  execSync('git config user.name "Osuna E2E"', { cwd: upstreamClone, stdio: "ignore" });
...
export async function readWorktreeBaseMetadata(
  ...
  const metadata = JSON.parse(
    await readFile(path.join(gitDir, "osuna", "worktree.json"), "utf8"),
