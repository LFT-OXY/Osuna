...
interface ProjectClient {
  ...
  close(): Promise<void>;
  createOsunaWorktree?: (...args: unknown[]) => Promise<OwnedResourceResult>;
...
export function withProjectOwnership<Client extends object>(client: Client): Client {
  ...
  return new Proxy(client, {
    get(_target, property) {
      ...
      if (property === "createOsunaWorktree" && target.createOsunaWorktree) {
        const createOsunaWorktree = target.createOsunaWorktree.bind(target);
        return async (...args: unknown[]) => trackProject(await createOsunaWorktree(...args));
      }
