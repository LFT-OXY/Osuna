    ...
    (window as unknown as { osunaDesktop: unknown }).osunaDesktop = {
      ...
      opener: {
        openUrl: async (url: string) => {
          localStorage.setItem("@osuna:e2e-opened-url", url);
        },
      },
    };
  }, daemon.serverId);
...
export async function expectRelayConsent(page: Page): Promise<void> {
  ...
  await expect(modal.getByRole("link", { name: "Read how Osuna relay works" })).toBeVisible();
...
export async function openRelaySecurityDocs(page: Page): Promise<void> {
  await page.getByRole("link", { name: "Read how Osuna relay works" }).click();
  await expect
    .poll(() => page.evaluate(() => localStorage.getItem("@osuna:e2e-opened-url")))
    .toBe("https://github.com/LFT-OXY/Osuna/blob/main/public-docs/security.md");
}
