...
export interface OsunaHomeMetadataForkResult {
  sourceHome: string;
  targetHome: string;
  agentFiles: number;
  agentBytes: number;
...
export function resolveOsunaHomePath(value: string): string {
  if (value === "~") {
    return homedir();
  }
  if (value.startsWith("~/")) {
...
export async function forkOsunaHomeMetadata(input: {
  sourceHome: string;
  targetHome: string;
}): Promise<OsunaHomeMetadataForkResult> {
  const sourceHome = resolveOsunaHomePath(input.sourceHome);
  const targetHome = resolveOsunaHomePath(input.targetHome);

  if (sourceHome === targetHome) {
    throw new Error("Refusing to fork Osuna metadata onto the same OSUNA_HOME.");
  }
