...
export async function expectUncommittedSetupWarning(page: Page): Promise<void> {
  const warning = page.getByRole("alert").filter({ hasText: "Commit osuna.json changes" });
  await expect(warning).toContainText("Commit osuna.json changes");
...
// Proxies all daemon WS traffic transparently, but rejects osuna.json reads
