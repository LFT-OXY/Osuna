...
async function seedPr(args: {
  ...
  git(["config", "user.email", "e2e@osuna.test"], localPath);
  git(["config", "user.name", "Osuna E2E"], localPath);
...
const TEMP_GITHUB_REPO_PREFIX = "osunatmp-";
...
export async function createTempGithubRepo(options: {
  ...
  git(["config", "user.email", "e2e@osuna.test"], basePath);
  git(["config", "user.name", "Osuna E2E"], basePath);
...
export async function cloneGithubRepoDefaultBranchOnly(
  ...
  git(["config", "user.email", "e2e@osuna.test"], clonePath);
  git(["config", "user.name", "Osuna E2E"], clonePath);
...
export async function createLocalGithubPrFixture(): Promise<LocalGhPrFixture> {
  const fixtureRoot = await mkdtemp(path.join("/tmp", "osuna-e2e-local-github-pr-"));
  ...
  const githubUrl = "https://github.com/osuna-e2e/local-fixture.git";
  ...
  git(["config", "user.email", "e2e@osuna.test"], checkoutPath);
  git(["config", "user.name", "Osuna E2E"], checkoutPath);
  ...
  return {
    pr: {
      ...
      url: "https://github.com/osuna-e2e/local-fixture/pull/1",
      branch: "pr-branch-1",
      localPath: basePath,
    },
    mainCheckout: {
