...
describe("createSidebarWorkspaceEntry workspace directory label", () => {
  it("uses the daemon-provided slug for an Osuna-owned worktree", () => {
    ...
    const projects = buildSidebarProjectsFromStructure({
      projects: [
        project({
          ...
          hosts: [
            {
              serverId: "relay:osuna-host",
              iconWorkingDir: "/repo/project-1",
              worktreeSupport: "supported" as const,
            },
          ],
          workspaceKeys: ["relay:osuna-host:ws-main"],
        }),
      ],
    });

    expect(projects[0]?.workspaces[0]).toMatchObject({
      workspaceKey: "relay:osuna-host:ws-main",
      serverId: "relay:osuna-host",
      workspaceId: "ws-main",
    });
  });
...
describe("shared sidebar workspace model", () => {
  it("feeds project placement and status grouping from the same cross-host workspace identities", () => {
    const model = buildSidebarWorkspacePlacementModel({
      projects: [
        project({
          projectKey: "lft-oxy/osuna",
          projectName: "lft-oxy/osuna",
          iconWorkingDir: "/repo/lft-oxy/osuna",
          hosts: [
            {
              serverId: "host-a",
              iconWorkingDir: "/repo/lft-oxy/osuna",
          ...
          workspaces: new Map([
            [
              ...
              workspace({
                id: "main",
                name: "main",
                projectId: "lft-oxy/osuna",
                projectDisplayName: "lft-oxy/osuna",
                status: "done",
              }),
            ],
          ]),
    ...
    expect(model.projectNamesByViewKey).toEqual(new Map([["lft-oxy/osuna", "lft-oxy/osuna"]]));
  });
