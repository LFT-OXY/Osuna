...
function getCronPreview(
  expression: string,
  timezone: string,
  error: ScheduleDescription | null,
): ScheduleDescription | null {
  if (error || !expression) {
    return null;
  }
  return describeCron({ type: "cron", expression, timezone }) ?? { text: expression };
}
...
export function CadenceEditor({ value, onChange, error, size = "md" }: CadenceEditorProps) {
  ...
  const selectedPresetDisplay = useMemo(
    () => ({ label: t(resolveCronPresetDisplay(currentCadence).labelKey) }),
    [currentCadence, t],
  );
  ...
  if (effectiveError) {
    feedback = <Text style={styles.error}>{renderScheduleDescription(t, effectiveError)}</Text>;
  } else if (preview) {
    feedback = <Text style={styles.preview}>{renderScheduleDescription(t, preview)}</Text>;
  }
    ...
    <Field label={t("schedules.cadence.label")}>
      <View style={styles.stack}>
        <SelectField
          ...
          placeholder={t("schedules.cadence.selectPlaceholder")}
