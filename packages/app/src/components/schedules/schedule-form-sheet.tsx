...
function OpenScheduleFormSheet({
  ...
  const submitNewAgent = useCallback(async (): Promise<boolean> => {
    ...
    await createSchedule({
      ...
      cadence: requireCronCadence(state.submitCadence, t("schedules.form.cronCadenceRequired")),
      target: {
        type: "new-agent",
        config: {
          provider,
  ...
  const header = useMemo<SheetHeader>(() => {
    if (mode !== "edit") {
      return { title: t("schedules.form.createTitle") };
    }
    const product = schedule ? scheduleProductName(schedule) : "schedule";
    return { title: t(`schedules.form.editTitle.${product}`) };
  }, [mode, schedule, t]);
...
interface ScheduleFormFieldsProps {
  ...
  cadenceError: ScheduleDescription | null;
    ...
    <>
      ...
      <Field label={t("schedules.form.maxRuns")}>
        <FormTextInput
          ...
          placeholder={t("schedules.form.maxRunsPlaceholder")}
...
function ScheduleTargetFields({
  ...
  const defaultModeDisplay = useMemo<SelectFieldDisplay>(
    ...
    <>
      ...
      {state.disclosure.showThinkingField ? (
        <SelectField
          ...
          emptyText={t("schedules.form.noThinkingOptions")}
      ...
      {state.disclosure.showModeField ? (
        <SelectField
          ...
          selectedDisplay={state.selectedModeDisplay ?? defaultModeDisplay}
          options={modeOptions}
...
function ScheduleIsolationField({
    ...
    () => [
      {
        id: "local",
        value: "local",
        label: t("schedules.form.isolationLocal"),
        testID: "schedule-isolation-local",
      },
      {
        id: "worktree",
        value: "worktree",
        label: t("schedules.form.isolationWorktree"),
    ...
    () => ({
      label:
        state.effectiveIsolation === "worktree"
          ? t("schedules.form.isolationWorktree")
          : t("schedules.form.isolationLocal"),
    }),
  ...
  return (
    <SelectField
      ...
      emptyText={t("schedules.form.noIsolationOptions")}
      searchable={false}
