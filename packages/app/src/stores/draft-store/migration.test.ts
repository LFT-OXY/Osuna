...
describe("draft-store migration", () => {
  it("keeps a supported legacy draft for migration", async () => {
    ...
    backing.values.set("osuna-drafts", JSON.stringify({ state: legacyState, version: 4 }));
    const storage = createValidatedPersistStorage(backing, PersistedDraftStoreSchema);

    const stored = await storage.getItem("osuna-drafts");
    ...
    expect(backing.values.has("osuna-drafts")).toBe(true);
  });
  ...
  it("drops a legacy workspace review attachment without deleting its draft", async () => {
    ...
    backing.values.set("osuna-drafts", JSON.stringify({ state: persistedState, version: 4 }));
    const storage = createValidatedPersistStorage(backing, PersistedDraftStoreSchema);
