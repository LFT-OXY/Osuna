...
const OSUNA_WORKTREE_PATH_PATTERN = /(^|\/)\.osuna\/worktrees(\/|$)/;
...
export function collectAgentWorkingDirectorySuggestions(
  ...
  for (const source of sources) {
    ...
    if (isOsunaOwnedWorktreePath(cwd)) {
      continue;
    }
...
function isOsunaOwnedWorktreePath(cwd: string): boolean {
  return OSUNA_WORKTREE_PATH_PATTERN.test(cwd.replace(/\\/g, "/"));
}
