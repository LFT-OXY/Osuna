...
describe("filterAndRankCommandAutocompleteEntries", () => {
  const entries = [
    { source: "provider" as const, command: { name: "osuna-committee" } },
    { source: "provider" as const, command: { name: "commit" } },
    { source: "provider" as const, command: { name: "osuna-advisor" } },
  ];
  ...
  it("ranks command-name prefixes above later word-boundary partial matches", () => {
    ...
    expect(result.map((entry) => entry.command.name)).toEqual(["commit", "osuna-committee"]);
  });
