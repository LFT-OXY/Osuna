...
const runningService: WorkspaceScriptPayload = {
  scriptName: "web",
  type: "service",
  hostname: "web--feature--osuna.localhost",
  port: 3000,
  localProxyUrl: "http://web--feature--osuna.localhost:6767",
  publicProxyUrl: null,
  proxyUrl: "http://web--feature--osuna.localhost:6767",
  lifecycle: "running",
  health: "healthy",
  exitCode: null,
  terminalId: null,
...
describe("resolveWorkspaceScriptLink", () => {
  it("defaults to the memorable Osuna URL locally and keeps direct as a fallback", () => {
    expect(
      resolveLink({ type: "directTcp", endpoint: "localhost:6767", display: "localhost:6767" }),
    ).toEqual({
      primary: {
        kind: "osuna",
        label: "web--feature--osuna.localhost:6767",
        url: "http://web--feature--osuna.localhost:6767",
      },
      targets: [
        {
          kind: "osuna",
  ...
  it("defaults to an explicitly configured reverse proxy", () => {
    const publicUrl = "https://web--feature--osuna.services.example.com";
    expect(
      resolveLink(
        { type: "directSocket", endpoint: "/tmp/osuna.sock", display: "socket" },
        { ...runningService, publicProxyUrl: publicUrl, proxyUrl: publicUrl },
      ),
    ).toEqual({
      primary: {
        kind: "public",
        label: "web--feature--osuna.services.example.com",
        url: publicUrl,
      },
      targets: [
        {
  ...
  it("offers the reverse proxy and direct route over a direct network connection", () => {
    ...
    expect(
      ...
      { kind: "public", label: "web--feature--osuna.services.example.com", url: publicUrl },
      {
        kind: "osuna",
        label: "web--feature--osuna.localhost:6767",
        url: "http://web--feature--osuna.localhost:6767",
