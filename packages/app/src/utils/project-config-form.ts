import type {
  OsunaConfigRaw,
  OsunaMetadataGeneration,
  OsunaMetadataGenerationEntry,
  OsunaScriptEntryRaw,
...
export interface ProjectConfigDraft {
  ...
  metadataGenerationBase: OsunaMetadataGeneration | undefined;
}
...
export function configToDraft(config: OsunaConfigRaw | null | undefined): ProjectConfigDraft {
...
interface ApplyDraftInput {
  ...
  base: OsunaConfigRaw | null | undefined;
}

export function applyDraftToConfig(input: ApplyDraftInput): OsunaConfigRaw {
  ...
  const nextScripts: Record<string, OsunaScriptEntryRaw> = {};
  for (const row of input.draft.scripts) {
    ...
    nextScripts[trimmedName] = nextEntry as OsunaScriptEntryRaw;
  }
  ...
  return result as OsunaConfigRaw;
}
