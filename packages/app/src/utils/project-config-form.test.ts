...
import { OsunaConfigRawSchema } from "@osuna/protocol/osuna-config-schema";
import type { OsunaConfigRaw } from "@osuna/protocol/messages";
...
describe("applyDraftToConfig", () => {
  it("preserves the original string kind when editing an existing setup field", () => {
    const base: OsunaConfigRaw = { worktree: { setup: "npm install" } };
  ...
  it("preserves unknown top-level, worktree, and script entry fields on round-trip", () => {
    const base = OsunaConfigRawSchema.parse({
      worktree: {
        setup: "npm install",
