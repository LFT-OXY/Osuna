...
function makeSources(): DesktopDiagnosticSources {
  return {
    ...
    getDaemonLogs: async () => ({
      logPath: "/osuna/home/daemon.log",
      contents: "daemon line one\ndaemon line two",
    }),
    getAppLogs: async () => ({
      logPath: "/logs/Osuna/main.log",
    ...
    getUpdaterDiagnostics: async () => ({
      ...
      shipItDirectory: "/cache/com.chinhae.osuna.desktop.ShipIt",
      state: {
        path: "/cache/com.chinhae.osuna.desktop.ShipIt/ShipItState.plist",
        exists: true,
      ...
      stdout: {
        path: "/cache/com.chinhae.osuna.desktop.ShipIt/ShipIt_stdout.log",
        exists: false,
        modifiedAt: null,
        contents: "",
        error: null,
      ...
      stderr: {
        path: "/cache/com.chinhae.osuna.desktop.ShipIt/ShipIt_stderr.log",
        exists: true,
...
describe("desktop diagnostic report", () => {
  ...
  test("includes the Electron main-process log after the daemon log", async () => {
    ...
    expect(report).toContain("  Log path: /osuna/home/daemon.log");
    expect(report).toContain("  App log path: /logs/Osuna/main.log");
