...
import type { JsonValue } from "@osuna/protocol/agent-types";
import { getOpenAgentTabLabel } from "@osuna/protocol/agent-labels";
import {
  memo,
  useCallback,
  useEffect,
...
function getFallbackTabOptionLabel(
  ...
  if (tab.target.kind === "session_history") {
    return labels.sessionHistory;
  }
...
function MobileWorkspaceTabOption({
  ...
  const fallbackLabels = useMemo(
    () => ({
      ...
      sessionHistory: t("panels.sessionHistory.label"),
    }),
    [t],
  );
...
export const WorkspaceScreen = memo(function WorkspaceScreen({
  ...
  useEffect(() => {
    traceInstant("osuna.workspace.mount", { serverId, workspaceId });
    return () => {
      traceInstant("osuna.workspace.unmount", { serverId, workspaceId });
    };
