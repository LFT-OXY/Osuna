...
describe("shouldFetchTimelineFrom", () => {
  const baseGate = {
    ...
    githubFeaturesEnabled: true,
    cwd: "/repo",
    identity: { prNumber: 42, repoOwner: "lft-oxy", repoName: "osuna" },
...
describe("fetchPrPaneTimelinePage", () => {
  it("forwards the cwd, PR number, owner, and name to the daemon client", async () => {
    ...
    expect(client.calls).toEqual([
      { cwd: "/repo", prNumber: 42, repoOwner: "lft-oxy", repoName: "osuna" },
    ]);
  });
      ...
      items: [
        {
          ...
          body: "Looks good",
          createdAt: Date.now(),
          url: "https://github.com/lft-oxy/osuna/pull/42#c1",
        },
      ],
    });
        ...
        items: [
          {
            ...
            createdAt: Date.now(),
            url: "https://github.com/lft-oxy/osuna/pull/41#c1",
          },
        ],
      }),
    });
