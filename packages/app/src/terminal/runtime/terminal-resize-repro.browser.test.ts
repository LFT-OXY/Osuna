...
// Regression: "streaming pino log, resized the Osuna terminal, old logs stayed
...
function mount(width: number, height: number): Mounted {
  ...
  const terminal = window.__osunaTerminal as InspectableTerminal | undefined;
  if (!terminal) {
...
describe("terminal resize reflow repro (Osuna terminal)", () => {
