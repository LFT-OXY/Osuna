...
const RESIDENT_HOST_ID = "osuna-browser-resident-webviews";
...
const profileHost: BrowserWebviewProfileHost = {
  profilePartition: "persist:osuna-browser",
...
describe("resident browser webviews", () => {
  ...
  it("creates a resident webview for an agent-created unfocused tab", () => {
    ...
    expect(webview?.isConnected).toBe(true);
    expect(webview?.getAttribute("data-osuna-browser-id")).toBe("browser-agent");
    expect(webview?.getAttribute("partition")).toBe("persist:osuna-browser");
  ...
  it("shares one profile and registers attached guests with explicit identity", () => {
    ...
    expect(firstWebview.getAttribute("partition")).toBe("persist:osuna-browser");
    expect(secondWebview.getAttribute("partition")).toBe("persist:osuna-browser");
