...
import type { ForgeSearchItem, ForgeSearchResponse } from "@osuna/protocol/messages";
import { useComposerForgeAutoAttach } from "./forge-auto-attach";

type ForgeSearchPayload = ForgeSearchResponse["payload"];

const remoteUrl = "git@github.com:acme/osuna.git";
const cwd = "/repo";
...
const pr101: ForgeSearchItem = {
  ...
  number: 101,
  title: "Attach PR",
  url: "https://github.com/acme/osuna/pull/101",
  state: "open",
  body: null,
  labels: [],
  baseRefName: "main",
...
const pr202: ForgeSearchItem = {
  ...
  url: "https://github.com/acme/osuna/pull/202",
...
const issue202: ForgeSearchItem = {
  ...
  number: 202,
  title: "Attach issue",
  url: "https://github.com/acme/osuna/issues/202",
  state: "open",
  body: null,
  labels: [],
  baseRefName: null,
...
const gitlabMr73: ForgeSearchItem = {
  ...
  number: 73,
  title: "Attach MR",
  url: "https://gitlab.com/acme/osuna/-/merge_requests/73",
  state: "opened",
  body: null,
  labels: [],
  projectPath: "acme/osuna",
  baseRefName: "main",
  headRefName: "feature",
};
...
const giteaIssue27: ForgeSearchItem = {
  ...
  url: "https://gitea.example.com/acme/osuna/issues/27",
  state: "open",
  body: null,
  labels: [],
};
...
describe("useComposerForgeAutoAttach", () => {
  it("adds a matching pasted GitHub PR URL as a composer attachment", async () => {
    ...
    act(() => {
      result.current.setText("Please review https://github.com/acme/osuna/pull/101");
    });
    ...
    const { result } = renderHook(
      () =>
        useHarness(client, {
          ...
          remote: "git@gitlab.com:acme/osuna.git",
        }),
      { wrapper: createWrapper() },
    );

    act(() => {
      result.current.setText("Review https://gitlab.com/acme/osuna/-/merge_requests/73/diffs");
    });
    await flushDebounce();
  ...
  it("adds a matching pasted self-hosted Gitea issue URL", async () => {
    ...
    const { result } = renderHook(
      () =>
        useHarness(client, {
          remote: "git@gitea.example.com:acme/osuna.git",
        }),
      { wrapper: createWrapper() },
    );

    act(() => {
      result.current.setText("See https://gitea.example.com/acme/osuna/issues/27");
    });
    await flushDebounce();
  ...
  it("ignores URLs that do not match the current remote", async () => {
    ...
    act(() => {
      result.current.setText("Other repo https://github.com/other/osuna/pull/101");
    });
    await flushDebounce();
  ...
  it("does not add a second pill when the ref is already attached", async () => {
    ...
    act(() => {
      result.current.setText("Already here https://github.com/acme/osuna/pull/101");
    });
    await flushDebounce();
  ...
  it("does not re-add a GitHub ref removed earlier in the same composer session", async () => {
    ...
    act(() => {
      ...
      result.current.setText("Re-pasted https://github.com/acme/osuna/pull/101");
    });
    await flushDebounce();
  ...
  it("handles multiple matching URLs from one paste", async () => {
    ...
    act(() => {
      result.current.setText(
        "Refs https://github.com/acme/osuna/pull/101 and https://github.com/acme/osuna/issues/202",
      );
    });
    await flushDebounce();
  ...
  it("reports pasted pull requests in source order when lookups finish out of order", async () => {
    ...
    act(() => {
      result.current.setText(
        "Refs https://github.com/acme/osuna/pull/101 and https://github.com/acme/osuna/pull/202",
      );
    });
    await flushDebounce();
  ...
  it("preserves pull request order across lookup batches", async () => {
    ...
    act(() => {
      result.current.setText("Review https://github.com/acme/osuna/pull/101");
    });
    await flushDebounce();
    act(() => {
      result.current.setText(
        "Review https://github.com/acme/osuna/pull/101 and https://github.com/acme/osuna/pull/202",
      );
    });
    expect(result.current.isResolving).toBe(true);
    await flushDebounce();
  ...
  it("uses the latest pull request order when URLs are reordered during debounce", async () => {
    ...
    act(() => {
      result.current.setText(
        "Refs https://github.com/acme/osuna/pull/202 and https://github.com/acme/osuna/pull/101",
      );
    });
    await flushDebounce();
  ...
  it("does not report a buffered pull request after its URL is removed", async () => {
    ...
    act(() => {
      result.current.setText("Still https://github.com/acme/osuna/pull/101");
    });
    await flushDebounce();
  ...
  it("releases a removed ref without waiting for its lookup to settle", async () => {
    ...
    act(() => {
      result.current.setText("Still https://github.com/acme/osuna/pull/202");
    });
    await flushDebounce();
  ...
  it("stays resolving when an unrelated attachment is added during lookup", async () => {
    ...
    act(() => {
      ...
      result.current.setText("Review https://github.com/acme/osuna/pull/101 please");
    });
