...
const fixture = `
  <div data-testid="assistant-message">
    <div data-osuna-markdown-tag="p">Prefix <span data-osuna-markdown-tag="strong">bold text</span> and <span data-osuna-markdown-tag="code">inline code</span> suffix.</div>
    <div data-osuna-markdown-tag="ul">
      <div data-osuna-markdown-tag="li"><span data-osuna-markdown-ignore="true" data-osuna-markdown-list-marker="true">•</span><div><span>First bullet text</span></div></div>
      <div data-osuna-markdown-tag="li"><span data-osuna-markdown-ignore="true" data-osuna-markdown-list-marker="true">•</span><div><span>Second bullet text</span></div></div>
    </div>
    <div data-osuna-markdown-tag="pre" data-osuna-markdown-language="ts"><span data-osuna-markdown-tag="code">const answer = true;</span></div>
  </div>
`;
...
describe("assistant selection copy ranges", () => {
  it("does not handle absent, collapsed, or non-assistant selections", () => {
    ...
    const strong = fixtureElement(message, '[data-osuna-markdown-tag="strong"]');
  ...
  it("does not replace the browser clipboard for a range spanning assistant messages", () => {
    ...
    const firstText = fixtureElement(firstMessage, '[data-osuna-markdown-tag="strong"]');
    const secondText = fixtureElement(secondMessage, '[data-osuna-markdown-tag="strong"]');
  ...
  it.each([
    ...
    ({ tag, range, expected, forbiddenHtml }) => {
      const message = mountFixture();
      const element = fixtureElement(message, `[data-osuna-markdown-tag="${tag}"]`);
  ...
  it.each(["strong", "em", "s", "h2", "blockquote"])(
    ...
    (tag) => {
      const message = mountFixture();
      const element = fixtureElement(message, '[data-osuna-markdown-tag="strong"]');
      element.setAttribute("data-osuna-markdown-tag", tag);
      const content = createAssistantSelectionClipboardContent(
  ...
  it("copies complete inline code without delimiters when the selection stays inside", () => {
    const message = mountFixture();
    const element = fixtureElement(message, '[data-osuna-markdown-tag="code"]');
    const content = createAssistantSelectionClipboardContent(
      selectText(element, 0, textNode(element).length),
    );
  ...
  it("keeps a complete inline node but drops formatting from a partial node at the other edge", () => {
    const message = mountFixture();
    const strong = fixtureElement(message, '[data-osuna-markdown-tag="strong"]');
    const code = fixtureElement(message, '[data-osuna-markdown-tag="code"]');
  ...
  it("copies list-item text without inventing a bullet", () => {
    const message = mountFixture();
    const itemText = fixtureElement(message, '[data-osuna-markdown-tag="li"] div span');
    const content = createAssistantSelectionClipboardContent(
  ...
  it("retains a bullet when the range includes the marker and the complete item", () => {
    const message = mountFixture();
    const item = fixtureElement(message, '[data-osuna-markdown-tag="li"]');
  ...
  it("retains a bullet when a drag selects from the rendered marker through the item text", () => {
    const message = mountFixture();
    const marker = fixtureElement(message, '[data-osuna-markdown-list-marker="true"]');
    const itemText = fixtureElement(message, '[data-osuna-markdown-tag="li"] div span');
  ...
  it("retains every selected marker across a partial multi-item drag", () => {
    const message = mountFixture();
    const markerSelector = '[data-osuna-markdown-list-marker="true"]';
    const textSelector = '[data-osuna-markdown-tag="li"] div span';
  ...
  it("retains the original number when a marker drag starts mid-list", () => {
    const message = mountFixture();
    const list = fixtureElement(message, '[data-osuna-markdown-tag="ul"]');
    list.setAttribute("data-osuna-markdown-tag", "ol");
    list.setAttribute("data-osuna-markdown-list-start", "5");
    const markers = list.querySelectorAll<HTMLElement>('[data-osuna-markdown-list-marker="true"]');
    markers.item(0).textContent = "5.";
    markers.item(1).textContent = "6.";
    const secondMarker = fixtureElement(list, '[data-osuna-markdown-list-marker="true"]', 1);
    const secondText = fixtureElement(list, '[data-osuna-markdown-tag="li"] div span', 1);
  ...
  it("retains nested markers when a drag includes the complete outer item", () => {
    const message = mountFixture();
    const firstItem = fixtureElement(message, '[data-osuna-markdown-tag="li"]');
    firstItem.innerHTML = [
      '<span data-osuna-markdown-ignore="true" data-osuna-markdown-list-marker="true">•</span>',
      "<div>",
      "<span>Outer text</span>",
      '<div data-osuna-markdown-tag="ul">',
      '<div data-osuna-markdown-tag="li">',
      '<span data-osuna-markdown-ignore="true" data-osuna-markdown-list-marker="true">•</span>',
    ...
    const outerMarker = fixtureElement(
      firstItem,
      ':scope > [data-osuna-markdown-list-marker="true"]',
    );
    const innerText = fixtureElement(
      firstItem,
      ':scope [data-osuna-markdown-tag="ul"] > [data-osuna-markdown-tag="li"] > div > span',
    );
  ...
  it("preserves paragraph breaks when rich HTML flattens a loose list item", () => {
    ...
    item.insertAdjacentHTML(
      "beforeend",
      '<div data-osuna-markdown-tag="p">First paragraph</div><div data-osuna-markdown-tag="p">Second paragraph</div>',
    );
  ...
  it("omits a partial leading bullet and retains the marker crossed before the trailing item", () => {
    const message = mountFixture();
    const selector = '[data-osuna-markdown-tag="li"] div span';
  ...
  it("copies part of a fenced code block without adding an inline or fenced delimiter", () => {
    ...
    const blockCode = fixtureElement(
      message,
      '[data-osuna-markdown-tag="pre"] [data-osuna-markdown-tag="code"]',
    );
...
function highlightedFixture(language: string | null): string {
  const languageAttribute =
    language === null ? "" : ` data-osuna-markdown-language="${escapeAttribute(language)}"`;
  return [
    '<div data-testid="assistant-message">',
    `<div data-osuna-markdown-tag="pre"${languageAttribute}>`,
    '<span data-osuna-markdown-tag="code">',
    ...
    "<span>    doThing();</span>",
    "</span>",
    '<div data-osuna-markdown-ignore="true"><span>Copy</span></div>',
    "</div>",
    '<div data-osuna-markdown-tag="p"><span>After the block.</span></div>',
    "</div>",
  ].join("");
}
