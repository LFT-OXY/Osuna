...
const FDROID_ABI_VERSION_CODE_BLOCK = `// Osuna F-Droid single-ABI version codes
def osunaAbiVersionCodes = [
${FDROID_ABI_VERSION_CODE_ENTRIES}
]
def osunaArchitectures = (findProperty("reactNativeArchitectures") ?: "")
    .toString()
    .split(",")
    .collect { it.trim() }
    .findAll { !it.isEmpty() }

if (osunaArchitectures.size() == 1) {
    def osunaAbi = osunaArchitectures[0]
    def osunaAbiVersionCode = osunaAbiVersionCodes[osunaAbi]
    if (osunaAbiVersionCode == null) {
        throw new GradleException("Unsupported Osuna Android ABI: " + osunaAbi)
    }
    android.defaultConfig.versionCode = android.defaultConfig.versionCode * 10 + osunaAbiVersionCode
}
`;
...
function configureFdroidAppBuildGradle(contents) {
  ...
  if (!configuredContents.includes("// Osuna F-Droid single-ABI version codes")) {
