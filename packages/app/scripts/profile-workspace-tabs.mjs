...
const serverId = requiredEnv("OSUNA_PROFILE_SERVER_ID");
const workspaceId = requiredEnv("OSUNA_PROFILE_WORKSPACE_ID");
const agentId = requiredEnv("OSUNA_PROFILE_AGENT_ID");
const switchCount = Number(process.env.OSUNA_PROFILE_SWITCH_COUNT ?? 6);
const switchWaitMs = Number(process.env.OSUNA_PROFILE_SWITCH_WAIT_MS ?? 250);
const idleWaitMs = Number(process.env.OSUNA_PROFILE_IDLE_WAIT_MS ?? 0);
...
const createdTerminalId = process.env.OSUNA_PROFILE_TERMINAL_ID ? null : createTerminal();
const terminalId = process.env.OSUNA_PROFILE_TERMINAL_ID ?? createdTerminalId;
...
async function main() {
  ...
  try {
    ...
    const samples = await page.evaluate(() => globalThis.__OSUNA_RENDER_PROFILE__ ?? []);
    const reasons = await page.evaluate(() => globalThis.__OSUNA_RENDER_PROFILE_REASONS__ ?? {});
    console.log(
      JSON.stringify(
        {
          appUrl: baseUrl,
