...
const cadenceMs = Number(process.env.OSUNA_PROFILE_TYPING_CADENCE_MS ?? 16);
const scenario = process.env.OSUNA_PROFILE_TYPING_SCENARIO ?? "continuous-text";
if (!["continuous-text", "height-growth"].includes(scenario)) {
  throw new Error(
    `Unknown OSUNA_PROFILE_TYPING_SCENARIO: ${scenario}. Use continuous-text or height-growth.`,
  );
}
const sampleCount = Number(
  process.env.OSUNA_PROFILE_TYPING_KEYS ?? (scenario === "height-growth" ? 24 : 300),
);
const repeatCount = Number(process.env.OSUNA_PROFILE_TYPING_REPEATS ?? 3);
const workspaceDigit = process.env.OSUNA_PROFILE_WORKSPACE_DIGIT ?? "1";
...
async function installTypingProbe(page, target) {
  await page.evaluate((probeTarget) => {
    globalThis.__OSUNA_TYPING_BENCHMARK_CLEANUP__?.();
    const element =
    ...
    globalThis.__OSUNA_TYPING_BENCHMARK__ = {
      get receivedKeys() {
        return sequence;
      },
      get paintedFrames() {
...
async function waitForMeasurement(page, expectedKeys) {
  await page.waitForFunction(
    (count) => globalThis.__OSUNA_TYPING_BENCHMARK__?.receivedKeys === count,
    expectedKeys,
    { timeout: 15_000 },
  );
  await page.waitForFunction(
    (count) =>
      globalThis.__OSUNA_TYPING_BENCHMARK__?.paintedAt.filter(Number.isFinite).length === count,
    expectedKeys,
    { timeout: 15_000 },
  );
}
...
async function measureTarget(page, input, target, originalText) {
  ...
  await page.evaluate(() => globalThis.__OSUNA_RESET_RENDER_PROFILE__?.());
  ...
  const diagnostics = await page.evaluate(() => ({
    state: globalThis.__OSUNA_TYPING_BENCHMARK__,
    commits: globalThis.__OSUNA_RENDER_PROFILE__ ?? [],
    renderReasons: globalThis.__OSUNA_RENDER_PROFILE_REASONS__ ?? {},
