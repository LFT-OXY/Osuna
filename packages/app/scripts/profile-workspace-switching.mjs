...
const baseUrl = process.env.OSUNA_PROFILE_APP_URL ?? "http://localhost:19010";
const digits = parseDigits(process.env.OSUNA_PROFILE_WORKSPACE_DIGITS ?? "1234567");
const lruSize = Number(process.env.OSUNA_PROFILE_WORKSPACE_LRU_SIZE ?? 3);
const retainedRepeatCount = Number(process.env.OSUNA_PROFILE_RETAINED_REPEATS ?? 1);
const warmQuietMs = Number(process.env.OSUNA_PROFILE_WARM_QUIET_MS ?? 300);
const finalWaitMs = Number(process.env.OSUNA_PROFILE_FINAL_WAIT_MS ?? 1_000);
const headless = process.env.OSUNA_PROFILE_HEADLESS !== "0";
const dumpCommits = process.env.OSUNA_PROFILE_DUMP_COMMITS === "1";
const cpuProfilePath = process.env.OSUNA_PROFILE_CPU_PATH;
const tracePath = process.env.OSUNA_PROFILE_TRACE_PATH;
const traceInvalidations = process.env.OSUNA_PROFILE_TRACE_INVALIDATIONS === "1";
const traceFocus = process.env.OSUNA_PROFILE_TRACE_FOCUS === "1";

function parseDigits(value) {
  const parsed = [...value].filter((digit) => /^[1-9]$/.test(digit));
  if (parsed.length === 0 || new Set(parsed).size !== parsed.length) {
    throw new Error("OSUNA_PROFILE_WORKSPACE_DIGITS must contain unique digits from 1 through 9");
  }
  return parsed;
}
    ...
    ({ profileFocus }) => {
      ...
      if (profileFocus) {
        ...
        HTMLElement.prototype.focus = function (...args) {
          ...
          globalThis.__OSUNA_FOCUS_PROFILE__ ??= [];
          globalThis.__OSUNA_FOCUS_PROFILE__.push({
            time: start,
...
async function waitForProfilerIdle(page) {
  ...
  while (Date.now() < deadline) {
    const count = await page.evaluate(() => globalThis.__OSUNA_RENDER_PROFILE__?.length ?? 0);
...
async function beginBrowserMeasurements(page, scenarioName) {
  await page.evaluate((name) => {
    globalThis.__OSUNA_RESET_RENDER_PROFILE__?.();
    globalThis.__OSUNA_WORKSPACE_SWITCH_BENCHMARK_CLEANUP__?.();
    globalThis.__OSUNA_FOCUS_PROFILE__ = [];
    ...
    globalThis.__OSUNA_WORKSPACE_SWITCH_BENCHMARK__ = measurements;
    globalThis.__OSUNA_RECORD_WORKSPACE_SWITCH_KEYDOWN__ = recordKeydown;
    globalThis.__OSUNA_WORKSPACE_SWITCH_BENCHMARK_CLEANUP__ = () => {
      observer.disconnect();
    };
  }, scenarioName);
...
async function runScenario(page, name, scenarioDigits, targets) {
  ...
  for (const digit of scenarioDigits) {
    await page.evaluate((nextDigit) => {
      globalThis.__OSUNA_RECORD_WORKSPACE_SWITCH_KEYDOWN__?.(nextDigit);
    }, digit);
    await page.keyboard.press(digit);
  }
  await page.keyboard.up("Meta");
  ...
  const result = await page.evaluate(() => ({
    measurements: globalThis.__OSUNA_WORKSPACE_SWITCH_BENCHMARK__,
    samples: globalThis.__OSUNA_RENDER_PROFILE__ ?? [],
    reasons: globalThis.__OSUNA_RENDER_PROFILE_REASONS__ ?? {},
    focusCalls: globalThis.__OSUNA_FOCUS_PROFILE__ ?? [],
  }));
  await page.evaluate(() => globalThis.__OSUNA_WORKSPACE_SWITCH_BENCHMARK_CLEANUP__?.());
  return {
