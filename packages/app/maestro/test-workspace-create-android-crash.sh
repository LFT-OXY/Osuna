...
render_flow() {
  ...
  perl -0pe '
    ...
    s/\$\{OSUNA_MAESTRO_PROJECT_NAME\}/$ENV{OSUNA_MAESTRO_PROJECT_NAME}/g;
  ' "$source" > "$target"
}
...
export OSUNA_MAESTRO_PROJECT_NAME="${OSUNA_MAESTRO_PROJECT_NAME:-$(basename "$OSUNA_MAESTRO_PROJECT_PATH")}"

echo "=== Workspace Create Android Crash Harness ==="
echo "Output dir: $OUT_DIR"
echo "App id: $OSUNA_MAESTRO_APP_ID"
echo "Android direct endpoint: $OSUNA_MAESTRO_DIRECT_ENDPOINT"
echo "Daemon websocket: $OSUNA_MAESTRO_DAEMON_WS_URL"
echo "Project: $OSUNA_MAESTRO_PROJECT_PATH"
echo "Project name: $OSUNA_MAESTRO_PROJECT_NAME"
...
const projectPath = process.env.OSUNA_MAESTRO_PROJECT_PATH;
const daemonUrl = process.env.OSUNA_MAESTRO_DAEMON_WS_URL;
