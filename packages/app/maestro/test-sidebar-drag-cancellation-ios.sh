...
export OSUNA_MAESTRO_DIRECT_ENDPOINT="${OSUNA_MAESTRO_DIRECT_ENDPOINT:-127.0.0.1:6777}"
export OSUNA_MAESTRO_DAEMON_WS_URL="${OSUNA_MAESTRO_DAEMON_WS_URL:-ws://127.0.0.1:6777/ws}"
export OSUNA_MAESTRO_DAEMON_HEALTH_URL="${OSUNA_MAESTRO_DAEMON_HEALTH_URL:-http://127.0.0.1:6777/api/health}"
...
if [ -z "${OSUNA_MAESTRO_IOS_UDID:-}" ]; then
  export OSUNA_MAESTRO_IOS_UDID
  OSUNA_MAESTRO_IOS_UDID="$({ xcrun simctl list devices booted -j || true; } | node -e '
    let input = "";
...
render_flow() {
  ...
  local target="$2"
  perl -0pe '
    s/^appId: com\.chinhae\.osuna$/appId: $ENV{OSUNA_MAESTRO_APP_ID}/m;
    s/\$\{OSUNA_MAESTRO_APP_ID\}/$ENV{OSUNA_MAESTRO_APP_ID}/g;
    s/\$\{OSUNA_MAESTRO_DIRECT_ENDPOINT\}/$ENV{OSUNA_MAESTRO_DIRECT_ENDPOINT}/g;
    s/\$\{OSUNA_MAESTRO_DRAG_A_NAME\}/$ENV{OSUNA_MAESTRO_DRAG_A_NAME}/g;
    s/\$\{OSUNA_MAESTRO_DRAG_B_NAME\}/$ENV{OSUNA_MAESTRO_DRAG_B_NAME}/g;
    s/\$\{OSUNA_MAESTRO_DRAG_Z_NAME\}/$ENV{OSUNA_MAESTRO_DRAG_Z_NAME}/g;
  ' "$source" > "$target"
}

for project_name in \
  "$OSUNA_MAESTRO_DRAG_A_NAME" \
  "$OSUNA_MAESTRO_DRAG_B_NAME" \
  "$OSUNA_MAESTRO_DRAG_Z_NAME"; do
...
const client = new DaemonClient({
  url: process.env.OSUNA_MAESTRO_DAEMON_WS_URL,
...
const { DaemonClient } = await import(moduleUrl);
const projectNames = [
  process.env.OSUNA_MAESTRO_DRAG_A_NAME,
  process.env.OSUNA_MAESTRO_DRAG_B_NAME,
  process.env.OSUNA_MAESTRO_DRAG_Z_NAME,
];
const client = new DaemonClient({
  url: process.env.OSUNA_MAESTRO_DAEMON_WS_URL,
...
echo "Running sidebar drag cancellation regression on $OSUNA_MAESTRO_IOS_UDID"
echo "Artifacts: $OUT_DIR"
(cd "$OUT_DIR" && maestro test "$FLOW" --udid "$OSUNA_MAESTRO_IOS_UDID")
