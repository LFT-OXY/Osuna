...
#define METHOD(name) Java_com_chinhae_osuna_diffprototype_OsunaDiffPrototypeModule_##name
...
extern "C" JNIEXPORT jdoubleArray JNICALL METHOD(prepareDocument)(
  ...
  try {
    ...
    osuna::diff::prepare(*document, SkFontMgr_New_Android(nullptr), fontFamily.c_str(), size, job->cancelled);
...
extern "C" JNIEXPORT jdoubleArray JNICALL METHOD(readDocument)(
  ...
  try {
    ...
    return numbers(env, osuna::diff::read(*document, start, count));
  } catch (const std::exception& error) {
    fail(env, error);
    return nullptr;
  }
