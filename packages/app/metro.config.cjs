...
const isFdroidBuild = process.env.OSUNA_FDROID_BUILD === "1";
...
const customWebPlatform = (process.env.OSUNA_WEB_PLATFORM ?? "")
  .trim()
  .replace(/^\./, "")
  .toLowerCase();
...
if (process.env.OSUNA_SERVE_SIM_PREVIEW === "1") {
  ...
  config.server.enhanceMiddleware = (metroMiddleware, server) => {
    ...
    const serveSimulator = simMiddleware({
      basePath: "/.sim",
      device: process.env.OSUNA_SERVE_SIM_DEVICE_UDID,
    });
    return (req, res, next) => {
